<?php //include('include/usertypecheck.php');
    ...
    function getAllProductWithCurrentStock($alphabet = null, $like = null, $expired = 0, $id = [], $enddate = null, $mrp = null, $gst_id = null, $onlywithgst = 0){
        ...
        if(isset($expired) && $expired == 1){
            // $where[] = "p.ex_date >= '".date('Y-m-d')."'";    /// Changes By kartik >= ///
        ...
        if($res && mysqli_num_rows($res) > 0){
            while ($row = mysqli_fetch_assoc($res)) {
                /* PTR COUNT*/
                $mrp = (isset($row['mrp']) && $row['mrp'] != '' && $row['mrp'] > 0) ? $row['mrp'] : 1;
                $gst = (isset($row['igst']) && $row['igst'] != '') ? $row['igst'] : 0;
                $ratio = (isset($row['ratio']) && $row['ratio'] != '' && $row['ratio'] > 0) ? $row['ratio'] : 1;
                
                $ptr = (($mrp*$gst)/(100+$gst));
                $ptr = ($mrp-$ptr);
                $ptr = ($ptr/$ratio);
                $row['ptr'] = number_format($ptr, 2, '.', '');
                if((isset($row['discount']) && $row['discount'] == 1) && (isset($row['discount_per']) && $row['discount_per'] != '' && $row['discount_per'] > 0)){
                    $dis_amount = ($ptr*$row['discount_per']/100);
                    $ptr -= $dis_amount;
                }
                $row['rate'] = number_format($ptr, 2, '.', '');
                ...
                if(isset($enddate) && $enddate != ''){
                    $purchaseReturnQ .= " AND pr.debit_note_date <= '".$enddate."'";
                }
                ...
                $saleReturnQ = "SELECT SUM(srd.qty) as total_qty, SUM(srd.freeqty) as total_free_qty FROM sale_return_details srd INNER JOIN sale_return sr ON srd.sale_return_id = sr.id WHERE srd.product_id = '".$row['id']."' AND sr.pharmacy_id = '".$pharmacy_id."' AND sr.financial_id = '".$financial_id."'";
                if(isset($enddate) && $enddate != ''){
                ...
                $currentstock = ($opening_qty) + ($ratio*$total_purchase_qty) + ($total_purchase_freeqty) - ($total_consumption_qty) + ($total_inward_qty) - ($total_outward_qty) - ($total_sale_qty+$total_sale_freeqty) - ($total_purchase_return_qty+$total_purchase_return_freeqty) + ($total_sale_return_qty+$total_sale_return_freeqty);
    ...
    function getAllNotification(){
        ...
        $date = date('Y-m-d');
            ...
            if(isset($_SESSION['auth']['pharmacy_user_type']) && $_SESSION['auth']['pharmacy_user_type'] == 'ihis'){
                /*-------------NOTIFICATION FOR IHIS PRESCRIPTION DATA - START-------------------*/
                $prescription = getIhisPrescriptionNotification();
                if(!empty($prescription)){
                    foreach($prescription as $key => $value){
                        $arr['type'] = 'ihis_prescription';
                        $arr['typename'] = 'I-HIS Prescription';
                        $arr['name'] = (isset($value['type'])) ? $value['type'].' Prescription' : 'I-HIS Prescription';
                        $arr['subname'] = 'Patient Name: '.$value['patient_name'].' | Doctor Name: '.$value['doctor_name'];
                        $arr['class'] = $value['type'].'_'.$value['id'];
                        $arr['url'] = 'view-prescription.php?billtype=ihis&id='.$value['group_id'].'&type='.$value['type'].'&doctor_name='.$value['doctor_name'].'&prescriptionid='.$value['id'];
                        $data[] = $arr;
                    }
                }
                /*-------------NOTIFICATION FOR IHIS PRESCRIPTION DATA - END-------------------*/
            }elseif(isset($_SESSION['auth']['pharmacy_user_type']) && $_SESSION['auth']['pharmacy_user_type'] == 'eclinic'){
                /*-------------NOTIFICATION FOR ECLINIC PRESCRIPTION DATA - START-------------------*/
                    $prescription = getEclinicPrescriptionNotification();
                    if(!empty($prescription)){
                        foreach($prescription as $key => $value){
                            $arr['type'] = 'eclinic_prescription';
                            $arr['typename'] = 'Eclinic Prescription';
                            $arr['name'] = (isset($value['type'])) ? $value['type'].' Prescription' : 'Eclinic Prescription';
                            $arr['subname'] = 'Patient Name: '.$value['patient_name'].' | Doctor Name: '.$value['doctor_name'];
                            $arr['class'] = $value['type'].'_'.$value['id'];
                            $arr['url'] = 'view-prescription.php?billtype=eclinic&id='.$value['group_id'].'&type='.$value['type'].'&doctor_name='.$value['doctor_name'].'&prescriptionid='.$value['id'];
                            $data[] = $arr;
                        }
                    }
                /*-------------NOTIFICATION FOR ECLINIC PRESCRIPTION DATA - END-------------------*/
            }  
            ...
            if(isset($_SESSION['auth']['user_type']) && $_SESSION['auth']['user_type'] != 'user'){
                ...
                $drugLicenseQ = "SELECT id, drug_lic, pharmacy_name FROM pharmacy_profile ";
                if($_SESSION['auth']['user_type'] == 'admin'){
                    $drugLicenseQ .= "WHERE id = '".$pharmacy_id."' ";
                }else{
                    $drugLicenseQ .= "WHERE created_by = '".$_SESSION['auth']['id']."' ";
                }
                $drugLicenseQ .= " AND drug_lic <= ('".$date."' + INTERVAL ".$notification_day." MONTH) AND drug_lic IS NOT NULL AND drug_lic != '0000-00-00'";
                ...
                $shopActQ = "SELECT id, exp_date, pharmacy_name FROM pharmacy_profile ";
                if($_SESSION['auth']['user_type'] == 'admin'){
                    $shopActQ .= "WHERE id = '".$pharmacy_id."' ";
                }else{
                    $shopActQ .= "WHERE created_by = '".$_SESSION['auth']['id']."' ";
                }
                $shopActQ .= " AND exp_date <= ('".$date."' + INTERVAL ".$notification_day." MONTH) AND exp_date IS NOT NULL AND exp_date != '0000-00-00'";
                ...
                $regnoQ .= " AND ppd.pharmacist_reg_date <= ('".$date."' + INTERVAL ".$notification_day." MONTH) AND ppd.pharmacist_reg_date IS NOT NULL AND ppd.pharmacist_reg_date != '0000-00-00'";
    ...
    function countRunningBalance($id = null, $from = null, $to = null, $is_financial = 0){
        ...
        if((isset($id) && $id != '') && (isset($pharmacy_id) && $pharmacy_id != '')){
            ...
            /*--------------------------------------------CALCULATE DEBIT TAX BILLING START---------------------------------------*/
            $taxbillingQ = "SELECT SUM(sale_amount) as total_bill FROM tax_billing WHERE pharmacy_id = '".$pharmacy_id."' AND customer_id = '".$id."' AND bill_type = 'Debit'";
            ...
            /*--------------------------------------------CALCULATE DEBIT TAX BILLING END-----------------------------------------*/
            
            /*--------------------------------------------CALCULATE SALE RETURN START---------------------------------------*/
            $saleReturnQ = "SELECT SUM(finalamount) as total_return FROM sale_return WHERE pharmacy_id = '".$pharmacy_id."' AND customer_id = '".$id."'";
            if(isset($is_financial) && $is_financial == 1){
            ...
            if($saleReturnR && mysqli_num_rows($saleReturnR) > 0){
                ...
                $total_sale_return = (isset($saleReturnRow['total_return']) && $saleReturnRow['total_return'] != '') ? $saleReturnRow['total_return'] : 0;
            }
            /*--------------------------------------------CALCULATE SALE RETURN END-----------------------------------------*/


            /*--------------------------------------------CALCULATE DEBIT PURCHASE START---------------------------------------*/
            ...
            /*--------------------------------------------CALCULATE DEBIT PURCHASE END-----------------------------------------*/
            
            
            /*--------------------------------------------CALCULATE PURCHASE RETURN START---------------------------------------*/
            $purchaseReturnQ = "SELECT SUM(finalamount) as total_return FROM purchase_return WHERE pharmacy_id = '".$pharmacy_id."' AND vendor_id = '".$id."'";
            if(isset($is_financial) && $is_financial == 1){
            ...
            if($purchaseReturnR && mysqli_num_rows($purchaseReturnR) > 0){
                ...
                $total_purchase_return = (isset($purchaseReturnRow['total_return']) && $purchaseReturnRow['total_return'] != '') ? $purchaseReturnRow['total_return'] : 0;
            }
    ...
    function countCashRunningBalance($from = null, $to = null, $is_financial = 0){
        ...
        /*----------------------------------CALCULATE OPENING BALANCE START----------------------------*/
            ...
            $openingBalanceQ = "SELECT opening_balance, opening_balance_type FROM ledger_master WHERE group_id = 6 AND pharmacy_id = '".$pharmacy_id."'";
            $openingBalanceR = mysqli_query($conn, $openingBalanceQ);
            if($openingBalanceR && mysqli_num_rows($openingBalanceR) > 0){
                while($openingBalanceRow = mysqli_fetch_assoc($openingBalanceR)){
                    $opening_balance_tmp = (isset($openingBalanceRow['opening_balance']) && $openingBalanceRow['opening_balance'] != '') ? $openingBalanceRow['opening_balance'] : 0;
                    $opening_balance += (isset($openingBalanceRow['opening_balance_type']) && $openingBalanceRow['opening_balance_type'] == 'DB') ? $opening_balance_tmp : -$opening_balance_tmp;
                }
            }
        ...
        /*----------------------------------CALCULATE PURCHASE BILLING CASH START--------------------------------*/
            ...
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
              $cashBillPurchaseQ .= " AND vouchar_date >= '".$from."' AND vouchar_date <= '".$to."'";
            }
        ...
        global $conn;
            ...
            if(isset($pharmacy_id) && $pharmacy_id != ''){
                ...
                /*----------------------------------CALCULATE PURCHASE BILLING CASH START--------------------------------*/
                    ...
                    if((isset($from) && $from != '') && (isset($to) && $to != '')){
                      $cashBillPurchaseQ .= " AND pr.vouchar_date >= '".$from."' AND pr.vouchar_date <= '".$to."'";
                    }
                    $cashBillPurchaseR = mysqli_query($conn, $cashBillPurchaseQ);
                    if($cashBillPurchaseR && mysqli_num_rows($cashBillPurchaseR) > 0){
            ...
            $countRunningBalance = countCashRunningBalance($from, $to);
    ...
    function customerLedger($customer_id = null, $from = null, $to = null, $is_financial = 0){
        ...
        if((isset($pharmacy_id) && $pharmacy_id != '') && (isset($customer_id) && $customer_id != '')){
            ...
            /*----------------------------------------------------COUNT TAX BILL START-------------------------------------------------------*/
            $billQ =  "SELECT id, invoice_date, invoice_no, sale_amount, bill_type FROM tax_billing WHERE pharmacy_id = '".$pharmacy_id."' AND customer_id = '".$customer_id."' AND bill_type = 'Debit'";
            if(isset($is_financial) && $is_financial == 1){
            ...
            if($billR && mysqli_num_rows($billR) > 0){
                while ($billRow = mysqli_fetch_assoc($billR)) {
                    ...
                    $billarr['debit'] = (isset($billRow['sale_amount']) && $billRow['sale_amount'] != '') ? $billRow['sale_amount'] : 0;
                    $billarr['credit'] = '';
            ...
            /*----------------------------------------------------SALE RETURN START-----------------------------------------------------*/
            $saleReturnQ = "SELECT id, credit_note_no, credit_note_date, remarks, finalamount FROM sale_return WHERE pharmacy_id = '".$pharmacy_id."' AND customer_id = '".$customer_id."'";
            if(isset($is_financial) && $is_financial == 1){
                $saleReturnQ .= " AND financial_id = '".$financial_id."'";
            }
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
            ...
            if(isset($saleReturnR) && mysqli_num_rows($saleReturnR) > 0){
                while($saleReturnRow = mysqli_fetch_assoc($saleReturnR)){
                    $returnarr['type'] = 'taxBillReturn';
                    $returnarr['id'] = $saleReturnRow['id'];
                    $returnarr['date'] = (isset($saleReturnRow['credit_note_date'])) ? $saleReturnRow['credit_note_date'] : '';
                    $returnarr['sr_no'] = (isset($saleReturnRow['credit_note_no'])) ? $saleReturnRow['credit_note_no'] : '';
                    $creditNoteNo = (isset($saleReturnRow['credit_note_no']) && $saleReturnRow['credit_note_no'] != '') ? ' - '.$saleReturnRow['credit_note_no'] : '';
                    $remarks = (isset($saleReturnRow['remarks']) && $saleReturnRow['remarks'] != '') ? ' - '.$saleReturnRow['remarks'] : '';
                    $returnarr['narration'] = 'Credit Note'.$creditNoteNo.''.$creditNoteNo.''.$remarks;
                    $returnarr['debit'] = '';
                    $returnarr['credit'] = (isset($saleReturnRow['finalamount']) && $saleReturnRow['finalamount'] != '') ? $saleReturnRow['finalamount'] : 0;
                    $returnarr['url'] = 'sales-return.php?id='.$saleReturnRow['id'];
                    $data[] = $returnarr;
                }
            }
            /*----------------------------------------------------SALE RETURN END------------------------------------------------------*/
    ...
    function vendorLedger($vendor_id = null, $from = null, $to = null, $is_financial = 0){
        ...
        if((isset($pharmacy_id) && $pharmacy_id != '') && (isset($vendor_id) && $vendor_id != '')){
            ...
            /*---------------------------------------------COUNT DEBIT PURCHASE BILL START------------------------------------------------*/
            $billQ =  "SELECT id, invoice_date, invoice_no, total_total as final_amount, purchase_type FROM purchase WHERE pharmacy_id = '".$pharmacy_id."' AND vendor = '".$vendor_id."' AND purchase_type = 'Debit'";
            if(isset($is_financial) && $is_financial == 1){
                $billQ .= " AND financial_id = '".$financial_id."'";
            }
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
            ...
            if($billR && mysqli_num_rows($billR) > 0){
                while ($billRow = mysqli_fetch_assoc($billR)) {
                    ...
                    $billarr['date'] = (isset($billRow['invoice_date'])) ? $billRow['invoice_date'] : '';
                    $billarr['sr_no'] = (isset($billRow['invoice_no'])) ? $billRow['invoice_no'] : '';
                    $PinvoiceNo = (isset($billRow['invoice_no']) && trim($billRow['invoice_no']) != '') ? ' - '.$billRow['invoice_no'] : '';
                    $billarr['narration'] = 'Purchase Bill'.$PinvoiceNo;
                    $billarr['debit'] = '';
            ...
            if($purchaseReturnR && mysqli_num_rows($purchaseReturnR) > 0){
                while($purchaseReturnRow = mysqli_fetch_assoc($purchaseReturnR)){
                    $returnarr['type'] = 'purchaseBill';
                    $returnarr['id'] = $purchaseReturnRow['id'];
                    $returnarr['date'] = (isset($purchaseReturnRow['debit_note_date'])) ? $purchaseReturnRow['debit_note_date'] : '';
                    $returnarr['sr_no'] = (isset($purchaseReturnRow['debit_note_no'])) ? $purchaseReturnRow['debit_note_no'] : '';
                    ...
                    $returnarr['narration'] = 'Debit Note'.$debitnoteno.''.$remarks;
                    $returnarr['debit'] = (isset($purchaseReturnRow['finalamount']) && $purchaseReturnRow['finalamount'] != '') ? $purchaseReturnRow['finalamount'] : 0;
                    $returnarr['credit'] = '';
                    $returnarr['url'] = 'purchase-return.php?id='.$purchaseReturnRow['id'];
                    $data[] = $returnarr;
                }
            }
        ...
        $finaldata['opening_balance'] = $opening_balance;
        $finaldata['data'] = $final_array;
        $countRunningBalance = countRunningBalance($vendor_id, $from, $to);
        $finaldata['running_balance'] = (isset($countRunningBalance['running_balance']) && $countRunningBalance['running_balance'] != '') ? $countRunningBalance['running_balance'] : 0;
        return $finaldata;
    }
    ...
    function bankLedger($bank_id = null, $from = null, $to = null, $is_financial = 0){
        ...
        $finaldata['opening_balance'] = $opening_balance;
        $finaldata['data'] = $final_array;
        $bankRunningBalance = bankrunningbalance($bank_id, $from, $to);
    ...
    function dailysales($from = null , $to = null){
        ...
        if((isset($pharmacy_id) && $pharmacy_id != '')){ 
            ...
            $dailysaleqry =  "SELECT l.id,t.id ,t.owner_id,t.invoice_date, t.invoice_no, t.final_amount, t.cr_db_type, l.name, c.name as city, d.name as doctor FROM (((tax_billing AS t INNER JOIN ledger_master AS l ON t.customer_id = l.id) LEFT JOIN doctor_profile AS d ON t.doctor = d.id) LEFT JOIN own_cities AS c ON c.id = l.city) WHERE t.pharmacy_id = '".$pharmacy_id."' AND t.financial_id = '".$financial_id."'"; 
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
    ...
    */
    ...
    function roundoffpurchase($from = null , $to = null, $financial = null){
        ...
        if((isset($pharmacy_id) && $pharmacy_id != '')){ 
            ...
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
                $billQ .= " AND p.invoice_date >= '".$from."' AND p.invoice_date <= '".$to."'";   
            }
            if(isset($_GET['financial']) && $_GET['financial'] != ''){
    ...
    //         $query = "SELECT * FROM ledger_master WHERE id ='".$id."'"; 
    //         $qrySel = mysqli_query($conn,$query);   
    //         $check = mysqli_num_rows($qrySel);
    //         if($check > 0) {
         
    //             $getLedger = mysqli_fetch_assoc($qrySel);
    //             $opening_balance = $getLedger['opening_balance'];
    //             $opening_balance_type = $getLedger['opening_balance_type'];
    ...
    //             $cashReceiptQ =  "SELECT * FROM accounting_cash_management WHERE pharmacy_id = '".$pharmacy_id."' AND perticular = '".$id."' AND payment_type = 'cash_receipt'";
    //             if(isset($is_financial) && $is_financial == 1){
    //                 $cashReceiptQ .= " AND financial_id = '".$financial_id."'";
    //             }
    //             if((isset($from) && $from != '') && (isset($to) && $to != '')){
    ...
    //                     $allarry['voucher_date'] = (isset($cashReceiptRow['voucher_date'])) ? $cashReceiptRow['voucher_date'] : '';
    //                     $allarry['narration'] = 'Cash Receipt';
    //                     $allarry ['voucher_no'] = (isset($cashReceiptRow['voucher_no'])) ? $cashReceiptRow['voucher_no'] : '';
    //                     $allarry ['cheque_no'] =  '';
    //                     $allarry['debit'] =  '';
    //                     $allarry['credit'] = (isset($cashReceiptRow['amount'])) ? $cashReceiptRow['amount'] : ''; 
    //                     $allarry['url'] =  'accounting-cash-management.php?id='.$cashReceiptRow['id'];
    //                     $data[] = $allarry;
    //                 }
    //             }
    ...
    //             $cashPaymentQ =  "SELECT * FROM accounting_cash_management WHERE pharmacy_id = '".$pharmacy_id."' AND perticular = '".$id."' AND payment_type = 'cash_payment'";
    //             if(isset($is_financial) && $is_financial == 1){
    //                 $cashPaymentQ .= " AND financial_id = '".$financial_id."'";
    //             }
    //             if((isset($from) && $from != '') && (isset($to) && $to != '')){
    ...
    //                     $allcash['voucher_date'] = (isset($cashPaymentRow['voucher_date'])) ? $cashPaymentRow['voucher_date'] : '';
    //                     $allcash['narration'] = 'Cash Payment';
    //                     $allcash['voucher_no'] = (isset($cashPaymentRow['voucher_no'])) ? $cashPaymentRow['voucher_no'] : '';
    //                     $allcash ['cheque_no'] =  '';
    //                     $allcash['debit'] = (isset($cashPaymentRow['amount'])) ? $cashPaymentRow['amount'] : '';
    //                     $allcash['credit'] = '';
    //                     $allcash['url'] =  'accounting-cash-management.php?id='.$cashPaymentRow['id'];
    //                     $data[] = $allcash;
    //                 }
    //             }
    ...
    //             $checkPaymentQ =  "SELECT * FROM accounting_cheque WHERE pharmacy_id = '".$pharmacy_id."' AND perticular = '".$id."' AND voucher_type = 'payment'";
    //             if(isset($is_financial) && $is_financial == 1){
    ...
    //                 while($chPaymntRow = mysqli_fetch_assoc($checkPaymentR)){
    //                     $allpaymnt['voucher_date'] = (isset($chPaymntRow['voucher_date'])) ? $chPaymntRow['voucher_date'] : '';
    //                     $allpaymnt['narration'] = 'Cheque Payment';
    //                     $allpaymnt ['voucher_no'] = (isset($chPaymntRow['voucher_no'])) ? $chPaymntRow['voucher_no'] : '';
    //                     $allpaymnt ['cheque_no'] = (isset($chPaymntRow['cheque_no'])) ? $chPaymntRow['cheque_no'] : '';
    //                     $allpaymnt['debit'] = (isset($chPaymntRow['amount'])) ? $chPaymntRow['amount'] :  '';
    //                     $allpaymnt['credit'] = '';
    //                     $allpaymnt['url'] =  'accounting-cheque.php?id='.$chPaymntRow['id'];
    //                     $data[] = $allpaymnt;
    //                 }
    //             }
    //             /*-----------------------------COUNT CHECK PAYMENT END---------------------------------------*/
    ...
    //             $checkreceiptQ =  "SELECT * FROM accounting_cheque WHERE pharmacy_id = '".$pharmacy_id."' AND perticular = '".$id."' AND voucher_type = 'receipt'";
    //             if(isset($is_financial) && $is_financial == 1){
    ...
    //                 while($chRecptRow = mysqli_fetch_assoc($checkreceiptR)){
    //                     $allrecpt['voucher_date'] = (isset($chRecptRow['voucher_date'])) ? $chRecptRow['voucher_date'] : '';
    //                     $allrecpt['narration'] = 'Cheque Receipt';
    //                     $allrecpt ['voucher_no'] = (isset($chRecptRow['voucher_no'])) ? $chRecptRow['voucher_no'] : '';
    //                     $allrecpt ['cheque_no'] = (isset($chRecptRow['cheque_no'])) ? $chRecptRow['cheque_no'] : '';
    //                     $allrecpt['debit'] =  '';
    //                     $allrecpt['credit'] = (isset($chRecptRow['amount'])) ? $chRecptRow['amount'] : '';
    //                     $allrecpt['url'] =  'accounting-cheque.php?id='.$chRecptRow['id'];
    //                     $data[] = $allrecpt;
    //                 }
    //             }
    //             /*-----------------------------------------COUNT CHECK RECEIPT END------------------------------------------*/
    
    //             /*------------------------------------------JOURANAL VOUCHAR DETAIL START-------------------------------------*/
    //             $jurnalQ = "SELECT * FROM journal_vouchar_details jvd INNER JOIN journal_vouchar jv ON jvd.voucher_id = jv.id WHERE jvd.particular = '".$id."' AND jv.pharmacy_id = '".$pharmacy_id."'";
    //             if(isset($is_financial) && $is_financial == 1){
    //                 $jurnalQ .= " AND jv.financial_id = '".$financial_id."'";
    //             }
    //             if((isset($from) && $from != '') && (isset($to) && $to != '')){
    ...
    //                     $alljurnal['voucher_date'] = (isset($jurnalRow['voucher_date'])) ? $jurnalRow['voucher_date'] : '';
    //                     $alljurnal['narration'] = 'Journal Vouchar';
    //                     $alljurnal ['voucher_no'] =  '';
    //                     $alljurnal ['cheque_no'] =  '';
    //                     $alljurnal['credit'] = (isset($jurnalRow['credit'])) ? $jurnalRow['credit'] : '';
    //                     $alljurnal['debit'] = (isset($jurnalRow['debit'])) ? $jurnalRow['debit'] : '';
    //                     $alljurnal['url'] =  'journal-vouchar.php?id='.$jurnalRow['id'];
    //                     $data[] =  $alljurnal;
    //                 }
    //             }
    //             /*------------------------------------------JOURANAL VOUCHAR DETAIL END-------------------------------------*/ 
    ...
    //             $finaldata['data'] = $data;
    //             $finaldata['opening_balance'] = $opening_balance;
    //             $count =  countRunningBalance($id ,$from, $to);
    //             $finaldata['running_balance'] = (isset($count['running_balance']) && $count['running_balance'] != '') ? $count['running_balance'] : 0;
    
    //             return $finaldata;
          
    // }

    //-------------------------------------------------------------------------ALL GROUP LEDGER -----------------------------------START----------------------//  
    function allLedgerdetaills($id = null, $from = null, $to = null, $is_financial = 0){
        global $conn;
        global $pharmacy_id;
        global $financial_id;
        ...
        if($ledgerR && mysqli_num_rows($ledgerR) > 0){
            ...
            if(isset($ledgerRow['acc_flag']) && $ledgerRow['acc_flag'] == $account_flag['GENERAL_SALE']){
                ...
                if(isset($ledgerRow['group_id']) && $ledgerRow['group_id'] == 10){//for customer only
                    ...
                    $taxbillingQ = "SELECT id, invoice_date, invoice_no, bill_type, sale_amount FROM tax_billing WHERE pharmacy_id = '".$pharmacy_id."' AND customer_id = '".$id."' AND bill_type = 'Debit'";
                    if(isset($is_financial) && $is_financial == 1){
                        $taxbillingQ .= " AND financial_id = '".$financial_id."'";
                    }
                    if((isset($from) && $from != '') && (isset($to) && $to != '')){
                    ...
                    if($taxbillingR && mysqli_num_rows($taxbillingR) > 0){
                        while ($taxbillingRow = mysqli_fetch_assoc($taxbillingR)) {
                            ...
                            $tmp1['debit'] = (isset($taxbillingRow['sale_amount']) && $taxbillingRow['sale_amount'] != '') ? $taxbillingRow['sale_amount'] : 0;
                            $tmp1['credit'] = '';
                ...
                if(isset($ledgerRow['group_id']) && $ledgerRow['group_id'] == 14){//for vendor only
                    ...
                    if($purchaseR && mysqli_num_rows($purchaseR) > 0){
                        while ($purchaseRow = mysqli_fetch_assoc($purchaseR)) {
                            ...
                            $tmp3['url'] = 'purchase.php?id='.$tmp2['id'];
                            $detail[] = $tmp3;
                        }
                    }
                }
  ...
  function getGeneralSaleReport($from = null, $to = null, $is_financial = 0){
        ...
        $query = "SELECT tb.id,  tb.invoice_date, tb.invoice_no, tb.bill_type, tb.final_amount,  ct.name as customer_city , lg.name as l_customer_name, lg.mobile as l_customer_mobile FROM tax_billing tb LEFT JOIN own_cities ct ON tb.city_id = ct.id LEFT JOIN ledger_master lg ON tb.customer_id = lg.id WHERE tb.pharmacy_id = $pharmacy_id ";
        if(isset($is_financial) && $is_financial == 1){
    ...
    function getGstOnSaleReport($from = null, $to = null, $is_financial = 0){
        ...
        $query = "SELECT tb.id, tb.invoice_date, tb.invoice_no, tb.bill_type, (tb.totalcgst+tb.totalsgst) as amount, ct.name as customer_city, lg.name as l_customer_name, lg.mobile as l_customer_mobile FROM tax_billing tb LEFT JOIN own_cities ct ON tb.city_id = ct.id LEFT JOIN ledger_master lg ON tb.customer_id = lg.id WHERE tb.pharmacy_id = $pharmacy_id AND tb.totalcgst IS NOT NULL AND tb.totalcgst > 0";
        if(isset($is_financial) && $is_financial == 1){
            $query .= " AND tb.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
    ...
    function getIgstOnSaleReport($from = null, $to = null, $is_financial = 0){
        ...
        $query = "SELECT tb.id, tb.invoice_date, tb.invoice_no, tb.bill_type, tb.totaligst as amount, ct.name as customer_city, lg.name as l_customer_name, lg.mobile as l_customer_mobile FROM tax_billing tb LEFT JOIN own_cities ct ON tb.city_id = ct.id LEFT JOIN ledger_master lg ON tb.customer_id = lg.id WHERE tb.pharmacy_id = $pharmacy_id AND tb.totaligst IS NOT NULL AND tb.totaligst > 0";
        if(isset($is_financial) && $is_financial == 1){
            $query .= " AND tb.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
    ...
    function getSaleAccReport($from = null, $to = null, $is_financial = 0){
        ...
        $query = "SELECT tb.id, tb.invoice_date, tb.invoice_no, tb.bill_type, (tb.alltotalamount+tb.couriercharge_val) as amount, ct.name as customer_city, lg.name as l_customer_name, lg.mobile as l_customer_mobile FROM tax_billing tb LEFT JOIN own_cities ct ON tb.city_id = ct.id LEFT JOIN ledger_master lg ON tb.customer_id = lg.id WHERE tb.pharmacy_id = $pharmacy_id AND tb.alltotalamount IS NOT NULL AND tb.alltotalamount > 0";
        if(isset($is_financial) && $is_financial == 1){
            $query .= " AND tb.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
...
function getSaleAccOgsReport($from = null, $to = null, $is_financial = 0){
        ...
        $query = "SELECT tb.id,  tb.invoice_date, tb.invoice_no, tb.bill_type, (tb.alltotalamount-tb.discount_rs) as amount, ct.name as customer_city, lg.name as l_customer_name, lg.mobile as l_customer_mobile FROM tax_billing tb LEFT JOIN own_cities ct ON tb.city_id = ct.id LEFT JOIN own_states st ON ct.state_id = st.id LEFT JOIN ledger_master lg ON tb.customer_id = lg.id WHERE tb.pharmacy_id = $pharmacy_id AND tb.alltotalamount IS NOT NULL AND tb.alltotalamount > 0 AND st.state_code_gst != '".$current_state_code."'";
        if(isset($is_financial) && $is_financial == 1){
            $query .= " AND tb.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
...
function getClosingStockReport ($from = null, $to = null, $is_financial = 0){
 ...
 return $data;
  ...
  function TaxLiabilityReport ($from = null, $to = null, $is_financial = 0){
   ...
   $acm =  "SELECT id ,voucher_date , voucher_no ,amount FROM `accounting_cash_management` where perticular = 61  AND amount > 0 AND amount IS NOT NULL AND pharmacy_id = '".$pharmacy_id."'";
   if(isset($is_financial) && $is_financial == 1){
   ...
   if((isset($from) && $from != '') && (isset($to) && $to != '')){
    ...
    $Acheque = "SELECT id , voucher_no , voucher_date, amount FROM `accounting_cheque` WHERE perticular = 61 AND amount > 0 AND amount IS NOT NULL AND pharmacy_id = '".$pharmacy_id."'";
    if(isset($is_financial) && $is_financial == 1){
    ...
    if((isset($from) && $from != '') && (isset($to) && $to != '')){
     ...
     $jvd = "SELECT jv.id, jv.voucher_date , jvd.debit , jvd.credit FROM `journal_vouchar` jv INNER JOIN `journal_vouchar_details` jvd WHERE jv.id = jvd.voucher_id AND jvd.particular = 61 AND jv.pharmacy_id = '".$pharmacy_id."'";
...
function getAllProductSale($id = [], $startdate = null , $enddate = null){
   ...
   global $conn;
    ...
    // added by gautam makwana 19-01-19 for get cash payment/receipt voucher no
    ...
    // added by gautam makwana 19-01-19 for get bank payment/receipt voucher no
    ...
    // added by gautam makwana 17-01-19 for get journal voucher no
    ...
    function getInvoiceNo($bill_type = null){
        ...
        $pefixQ = "SELECT id, pefix_name_cash, pefix_name_debit FROM `sale_pefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_array($pefixR);
            $pefix_row['pefix_name_cash'] = (isset($pefixRow['pefix_name_cash'])) ? $pefixRow['pefix_name_cash'] : '';
            $pefix_row['pefix_name_debit'] = (isset($pefixRow['pefix_name_debit'])) ? $pefixRow['pefix_name_debit'] : '';
        }else{
    ...
    function getpurchaseinvoiceno($bill_type = null){
        ...
        $pefixQ = "SELECT id, purchase_cash, purchase_debit FROM `sale_pefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_array($pefixR);
    ...
    function getDebitNoteNo(){
        ...
        $pefixQ = "SELECT id, purchase_debit_note FROM `sale_pefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_assoc($pefixR);
            $debit_note_prefix = (isset($pefixRow['purchase_debit_note']) && $pefixRow['purchase_debit_note'] != '') ? $pefixRow['purchase_debit_note'] : '';
        }else{
            $debit_note_prefix = '';
        }
    ...
    function getaccountingcashinvoiceno($bill_type = null){
        ...
        $pefixQ = "SELECT id, cash_management_cash, cash_management_receipt FROM `sale_pefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_array($pefixR);
    ...
    function getvendorreceiptno(){
        ...
        $pefixQ = "SELECT id, vendor_payments FROM `sale_pefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_assoc($pefixR);
    ...
    function getcustomerreceiptno(){
        ...
        $pefixQ = "SELECT id, customer_receipts FROM `sale_pefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_assoc($pefixR);
    ...
    function getaccountingchequevoucher($bill_type = null){
        ...
        $pefixQ = "SELECT id, accounting_bank_cheque, accounting_bank_cheque_receipt FROM `sale_pefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_array($pefixR);
    ...
    function getpaymentvoucher($bill_type = null){
        ...
        $pefixQ = "SELECT id, payment_tax, payment_tax_free FROM `sale_pefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_array($pefixR);
    ...
    function getcourierpaymentvoucherno(){
        ...
        $pefixQ = "SELECT id, courier_payment FROM `sale_pefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_assoc($pefixR);
    ...
    //THIS FUNCTION USED TO DELIVERY CHALLAN VOUCHER NUMBER GENERATE
    ...
    function getChallanInvoiceNo($bill_type = null){
        ...
        $pefixQ = "SELECT id, challan_cash, challan_debit FROM `sale_pefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_array($pefixR);
    ...
    function getCreditNoteNo(){
        ...
        $pefixQ = "SELECT id, sale_credit_note FROM `sale_pefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_assoc($pefixR);
            $credit_note_prefix = (isset($pefixRow['sale_credit_note']) && $pefixRow['sale_credit_note'] != '') ? $pefixRow['sale_credit_note'] : '';
        }else{
            $credit_note_prefix = '';
        }
    ...
    /*---THIS FUNCTION IS USED TO DELETE ADMIN - GAUTAM MAKWANA - 03-12-2018 - START----*/ 
    function deleteAdmin($id = null){
        ...
        if(isset($id) && $id != ''){
            ...
            // CHECK ALREADY ANY USER CREATE OR NOT
            $userQ = "SELECT id FROM users WHERE admin_id = '".$id."'";
            ...
            // CHECK PURCHASE
            $purchaseQ = "SELECT id FROM purchase WHERE createdby = '".$id."'";
            $purchaseR = mysqli_query($conn, $purchaseQ);
            if ($purchaseR && mysqli_num_rows($purchaseR) > 0) {
                $is_delete = 1;
                $is_ledger[] = 'Purchase';
            ...
            // CHEQUE SELF CONSUMPTION
            $selfConsumptionQ = "SELECT id FROM self_consumption WHERE createdby = '".$id."'";
            $selfConsumptionR = mysqli_query($conn, $selfConsumptionQ);
            if($selfConsumptionR && mysqli_num_rows($selfConsumptionR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Self Consumption';
            }

            // CHECK ADJUSTMENT INVENTORY
            $inventoryAdjustmentQ = "SELECT id FROM adjustment WHERE created_by = '".$id."'";
            $inventoryAdjustmentR = mysqli_query($conn, $inventoryAdjustmentQ);
            if($inventoryAdjustmentR && mysqli_num_rows($inventoryAdjustmentR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Inventory Adjustment';
            }

            // CHECK SALE
            $taxbillQ = "SELECT id FROM tax_billing WHERE createdby = '".$id."'";
            $taxbillR = mysqli_query($conn, $taxbillQ);
            if($taxbillR && mysqli_num_rows($taxbillR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Sale Tax Bill';
            }

            // CHECK SALE ORDER
            $saleorderQ = "SELECT id FROM sales_order WHERE createdby = '".$id."'";
            $saleorderR = mysqli_query($conn, $saleorderQ);
            if($saleorderR && mysqli_num_rows($saleorderR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Sale Order';
            ...
            // CHECK SALE ESTIMATE
            $saleEstimateQ = "SELECT id FROM sales_estimate WHERE createdby = '".$id."'";
            $saleEstimateR = mysqli_query($conn, $saleEstimateQ);
            if($saleEstimateR && mysqli_num_rows($saleEstimateR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Sale Estimate';
            ...
            $purchaseOrderQ = "SELECT id FROM orders WHERE createdby = '".$id."'";
            $purchaseOrderR = mysqli_query($conn, $purchaseOrderQ);
            if($purchaseOrderR && mysqli_num_rows($purchaseOrderR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Purchase Order';
            }

            // CHECK CASH MANAGEMENT
            $cashManagementQ = "SELECT id FROM accounting_cash_management WHERE createdby = '".$id."'";
            $cashManagementR = mysqli_query($conn, $cashManagementQ);
            if($cashManagementR && mysqli_num_rows($cashManagementR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Cash Receipt Or Payment';
            }

             // CHECK CUSTOMER RECEIPT
            $customerReceiptQ = "SELECT id FROM cash_receipt WHERE createdby = '".$id."'";
            $customerReceiptR = mysqli_query($conn, $customerReceiptQ);
            if($customerReceiptR && mysqli_num_rows($customerReceiptR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Customer Receipt';
            }

            // CHECK CHEQUE
            $chequeQ = "SELECT id FROM accounting_cheque WHERE createdby = '".$id."'";
            $chequeR = mysqli_query($conn, $chequeQ);
            if($chequeR && mysqli_num_rows($chequeR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Cheque Receipt Or Payment';
            }

            // CHECK VENDOR PAYMENT
            $vendorPaymentQ = "SELECT id FROM accounting_vendor_payment WHERE created_by = '".$id."'";
            $vendorPaymentR = mysqli_query($conn, $vendorPaymentQ);
            if($vendorPaymentR && mysqli_num_rows($vendorPaymentR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Vendor Payment';
            }

            // CHECK JOURNAL VOUCHER
            $voucherQ = "SELECT id FROM journal_vouchar WHERE createdby = '".$id."'";
            $voucherR = mysqli_query($conn, $voucherQ);
            if($voucherR && mysqli_num_rows($voucherR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Journal Voucher';
            }
            
            // CHECK COURIER TRANSPORT
            $courierQ = "SELECT id FROM courier_transport WHERE createdby = '".$id."'";
            $courierR = mysqli_query($conn, $courierQ);
            if($courierR && mysqli_num_rows($courierR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Courier And Transport';
            }

            // CHECK PAYMENT VOUCHER FOR COURIER
            $paymentVoucherQ = "SELECT id FROM add_payment_voucher WHERE createdby = '".$id."'";
            $paymentVoucherR = mysqli_query($conn, $paymentVoucherQ);
            if($paymentVoucherR && mysqli_num_rows($paymentVoucherR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Courier Payment Voucher';
            }

            // CHECK COURIER PAYMENT
            $courierPaymentQ = "SELECT id FROM courier_payment WHERE createdby = '".$id."'";
            $courierPaymentR = mysqli_query($conn, $courierPaymentQ);
            if($courierPaymentR && mysqli_num_rows($courierPaymentR) > 0){
                $is_delete = 1;
                $is_ledger[] = 'Courier Payment';
            }

            if($is_delete == 0){
                $deleteAdminQ = "DELETE FROM users WHERE id = '".$id."'";
                $deleteAdminR = mysqli_query($conn, $deleteAdminQ);
                if($deleteAdminR){
                    $data['status'] = 1;
                    $data['message'] = 'Record Delete Succcessfully.';
                }
    ...
    /*---THIS FUNCTION IS USED TO DELETE ADMIN - GAUTAM MAKWANA - 03-12-2018 - END----*/
    ...
    function getcashpaymentno_cheque($type = null){
        ...
        if($type == "payment"){
             ...
             $voucherqry = "SELECT voucher_no FROM accounting_cheque WHERE voucher_type = 'payment' AND pharmacy_id = '".$pharmacy_id."' AND `financial_id` = '".$financial_id."' ORDER BY id DESC LIMIT 1";
             ...
             $voucherqry = "SELECT voucher_no FROM accounting_cheque WHERE voucher_type = 'receipt' AND pharmacy_id = '".$pharmacy_id."' AND `financial_id` = '".$financial_id."' ORDER BY id DESC LIMIT 1";
              $voucherrun = mysqli_query($conn, $voucherqry);
              if($voucherrun){
                $count = mysqli_num_rows($voucherrun);
                if($count !== '' && $count !== 0){
    ...
    function IpdPrescriptionData($today = null){
        global $ihis_conn;
        $data = ihis_check_pharmacy();
        $IpdPrescriptionData = array();
        $ihis_firm_id = (defined('ihis_firm_id')) ? ihis_firm_id : '';
        
        if($data['count'] == 1 || $data['pharmacy_type'] == 1){
            /*  IPD Prescription Start  */ 
            $ipdQ = "SELECT 'ihis' as bill_type, ipd.group_id, ipd.firm_id as ihis_firm_id, ipd.user_id as ihis_user_id, ipd.ipd_id as ihis_ipd_id, ipd.treatment_by as ihis_treatment_by, ipd.patient_id as ihis_patient_id, ipd.followup_id as ihis_followup_id, ipd.is_pharmacy_bill, ps.patient_id,ps.email,ps.mobile_no , ps.city_id,ps.patient_opd_id,CONCAT(u.fname,' ',u.lname) as doctor_name, u.id as doctor_id, u.mobile_no as doctor_mobile, CONCAT(ps.fname,' ',ps.lname) as p_name, ps.id as patient_primary_id,ipd.created_at as date FROM ipd_prescription_details ipd LEFT JOIN patient_master ps ON ps.id = ipd.patient_id LEFT JOIN users u ON u.id = ipd.user_id WHERE ipd.firm_id = '".$ihis_firm_id."'"; 
            if(isset($today) && $today == 1){
                $ipdQ .= " AND DATE_FORMAT(ipd.created_at,'%Y-%m-%d') = '".date('Y-m-d')."'";
                
            }
            $ipdQ .= " GROUP BY ipd.group_id";
        ...
        if($data['count'] == 1 || $data['pharmacy_type'] == 1){  
            ...
            /*  ICU Treatment Drugs Start  */
            $ICUQ = "SELECT 'ihis' as bill_type, icu_d.group_id, icu_d.firm_id as ihis_firm_id, icu_d.user_id as ihis_user_id, icu_d.patient_id as ihis_patient_id, icu_d.follow_id as ihis_followup_id,icu_d.is_pharmacy_bill, pm.patient_id, pm.mobile_no, pm.email, pm.city_id, pm.patient_opd_id, CONCAT(u.fname,' ',u.lname) as doctor_name, u.id as doctor_id, u.mobile_no as doctor_mobile, CONCAT(pm.fname,' ',pm.lname) as p_name, pm.id as patient_primary_id, icu_d.created_at as date FROM icu_treatment_drugs icu_d LEFT JOIN patient_master pm ON icu_d.patient_id = pm.id LEFT JOIN users u ON icu_d.user_id = u.id WHERE icu_d.firm_id = '".$ihis_firm_id."'";
            if(isset($today) && $today == 1){
                $ICUQ .= " AND DATE_FORMAT(icu_d.created_at,'%Y-%m-%d') = '".date('Y-m-d')."'";
            }
            $ICUQ .= " GROUP BY icu_d.group_id";
            $ICUR = mysqli_query($ihis_conn, $ICUQ);
            if($ICUR && mysqli_num_rows($ICUR) > 0){
                while($ICURow = mysqli_fetch_assoc($ICUR)){
                    $ICURow['type'] = 'ICU Drugs';
                    $IpdPrescriptionData[] = $ICURow;
                }
            }
            /*  ICU Treatment Drugs END  */
        }
    ...
    /*-----------------------------------eclinic Data  - 25-01-19 - KARTIK CHAMPANERIYA - START---------------------------*/
    function eClinicPrescriptionData($today = null){
        /*  OPD Prescription Start  */ 
            global $eclinic_conn;
            $ec_hsp_id = (defined('ec_hsp_id')) ? ec_hsp_id : '';
            $ecPrescriptionData = [];
            
            $opdQ = "SELECT 'eclinic' as bill_type, eopd.group_id, eopd.firm_id as ihis_firm_id, eopd.user_id as ihis_user_id, eopd.patient_id as ihis_patient_id, eopd.followup_id as ihis_followup_id, eopd.is_pharmacy_bill, em.patient_id, em.mobile_no, em.email,em.register_type, em.city_id, em.patient_opd_id, CONCAT(u.fname,' ',u.lname) as doctor_name, u.id as doctor_id, u.mobile_no as doctor_mobile, CONCAT(em.fname,' ',em.lname) as p_name, em.id as patient_primary_id, eopd.created_at as date, pf.register_type as followup_register_type, pf.infertility_register_type FROM ec_opd_prescription_details eopd LEFT JOIN ec_patient_master em ON eopd.patient_id = em.id LEFT JOIN ec_users u ON eopd.user_id = u.id LEFT JOIN ec_patient_followup pf ON eopd.followup_id = pf.id WHERE eopd.firm_id = '".$ec_hsp_id."'";
            if(isset($today) && $today == 1){
                $opdQ .= " AND DATE_FORMAT(eopd.created_at,'%Y-%m-%d') = '".date('Y-m-d')."'";
            }
            $opdQ .= " GROUP BY eopd.group_id";
            $opdR = mysqli_query($eclinic_conn, $opdQ);
            if($opdR && mysqli_num_rows($opdR) > 0){
                while($opdRow = mysqli_fetch_assoc($opdR)){
                    if($opdRow['ihis_followup_id'] == 0){
                        $opdRow['register_type'] = $opdRow['register_type'];
                    ...
                    $opdRow['type'] = 'OPD';
                    $ecPrescriptionData[] = $opdRow;
                }
            }
            /*  OPD Prescription END  */
            return $ecPrescriptionData;
    }
    
    /*-----------------------------------eclinic Data  - 25-01-19 - KARTIK CHAMPANERIYA - END---------------------------*/
    
    /*-----------------------------------IHIS Pharmcy Check  - 05-01-19 - KARTIK CHAMPANERIYA - START---------------------------*/
    function ihis_check_pharmacy(){
        global $conn;
        global $pharmacy_id;
        ...
        $data['pharmacy_type'] = '';
        if($ihis_firm_id != ''){
            $getihisIDQ = "SELECT `ihis_hsp_id`,`pharmacy_type` FROM `pharmacy_profile` WHERE id='".$pharmacy_id."'";
            $getihisIDR = mysqli_query($conn, $getihisIDQ);
            if($getihisIDR && mysqli_num_rows($getihisIDR) > 0){
                $getihisID_row = mysqli_fetch_assoc($getihisIDR);
                $getPharmacyQ = "SELECT * FROM `pharmacy_profile` WHERE ihis_hsp_id='".$getihisID_row['ihis_hsp_id']."'";
                $getPharmacyR = mysqli_query($conn, $getPharmacyQ);
                $data['count'] = mysqli_num_rows($getPharmacyR);
                $data['pharmacy_type'] = $getihisID_row['pharmacy_type'];
            }
            
        }
        return $data;
    }
    /*-----------------------------------IHIS Pharmcy Check  - 05-01-19 - KARTIK CHAMPANERIYA - END---------------------------*/
    
    
    function getIhisPrescriptionNotification(){
        global $ihis_conn;
        $ihisPharmacy = ihis_check_pharmacy();
        $ihis_firm_id = (defined('ihis_firm_id')) ? ihis_firm_id : '';
        $data = [];
        
        if($ihisPharmacy['count'] == 1 || $ihisPharmacy['pharmacy_type'] == 1){
            /*------------IPD PRESCRIPTION START----------------*/
            $ipdQuery = "SELECT 'IPD' as type, ipd.id, ipd.group_id, CONCAT(ps.fname,' ',ps.lname) as patient_name, CONCAT(u.fname,' ',u.lname) as doctor_name, ipd.created_at as date FROM ipd_prescription_details ipd LEFT JOIN patient_master ps ON ps.id = ipd.patient_id LEFT JOIN users u ON u.id = ipd.user_id WHERE ipd.firm_id = '".$ihis_firm_id."' AND ipd.is_notification = 0";
            $ipdRes = mysqli_query($ihis_conn, $ipdQuery);
            if($ipdRes && mysqli_num_rows($ipdRes) > 0){
                while($ipdRow = mysqli_fetch_assoc($ipdRes)){
                    $data[] = $ipdRow;
                }
            }
            /*------------IPD PRESCRIPTION END----------------*/
        }
        if($ihisPharmacy['count'] == 1 || $ihisPharmacy['pharmacy_type'] == 0){
            /*------------OPD PRESCRIPTION START----------------*/
            $opdQuery = "SELECT 'OPD' as type, opd.id, opd.group_id, CONCAT(ps.fname,' ',ps.lname) as patient_name, CONCAT(u.fname,' ',u.lname) as doctor_name, opd.created_at as date FROM opd_prescription_details opd LEFT JOIN patient_master ps ON opd.patient_id = ps.id LEFT JOIN users u ON opd.user_id = u.id WHERE opd.firm_id = '".$ihis_firm_id."' AND opd.is_notification = 0";
            $opdRes = mysqli_query($ihis_conn, $opdQuery);
            if($opdRes && mysqli_num_rows($opdRes) > 0){
                while($opdRow = mysqli_fetch_assoc($opdRes)){
                    $data[] = $opdRow;
                }
            }
            /*------------OPD PRESCRIPTION END----------------*/
        ...
        if($ihisPharmacy['count'] == 1 || $ihisPharmacy['pharmacy_type'] == 1){
            /*----------ICU PRESCRIPTION START----------------*/
            $icuQuery = "SELECT 'ICU' as type, itp.id, itp.group_id, CONCAT(ps.fname,' ',ps.lname) as patient_name, CONCAT(u.fname,' ',u.lname) as doctor_name, itp.created_at as date FROM icu_treatment_prescription itp LEFT JOIN patient_master ps ON itp.patient_id = ps.id LEFT JOIN users u ON itp.user_id = u.id WHERE itp.firm_id = '".$ihis_firm_id."' AND itp.is_notification = 0";
            $icuRes = mysqli_query($ihis_conn, $icuQuery);
            if($icuRes && mysqli_num_rows($icuRes)){
                while($icuRow = mysqli_fetch_assoc($icuRes)){
                    $data[] = $icuRow;
                }
            }
            /*----------ICU PRESCRIPTION START----------------*/
            ...
            /*----------OT PRESCRIPTION START---------------*/
            $otQuery = "SELECT 'OT' as type, otp.id, otp.group_id, CONCAT(ps.fname,' ',ps.lname) as patient_name, CONCAT(u.fname,' ',u.lname) as doctor_name, otp.created_at as date FROM ot_prescription otp LEFT JOIN patient_master ps ON otp.patient_id = ps.id LEFT JOIN users u ON otp.user_id = u.id WHERE otp.firm_id = '".$ihis_firm_id."' AND otp.is_notification = 0";
            $otRes = mysqli_query($ihis_conn, $otQuery);
            if($otRes && mysqli_num_rows($otRes) > 0){
                while($otRow = mysqli_fetch_assoc($otRes)){
                    $data[] = $otRow;
                }
            }
            /*----------OT PRESCRIPTION END----------------*/
        
            /*----------ICU DRUG PRESCRIPTION START----------------*/
            $icuDrugQuery = "SELECT 'ICUDRUG' as type, icu_d.id, icu_d.group_id, CONCAT(ps.fname,' ',ps.lname) as patient_name, CONCAT(u.fname,' ',u.lname) as doctor_name, icu_d.created_at as date FROM icu_treatment_drugs icu_d LEFT JOIN patient_master ps ON icu_d.patient_id = ps.id LEFT JOIN users u ON icu_d.user_id = u.id WHERE icu_d.firm_id = '".$ihis_firm_id."' AND icu_d.is_notification = 0";
            $icuDrugRes = mysqli_query($ihis_conn, $icuDrugQuery);
            if($icuDrugRes && mysqli_num_rows($icuDrugRes) > 0){
                while($icuDrugRow = mysqli_fetch_assoc($icuDrugRes)){
                    $data[] = $icuDrugRow;
                }
            }
            /*----------ICU DRUG PRESCRIPTION END----------------*/
        }
        return $data;
    }
    
    function getEclinicPrescriptionNotification(){
        global $eclinic_conn;
        $ec_hsp_id = (defined('ec_hsp_id')) ? ec_hsp_id : '';
        $data = [];
        
        /*------------OPD PRESCRIPTION START----------------*/
        $opdQuery = "SELECT 'OPD' as type, opd.id, opd.group_id, CONCAT(ps.fname,' ',ps.lname) as patient_name, CONCAT(u.fname,' ',u.lname) as doctor_name, opd.created_at as date FROM ec_opd_prescription_details opd LEFT JOIN ec_patient_master ps ON opd.patient_id = ps.id LEFT JOIN ec_users u ON opd.user_id = u.id WHERE opd.firm_id = '".$ec_hsp_id."' AND opd.is_notification = 0";
        $opdRes = mysqli_query($eclinic_conn, $opdQuery);
        if($opdRes && mysqli_num_rows($opdRes) > 0){
            while($opdRow = mysqli_fetch_assoc($opdRes)){
                $data[] = $opdRow;
            }
    ...
    function ihiscustomerLedger($customer_id = null, $from = null, $to = null, $type_by= null){
        ...
        if((isset($pharmacy_id) && $pharmacy_id != '')){
            ...
            /*----------------------------------------------------COUNT OPENING BALANCE START-------------------------------------------------------*/
            $opening_balance = 0;
            $customerQ =  "SELECT id, name, opening_balance, opening_balance_type FROM ledger_master WHERE pharmacy_id = '".$pharmacy_id."'";
            if(isset($type_by) && $type_by == 0){
                $customerQ .= " AND id IN (".$all_customer_id.")";
            }else{
                $customerQ .= " AND id = '".$customer_id."'";
            }
            $customerR = mysqli_query($conn, $customerQ);
            if($customerR && mysqli_num_rows($customerR) > 0){
                while($customerRow = mysqli_fetch_assoc($customerR)){
                    $opening_balance_tmp = (isset($customerRow['opening_balance']) && $customerRow['opening_balance'] != '') ? $customerRow['opening_balance'] : 0;
                    $opening_balance += (isset($customerRow['opening_balance_type']) && $customerRow['opening_balance_type'] == 'DB') ? $opening_balance_tmp : -$opening_balance_tmp;
                }
            }
            /*----------------------------------------------------COUNT OPENING BALANCE END-------------------------------------------------------*/

            /*----------------------------------------------------COUNT DABIT TAX BILL START-------------------------------------------------------*/
            $billQ =  "SELECT id, invoice_date, invoice_no, sale_amount FROM tax_billing WHERE pharmacy_id = '".$pharmacy_id."' AND financial_id = '".$financial_id."' AND bill_type = 'Debit'";
            if(isset($type_by) && $type_by == 0){
                $billQ .= " AND customer_id IN (".$all_customer_id.")";
            }else{
                $billQ .= " AND customer_id = '".$customer_id."'";
            }
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
              $billQ .= " AND invoice_date >= '".$from."' AND invoice_date <= '".$to."'";
            }
            ...
            if($billR && mysqli_num_rows($billR) > 0){
                while ($billRow = mysqli_fetch_assoc($billR)) {
                  ...
                  $billarr['date'] = (isset($billRow['invoice_date'])) ? $billRow['invoice_date'] : '';
                  $billarr['sr_no'] = (isset($billRow['invoice_no'])) ? $billRow['invoice_no'] : '';
                  $billarr['narration'] = (isset($billRow['invoice_no']) && $billRow['invoice_no'] != '') ? 'Tax Bill | '.$billRow['invoice_no'] : 'Tax Bill';
                  $billarr['debit'] = (isset($billRow['sale_amount']) && $billRow['sale_amount'] != '') ? $billRow['sale_amount'] : 0;
                  $billarr['credit'] = '';
                  $billarr['url'] = 'sales-tax-billing.php?id='.$billRow['id'];
                  $data[] = $billarr;
            ...
            /*----------------------------------------------------SALE RETURN START-----------------------------------------------------*/
            $saleReturnQ = "SELECT id, credit_note_no, credit_note_date, remarks, finalamount FROM sale_return WHERE pharmacy_id = '".$pharmacy_id."' AND financial_id = '".$financial_id."'";
            if(isset($type_by) && $type_by == 0){
                $saleReturnQ .= " AND customer_id IN (".$all_customer_id.")";
            }else{
                $saleReturnQ .= " AND customer_id = '".$customer_id."'";
            }
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
              $saleReturnQ .= " AND credit_note_date >= '".$from."' AND credit_note_date <= '".$to."'";
            }
            ...
            /*----------------------------------------------------CASH TRANSACTION START-----------------------------------------*/
            $cashTransactionQ = "SELECT id, payment_type, voucher_no, voucher_date, amount, narration FROM cash_transaction WHERE pharmacy_id = '".$pharmacy_id."' AND financial_id = '".$financial_id."'";
            if(isset($type_by) && $type_by == 0){
                $cashTransactionQ .= " AND perticular IN (".$all_customer_id.")";
            }else{
                $cashTransactionQ .= " AND perticular = '".$customer_id."'";
            }
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
              $cashTransactionQ .= " AND voucher_date >= '".$from."' AND voucher_date <= '".$to."'";
            }
            ...
            if(isset($type_by) && $type_by == 0){
                $bankTransactionQ .= " AND bt.perticular IN (".$all_customer_id.")";
            }else{
                $bankTransactionQ .= " AND bt.perticular = '".$customer_id."'";
            }
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
              $bankTransactionQ .= " AND bt.voucher_date >= '".$from."' AND bt.voucher_date <= '".$to."'";
            }
            ...
            /*-----------------------------------------------------COUNT JOURNAL VOUCHER START-----------------------------------------------*/
            $jurnalVoucherQ = "SELECT jv.id, jv.voucher_date, jvd.debit, jvd.credit FROM journal_vouchar_details jvd INNER JOIN journal_vouchar jv ON jvd.voucher_id = jv.id WHERE jv.pharmacy_id = '".$pharmacy_id."' AND jv.financial_id = '".$financial_id."'";
            if(isset($type_by) && $type_by == 0){
                $jurnalVoucherQ .= " AND jvd.particular IN (".$all_customer_id.")";
            }else{
                $jurnalVoucherQ .= " AND jvd.particular = '".$customer_id."'";
            }
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
              $jurnalVoucherQ .= " AND jv.voucher_date >= '".$from."' AND jv.voucher_date <= '".$to."'";
            }
    ...
    //   $closing_balance = $taccount['running_balance'];
      ...
      $closing_balance = (isset($taccount['closing_stock']) && $taccount['closing_stock'] != '') ? $taccount['closing_stock'] : 0;
                  ...
                  // purchase account
      $p_account = "SELECT  SUM(total_total) as total_amount FROM purchase WHERE pharmacy_id = '".$pharmacy_id."'"; 
      ...
      $s_account = "SELECT SUM(final_amount) as total_amount FROM  tax_billing WHERE pharmacy_id = '".$pharmacy_id."'" ;
...
return $profit_loss; 
    ...
    function balanceSheetReport($from = null, $to = null, $is_financial = 0){
        ...
        /*----------CAPITAL ACCOUNT START-------*/
            ...
            if($capitalAccR && mysqli_num_rows($capitalAccR) > 0){
                while ($capitalAccRow = mysqli_fetch_assoc($capitalAccR)) {
                    ...
                    $running1 = countRunningBalance($capitalAccRow['id'], $from, $to);
        ...
        /*----------LOAN LIABILITY START-------*/
            ...
            if($loanLiabilityR && mysqli_num_rows($loanLiabilityR) > 0){
                while ($loanLiabilityRow = mysqli_fetch_assoc($loanLiabilityR)) {
                    ...
                    $running2 = countRunningBalance($loanLiabilityRow['id'], $from, $to);
        ...
        /*----------CURRENT LIABILITY START-------*/
            ...
            $vendorData = getGroupWiseClosingBalance(14,$from,$to,0);
        ...
        /*----------SUNSPENSE ACCOUNT START-------*/
            ...
            if($sunspenseAccR && mysqli_num_rows($sunspenseAccR) > 0){
                while ($sunspenseAccRow = mysqli_fetch_assoc($sunspenseAccR)) {
                    ...
                    $running4 = countRunningBalance($sunspenseAccRow['id'], $from, $to);
        ...
        /*----------FIXED ASSETS START-------*/
            ...
            if($fixedAssetsR && mysqli_num_rows($fixedAssetsR) > 0){
                while ($fixedAssetsRow = mysqli_fetch_assoc($fixedAssetsR)) {
                    ...
                    $running5 = countRunningBalance($fixedAssetsRow['id'], $from, $to);
        ...
        /*----------INVESTMENT START-------*/
            ...
            if($investmentR && mysqli_num_rows($investmentR) > 0){
                while ($investmentRow = mysqli_fetch_assoc($investmentR)) {
                    ...
                    $running6 = countRunningBalance($investmentRow['id'], $from, $to);
        ...
        /*----------CURRENT ASSETS START-------*/
            ...
            /*$tmp7['id'] = '';
            $tmp7['name'] = 'Closing Stock';
            $allLedgerRunning = getTotalClosingBalance($from, $to);
            $tmp7['opening_balance'] = (isset($allLedgerRunning['opening_balance']) && $allLedgerRunning['opening_balance'] != '') ? $allLedgerRunning['opening_balance'] : 0;
            $tmp7['closing_balance'] = (isset($allLedgerRunning['running_balance']) && $allLedgerRunning['running_balance'] != '') ? $allLedgerRunning['running_balance'] : 0;
            $data['right']['currentAssets']['totalamount'] += $tmp7['closing_balance'];
            $data['right']['currentAssets']['data'][] = $tmp7;*/
            
            $tmp7['id'] = '';
            $totalClosingStock = getTotalClosingStock($from, $to);
            ...
            $totalcashinhand = countCashRunningBalance($from, $to);
            ...
            $totalbankaccount = allBankRunningBalance($from, $to,0);
...
//------------------getAllRate ------------start ------------shreya 29-12-2018---------------------
function getAllRate(){
        global $conn;
        global $pharmacy_id;
        $data = [];
