...
if(isset($_GET['from'])  && isset($_GET['to'])){
  ...
  // -------------------------------3.1(a)---------------------------------------//
  $outwardtotal = "SELECT SUM(tbd.totalamount) as taxable_value, SUM(((tbd.qty*tbd.rate)-(tbd.qty*tbd.rate)*(tbd.gst/(100+tbd.gst))) * tbd.gst/100) as TOTALGST FROM tax_billing_details tbd INNER JOIN tax_billing tb ON tb.id = tbd.tax_bill_id WHERE tb.pharmacy_id = '".$pharmacy_id."' AND tbd.gst != 0 AND tb.invoice_date >='".$first_day_this_month."' and tb.invoice_date <='".$last_day_this_month."'";
  $outwardR = mysqli_query($conn ,$outwardtotal);
  $outwardL = mysqli_fetch_assoc($outwardR);
 
  $Ccharge = "SELECT tb.id, st.state_code_gst as state_code FROM `tax_billing` tb INNER JOIN ledger_master lg ON tb.customer_id = lg.id LEFT JOIN own_states st ON lg.state = st.id WHERE tb.pharmacy_id='".$pharmacy_id."'AND tb.invoice_date >='".$first_day_this_month."' and tb.invoice_date <='".$last_day_this_month."'";
  $CchargeR = mysqli_query($conn ,$Ccharge);
  if($CchargeR && mysqli_num_rows($CchargeR) > 0){
      while($CchargeL = mysqli_fetch_assoc($CchargeR)){
             $outwardL['taxable_value'] = $outwardL['taxable_value'] - $outwardL['TOTALGST'];
          if($_SESSION['state_code'] == $CchargeL['state_code']){
              $gst = $outwardL['TOTALGST']/2;
              $outwardL['total_cgst'] = $gst;
              $outwardL['total_sgst'] = $gst;
          }else{
              $outwardL['total_igst'] = $outwardL['TOTALGST'];
          }
      }
  }

  $salereturn = "SELECT SUM(sr.finalamount) as taxable_amount, SUM(srd.gst_tax) as gst_tax ,IF(srd.igst != 0,SUM(srd.gst_tax),0) as total_Igst ,IF(srd.cgst != 0,SUM(srd.gst_tax/2),0) as total_Cgst , IF(srd.sgst != 0,SUM(srd.gst_tax/2),0) as total_Sgst FROM sale_return sr inner join sale_return_details srd on sr.id = srd.sale_return_id WHERE sr.pharmacy_id = '".$pharmacy_id."'  AND srd.gst != 0 and sr.credit_note_date >= '".$first_day_this_month."' and sr.credit_note_date <='".$last_day_this_month."'";
  $salereturnR = mysqli_query($conn ,$salereturn);
  if($salereturnR && mysqli_num_rows($salereturnR) > 0){
  while($salereturnRLa = mysqli_fetch_assoc($salereturnR)){
     
      $salereturnRL['taxable_amount'] = $salereturnRLa['taxable_amount'] - $salereturnRLa['gst_tax'];
      $salereturnRL['total_Igst'] = $salereturnRLa['total_Igst'];
      $salereturnRL['total_Cgst'] = $salereturnRLa['total_Cgst'];
      $salereturnRL['total_Sgst'] = $salereturnRLa['total_Sgst'];
  }
  }
 
// -------------------------------3.1(b-1)---------------------------------------// 

  $ZeroGST = "SELECT sum(tbd.totalamount) as amount ,(((tbd.qty*tbd.rate)-(tbd.qty*tbd.rate)*(tbd.gst/(100+tbd.gst))) * tbd.gst/100) as gst from tax_billing_details tbd inner join tax_billing tb WHERE tbd.tax_bill_id = tb.id and tbd.gst = '0' and tb.invoice_date >='".$first_day_this_month."' and tb.invoice_date <='".$last_day_this_month."' and tb.pharmacy_id ='".$pharmacy_id."'";
  $ZeroGSTR = mysqli_query($conn ,$ZeroGST);
   if($ZeroGSTR && mysqli_num_rows($ZeroGSTR) > 0){
       while($ZeroGSTLa = mysqli_fetch_assoc($ZeroGSTR)){
         $ZeroGSTL['amount'] = $ZeroGSTLa['amount'] - $ZeroGSTLa['gst'];
         $ZeroGSTL['gst'] = $ZeroGSTLa['gst'];
   }
   }
   $Zerosalereturn = "SELECT SUM(sr.finalamount) as taxable_amount, srd.gst_tax as gst FROM sale_return sr inner join sale_return_details srd  WHERE sr.id = srd.sale_return_id and srd.gst = '0' and sr.pharmacy_id = '".$pharmacy_id."' and sr.credit_note_date >= '".$first_day_this_month."' and sr.credit_note_date <='".$last_day_this_month."'";
   $ZerosalereturnR = mysqli_query($conn ,$Zerosalereturn);
   
    if($ZerosalereturnR && mysqli_num_rows($ZerosalereturnR) > 0){
     while($ZerosalereturnRLa = mysqli_fetch_assoc($ZerosalereturnR)){
         $ZerosalereturnRL['taxable_amount'] = $ZerosalereturnRLa['taxable_amount'] - $ZerosalereturnRLa['gst'];
         $ZerosalereturnRL['gst'] =  $ZerosalereturnRLa['gst'];
     }  
    }
...
//   $UnregistrL = mysqli_fetch_assoc($UnregistrR);
  ...
  //------------------------------------3.2(b)--------------------------------//
   $Composition = "SELECT sum(tbd.totalamount) as amount, SUM(((tbd.qty*tbd.rate)-(tbd.qty*tbd.rate)*(tbd.gst/(100+tbd.gst))) * tbd.gst/100) as GST ,SUM(((tbd.qty*tbd.rate)-(tbd.qty*tbd.rate)*(tbd.gst/(100+tbd.gst))) * tbd.gst/100) as totalIGST from tax_billing_details tbd inner join tax_billing tb on tbd.tax_bill_id = tb.id  inner join ledger_master lm on lm.id = tb.customer_id inner join own_states os on lm.state = os.id where os.state_code_gst != '".$_SESSION['state_code']."' AND tb.invoice_date >='".$first_day_this_month."' and tb.invoice_date <='".$last_day_this_month."' and lm.customer_type = 'GST_Composition' and tb.pharmacy_id ='".$pharmacy_id."'";
  $CompositionR = mysqli_query($conn ,$Composition);
//  $CompositionL = mysqli_fetch_assoc($CompositionR);
  if($CompositionR && mysqli_num_rows($CompositionR) > 0){
       while($CompositionLs = mysqli_fetch_assoc($CompositionR)){
           $CompositionL['amount'] = $CompositionLs['amount'] - $UnregistrLs['GST'];
           $CompositionL['totalIGST'] = $CompositionLs['totalIGST'];
       }
   } 
...
//  $holders = "SELECT sum(tbd.qty *tbd.rate) as amount, sum((tbd.igst *tbd.qty *tbd.rate)/100) as totalIGST from tax_billing_details tbd inner join tax_billing tb on tbd.tax_bill_id = tb.id  inner join ledger_master lm on lm.id = tb.customer_id inner join own_states os on lm.state = os.id where os.state_code_gst != '".$_SESSION['state_code']."' AND tb.invoice_date >='".$first_day_this_month."' and tb.invoice_date <='".$last_day_this_month."' and lm.customer_type = 'Unregistered' and tb.pharmacy_id ='".$pharmacy_id."'";
        ...
        <div class="content-wrapper">
          ...
          <div class="row">
            ...
            <div class="col-md-12 grid-margin stretch-card">
              ...
              <div class="card-body">
               <h5 class="card-title">3.2 3.2 Of the supplies shown in 3.1 (a) above, details of inter-State supplies made to unregistered persons,												
