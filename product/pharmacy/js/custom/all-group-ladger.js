$( document ).ready(function() {
    ...
    $("#ladger_list").change(function(){
      ...
      if(id !== ''){
          $.ajax({
            type: "POST",
            url: 'ajax.php',
            data: {'id':id,'from':from, 'to':to, 'group':group, 'action':'allLedgerCalculation'},
            dataType: "json",
  ...
  function getLedger(id){
                var id = id;
                var action = "getLedgersListByGroup";
                $.ajax({
                type: "POST",
                url: "ajax.php",
                data: {action:action,type:id},
                success: function(data){
                    $('#ladger_list').html(data);
                }
                });
            }
