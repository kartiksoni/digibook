      ...
      if($returnid != ''){
          ...
          if($count > 0){
            for ($i=0; $i < $count; $i++) { 
              ...
              $return_details['batchno'] = (isset($_POST['batch'][$i])) ? $_POST['batch'][$i] : '';
              ...
              $return_details['final_rate'] = (isset($_POST['final_rate'][$i]) && $_POST['final_rate'][$i] != '') ? $_POST['final_rate'][$i] : 0;
...
<?php 
  $vendorQ = "SELECT lg.id, lg.name, st.state_code_gst as statecode FROM ledger_master lg INNER JOIN purchase p ON lg.id = p.vendor LEFT JOIN own_states st ON lg.state = st.id WHERE lg.pharmacy_id = '".$pharmacy_id."' AND lg.group_id = 14 GROUP BY p.vendor";
  $vendorR = mysqli_query($conn, $vendorQ);
  if($vendorR && mysqli_num_rows($vendorR) > 0){
    $allvendor = [];
    while ($vendorRow = mysqli_fetch_assoc($vendorR)) {
      $allvendor[] = $vendorRow;
    }
  }
...
<head>
  ...
  <title>DigiBooks | <?php echo (isset($_GET['id']) && $_GET['id'] != '') ? 'Edit' : 'Add'; ?> Purchase Return</title>
  <!-- plugins:css -->
                      ...
                      <div class="col-12 col-md-2">
                       <label for="debit_note_date">Debit Note Date <span class="text-danger">*</span></label>
                            <div id="" class="input-group date datepicker">
                              <input type="text" class="form-control border debit_date" name="debit_note_date" data-parsley-errors-container="#error-dbnote" value="<?php echo (isset($editData['debit_note_date']) && $editData['debit_note_date'] != '' && $editData['debit_note_date'] != '0000-00-00') ? date('d/m/Y',strtotime($editData['debit_note_date'])) : date('d/m/Y'); ?>" required>
                      ...
                      <div class="col-12 col-md-3">
                        <label>Select Vendor <span class="text-danger">*</span></label>
                          <select class="js-example-basic-single vendor_id" style="width:100%" name="vendor_id" id="vendor_id" data-parsley-errors-container="#error-vendor" required> 
                              ...
                              <?php if(isset($allvendor) && !empty($allvendor)){ ?>
                                <?php foreach ($allvendor as $key => $value) { ?>
                                    <option data-state="<?php echo (isset($value['statecode'])) ? $value['statecode'] : ''; ?>" value="<?php echo (isset($value['id'])) ? $value['id'] : ''; ?>" <?php echo (isset($editData['vendor_id']) && $editData['vendor_id'] == $value['id']) ? 'selected' : ''; ?> ><?php echo (isset($value['name'])) ? $value['name'] : ''; ?></option>
                                <?php } ?>
                              <?php } ?>
                          </select>
                          ...
                          <tbody id="product-tbody">
                            <?php if(isset($editData['detail']) && !empty($editData['detail'])){ ?>.
                              <?php foreach ($editData['detail'] as $key => $value) { ?>
                                <tr class="product-tr">
                                  ...
                                  <td>
                                    <input name="mrp[]" type="text" class="form-control mrp onlynumber" value="<?php echo (isset($value['mrp'])) ? $value['mrp'] : ''; ?>" placeholder="MRP">
                                  </td>
                                  <td>
                                    <input name="mfg_co[]" type="text" class="form-control mfg_co" value="<?php echo (isset($value['mfg_co'])) ? $value['mfg_co'] : ''; ?>" placeholder="MFG. Co.">
                                  </td>
                                  <td>
                                    <input name="batch[]" type="text" class="form-control batch" value="<?php echo (isset($value['batchno'])) ? $value['batchno'] : ''; ?>" placeholder="Batch No.">
                                  </td>
                                  <td>
                                      <input name="expiry[]" type="text" class="form-control expiry datepicker-ex" value="<?php echo (isset($value['expiry']) && $value['expiry'] != '0000-00-00' && $value['expiry'] != '') ? date('d/m/Y',strtotime($value['expiry'])) : ''; ?>" placeholder="Expiry">
                                      <small class="text-danger expired"></small>
                                  </td>
                                  <td>
                                    <input name="qty[]" type="text" value="<?php echo (isset($value['qty'])) ? $value['qty'] : ''; ?>" class="form-control onlynumber qty" placeholder="Qty." required>
                                  </td>
                                  <td>
                                    <input name="free_qty[]" type="text" value="<?php echo (isset($value['free_qty'])) ? $value['free_qty'] : ''; ?>" class="form-control onlynumber free_qty" placeholder="Free Qty">
                                  </td>
                                  <td>
                                    <input name="rate[]" type="text" value="<?php echo (isset($value['rate'])) ? $value['rate'] : ''; ?>" class="form-control onlynumber rate" placeholder="Rate">
                                  </td>
                                  <td>
                                    <input name="discount[]" type="text" value="<?php echo (isset($value['discount'])) ? $value['discount'] : ''; ?>" class="form-control onlynumber discount" placeholder="Discount">
                                  </td>
                                  <td>
                                    <input type="text" name="final_rate[]" value="<?php echo (isset($value['final_rate'])) ? $value['final_rate'] : ''; ?>" class="form-control final_rate onlynumber priceOnly" placeholder="Rate" autocomplete="off" required>
                                  </td>
                                  <td>
                                      <!-- FOR GST START -->
                                      <input type="hidden" name="igst[]" value="<?php echo (isset($value['igst'])) ? $value['igst'] : ''; ?>" class="igst">
                                      <input type="hidden" name="cgst[]" value="<?php echo (isset($value['cgst'])) ? $value['cgst'] : ''; ?>" class="cgst">
                                      <input type="hidden" name="sgst[]" value="<?php echo (isset($value['sgst'])) ? $value['sgst'] : ''; ?>" class="sgst">
                                      <!-- FOR GST END -->

                                      <input name="amount[]" value="<?php echo (isset($value['amount'])) ? $value['amount'] : ''; ?>" type="text" class="form-control onlynumber amount" placeholder="Ammount" readonly>
                                  </td>
                                  <td>
                            ...
                            <?php }else{ ?>
                              <tr class="product-tr">
                                ...
                                <td><input name="batch[]" type="text" class="form-control batch" placeholder="Batch No."></td>
                                <td>
                                    <input name="expiry[]" type="text" class="form-control expiry datepicker-ex" placeholder="Expiry">
                                    <small class="text-danger expired"></small>
                                </td>
                                ...
                                <td><input type="text" name=final_rate[] class="form-control final_rate onlynumber priceOnly" placeholder="Rate" autocomplete="off" required></td>
                                <td>
                                    <!-- FOR GST START -->
                                    ...
                                    <input name="amount[]" type="text" class="form-control onlynumber amount" placeholder="Ammount" readonly>
                                </td>
                                <td>
                                  <a href="javascript:;" class="btn btn-primary btn-xs pt-2 pb-2 btn-addmore-product"><i class="fa fa-plus mr-0 ml-0"></i></a>
                                </td>
                            ...
                            <div class="col-12 col-md-12">
                                ...
                                <div class="row no-gutters">
                                  ...
                                  <div class="col">
                                      ...
                                      <input type="radio" class="form-check-input" name="debit_note_settle" id="optionsRadios1" value="1" <?php echo (isset($editData['debit_note_settle']) && $editData['debit_note_settle'] == 1) ? 'checked' : ''; ?> >
                                     ON HOLD
                                      </label>
                                      </div>
                                  </div>
                                  ...
                                  <div class="col">
                                      <div class="form-radio">
                                      <label class="form-check-label">
                                      <input type="radio" class="form-check-input" name="debit_note_settle" id="optionsRadios2" value="0" <?php echo (isset($editData['debit_note_settle']) && $editData['debit_note_settle'] == 0) ? 'checked' : ''; ?> <?php echo (!isset($_GET['id'])) ? 'checked' : ''; ?>>
