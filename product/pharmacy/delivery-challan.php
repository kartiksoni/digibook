...
<?php
  ...
  if(isset($_POST['save']) || isset($_POST['saveAndNext']) || isset($_POST['saveAndPrint'])){
    ...
    $tax_billing['customer_name'] = (isset($_POST['customer_name'])) ? $_POST['customer_name'] : NULL;
    ...
    $tax_billing['customer_id'] = $_POST['customer_id'];
    if($tax_billing['customer_id'] == ''){
        $_SESSION['msg']['fail'] = "Delivery Challan Added Fail! Customer Not Found!";
    ...
    if(isset($_GET['id']) && $_GET['id'] != '' ){
        ...
        $tax_billing['invoice_no'] = getChallanInvoiceNo((isset($_POST['bill_type'])) ? $_POST['bill_type'] : '');
    }
    ...
    $tax_billing['totaltaxgst'] = (isset($_POST['overalldiscount']) && $_POST['overalldiscount'] != '') ? $_POST['overalldiscount'] : 0;
    ...
    $tax_billing['total_amount'] = (isset($_POST['purchase_amount']) && $_POST['purchase_amount'] != '') ? $_POST['purchase_amount'] : 0;
    ...
    if($taxbillingRes){
      ...
      if($taxbillid != ''){
        ...
        $deleteOldItemQ = "DELETE FROM delivery_challan_details WHERE challan_id = '".$taxbillid."'";
        ...
        if($count != 0){
          for ($i=0; $i < $count; $i++) { 
            $tax_billing_details['challan_id'] = $taxbillid;
            ...
            $tax_billing_details['amount'] = (isset($_POST['totalamount'][$i]) && $_POST['totalamount'][$i] != '') ? $_POST['totalamount'][$i] : 0;
            $tax_billing_details['ptr'] = (isset($_POST['ptr'][$i]) && $_POST['ptr'][$i] != '') ? $_POST['ptr'][$i] : 0;
            $tax_billing_details['gst_per'] = (isset($_POST['gst'][$i]) && $_POST['gst'][$i] != '') ? $_POST['gst'][$i] : 0;
      ...
      if(isset($_POST['save'])){
          ...
          header('Location: delivery_challan.php');exit;
...
<?php 
    if(isset($_GET['id']) && $_GET['id'] != ''){
      $editQ = "SELECT dc.*, lg.customer_id as customer_u_id,lg.name as customer_name,lg.mobile as customer_mobile, lg.email as customer_email, lg.city as customer_city  FROM delivery_challan dc LEFT JOIN ledger_master lg ON dc.customer_id = lg.id WHERE dc.id = '".$_GET['id']."' AND dc.pharmacy_id = '".$pharmacy_id."' AND dc.financial_id = '".$financial_id."'";
      $editR = mysqli_query($conn, $editQ);
      if($editR && mysqli_num_rows($editR) > 0){
        $editdata = mysqli_fetch_assoc($editR);
        
        $billing_item_Q = "SELECT dcd.*, pm.product_name FROM delivery_challan_details dcd LEFT JOIN product_master pm ON dcd.product_id = pm.id WHERE dcd.challan_id = '".$editdata['id']."'";
...
<head>
  ...
  <title>DigiBooks | <?php echo (isset($_GET['id']) && $_GET['id'] != '') ? 'Edit' : 'Add'; ?> Delivery Challan</title>
  <!-- plugins:css -->
                      ...
                      <div class="row">
                              ...
                              <?php if(isset($editdata['detail']) && !empty($editdata['detail'])){ ?>
                                <?php foreach($editdata['detail'] as $key => $value){?>
                                  <tr>
                                    ...
                                    ?>
                                    <td>
                                      <input type="text" name="gst[]" class="form-control gst onlynumber" placeholder="GST(%)" value="<?php echo (isset($value['gst_per'])) ? $value['gst_per'] : ''; ?>">
                                    ...
                                    <?php } ?>
                                    <td><input type="text" name="totalamount[]" class="form-control totalamount onlynumber" placeholder="0.00" value="<?php echo (isset($value['amount'])) ? $value['amount'] : ''; ?>" readonly></td>
                                    <td>
                        ...
                        <?php 
                          /*if(isset($editdata['id']) && $editdata['id'] != ''){
                            $billing_returnitem_Q = "SELECT sl.*, pm.id as p_id, pm.product_name as p_name, pm.mfg_company as p_mfg_company, pm.batch_no as p_batch, pm.ex_date as p_expiry FROM sale_return sl LEFT JOIN product_master pm ON sl.product_id = pm.id WHERE sl.tax_bill_id = '".$editdata['id']."'";
                            $billing_returnitem_R = mysqli_query($conn, $billing_returnitem_Q);
                          } 
                        ?>
                        <div class="col-12">
                          <table class="table" id="return-table" style="border: solid 1px #e80a0a;<?php echo (isset($billing_returnitem_R) && $billing_returnitem_R && mysqli_num_rows($billing_returnitem_R) > 0) ? 'display: block;' : 'display: none'; ?>;">
                            <thead>
                              <tr>
                                  <th>Sr No</th>
                                  <th>Product</th>
                            ...
                            <tbody id="item-return-tbody">
                              <?php 
                                if(isset($billing_returnitem_R) && $billing_returnitem_R && mysqli_num_rows($billing_returnitem_R) > 0){
                                  $returnitemcount = 1;
                                  while ($returnRow = mysqli_fetch_array($billing_returnitem_R)) {
                              ?>
                                <tr>
                                  <td><?php echo $returnitemcount; ?></td>
                                  <td>
                                    <input type="text" name="r_product[]" value="<?php echo (isset($returnRow['p_name'])) ? $returnRow['p_name'] : ''; ?>" class="form-control r_product" placeholder="Product" readonly>
                                    <small class="producterror text-danger"></small>
                                    <input type="hidden" class="r_product_id" name="r_product_id[]" value="<?php echo (isset($returnRow['p_id'])) ? $returnRow['p_id'] : ''; ?>">
                                  </td>
                                  <td>
                                    <input type="text" name="r_mfg_co[]" value="<?php echo (isset($returnRow['p_mfg_company'])) ? $returnRow['p_mfg_company'] : ''; ?>" class="form-control r_mfg_co" placeholder="MFG. Co." readonly>
                                  </td>
                                  <td>
                                    <input type="text" name="r_batch[]" value="<?php echo (isset($returnRow['p_batch'])) ? $returnRow['p_batch'] : ''; ?>" class="form-control r_batch" placeholder="Batch" readonly>
                                  </td>
                                  <td>
                                    <input type="text" name="r_expiry[]" value="<?php echo (isset($returnRow['p_expiry']) && $returnRow['p_expiry'] != '') ? date('d/m/Y',strtotime($returnRow['p_expiry'])) : ''; ?>" class="form-control r_expiry" placeholder="Expiry" readonly>
                                  </td>
                                  <td>
                                    <input type="text" name="r_qty[]" value="<?php echo (isset($returnRow['qty'])) ? $returnRow['qty'] : ''; ?>" class="form-control r_qty onlynumber" placeholder="Qty." readonly>
                                  </td>
                                  <td>
                                    <input type="text" name="r_rate[]" value="<?php echo (isset($returnRow['rate'])) ? $returnRow['rate'] : ''; ?>" class="form-control r_rate onlynumber" placeholder="Rate" readonly>
                                  </td>
                                  <td>
                                    <input type="text" name="r_discount[]" value="<?php echo (isset($returnRow['discount'])) ? $returnRow['discount'] : ''; ?>" class="form-control r_discount onlynumber" placeholder="Discount(RS)" readonly>
                                  </td>
                                  <td>
                                    <input type="text" name="r_gst[]" value="<?php echo (isset($returnRow['gst'])) ? $returnRow['gst'] : ''; ?>" class="form-control r_gst onlynumber" placeholder="GST(%)" readonly>
                                    <input type="hidden" name="r_igst[]" value="<?php echo (isset($returnRow['igst'])) ? $returnRow['igst'] : ''; ?>" class="r_igst" value="0">
                                    <input type="hidden" name="r_cgst[]" value="<?php echo (isset($returnRow['cgst'])) ? $returnRow['cgst'] : ''; ?>" class="r_cgst" value="0">
                                    <input type="hidden" name="r_sgst[]" value="<?php echo (isset($returnRow['sgst'])) ? $returnRow['sgst'] : ''; ?>" class="r_sgst" value="0">
                                  </td>
                                  <td><input type="text" name="r_amount[]" value="<?php echo (isset($returnRow['amount'])) ? $returnRow['amount'] : ''; ?>" class="form-control r_amount onlynumber" placeholder="0.00" readonly></td>
                                  <td>
                                    <button type="button" class="btn btn-danger btn-xs pt-2 pb-2 btn-remove-item" style=""><i class="fa fa-close mr-0 ml-0"></i></button>
                                  </td>
                                </tr>
                          ...
                          <div class="form-group row">
                            <table class="table table-striped" width="100%">
                              <tbody>
                                ...
                                <tr>
                                  ...
                                  <td align="right">
                                    <input type="text" name="total_return_amount" id="total_return_amount" class="form-control onlynumber" placeholder="Total Return Amount" value="<?php echo (isset($editdata['total_return_amount'])) ? $editdata['total_return_amount'] : ''; ?>" readonly>
                                  </td>
                                </tr>
                                <tr>
                            ...
                            ?>
                                ...
                                <tr style="background:#ececec;">
                                  ...
                                  <td align="right">
                                    <input type="text" name="purchase_amount" id="purchase_amount" class="form-control onlynumber" placeholder="0.00" value="<?php echo (isset($editdata['total_amount'])) ? number_format($editdata['total_amount'], 2, '.', '') : ''; ?>" readonly>
                                  </td>
                                </tr>
