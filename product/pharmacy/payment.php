...
<?php 
  ...
  if(isset($_POST['add'])){
    ...
    if($count > 0){
      $cashVoucherNo = getCashTransactionVoucherNo('payment');
      $bankVoucherNo = getBankTransactionVoucherNo('payment');
      ...
      for($i=0; $i < $count; $i++) {
        ...
        if($bank_id == 'cash'){//cash payment
          $query = "INSERT INTO cash_transaction SET financial_id = '".$financial_id."', owner_id = '".$owner_id."', admin_id = '".$admin_id."', pharmacy_id = '".$pharmacy_id."', payment_type = 'payment', voucher_no = '".$cashVoucherNo."', voucher_date = '".$voucher_date."', type = '".$type."', perticular = '".$perticular."', amount = '".$amount."', narration = '".$narration."', reverse_charge = '".$reverse_charge."', gst = '".$gst."', batch = '".$batch."', created = '".date('Y-m-d H:i:s')."', createdby = '".$_SESSION['auth']['id']."'";
        }else{//bank payment
           $query = "INSERT INTO bank_transaction SET financial_id = '".$financial_id."', owner_id = '".$owner_id."', admin_id = '".$admin_id."', pharmacy_id = '".$pharmacy_id."', payment_type = 'payment', voucher_no = '".$bankVoucherNo."', voucher_date = '".$voucher_date."', bank_id = '".$bank_id."', type = '".$type."', perticular = '".$perticular."', amount = '".$amount."', payment_mode = '".$payment_mode."', payment_mode_date = '".$payment_mode_date."', payment_mode_no = '".$payment_mode_no."', card_name = '".$card_name."', other_reference = '".$other_reference."', reverse_charge = '".$reverse_charge."', gst = '".$gst."', batch = '".$batch."', created = '".date('Y-m-d H:i:s')."', createdby = '".$_SESSION['auth']['id']."'";
        }
        $res = mysqli_query($conn, $query);
      }
      $_SESSION['msg']['success'] = "Payment Added Successfully.";
      header('Location: view-payment.php');exit;
    }else{
      $_SESSION['msg']['fail'] = "At least one payment is required!";
  ...
  if(isset($_POST['update'])){
    ...
    $res = mysqli_query($conn, $query);
    if($res){
      $_SESSION['msg']['success'] = "Payment Update Successfully.";
      header('Location: view-payment.php');exit;
    }else{
      $_SESSION['msg']['fail'] = "Payment Update Fail!";
      header('Location: payment.php');exit;
    }
  }
  /*------------------UPDATE PAYMENT START-----------------------*/
  ...
  if(isset($_GET['id']) && $_GET['id'] != ''){
    if(isset($_GET['type']) && $_GET['type'] == 'cash'){
      $query = "SELECT *, 'cash' as bank_id, 'cash_payment' as record_type  FROM cash_transaction WHERE pharmacy_id = '".$pharmacy_id."' AND financial_id = '".$financial_id."' AND id = '".$_GET['id']."'";
      $res = mysqli_query($conn, $query);
      ...
      $query = "SELECT *, 'bank_payment' as record_type FROM bank_transaction WHERE pharmacy_id = '".$pharmacy_id."' AND financial_id = '".$financial_id."' AND id = '".$_GET['id']."'";
      $res = mysqli_query($conn, $query);
      if($res && mysqli_num_rows($res) > 0){
        $editData = mysqli_fetch_assoc($res);
      }else{
                  ...
                  <div class="card-body">
                    <div class="row form-group">
                      <div class="col-md-2">
                        ...
                        <input type="text" class="form-control" id="voucher_no" name="voucher_no" placeholder="Voucher No." value="<?php echo (isset($editData['voucher_no'])) ? $editData['voucher_no'] : getCashTransactionVoucherNo('payment'); ?>" required readonly>
                      </div>
                      <div class="col-md-2">
                ...
                <div class="card">
                  <div class="card-body">
                    ...
                    <div id="detail-footer">
                      <a href="view-payment.php" class="btn btn-light">Back</a>
