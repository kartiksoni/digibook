...
<?php 
  ...
  if(isset($_POST['submit'])){
    ...
    $setting_group = $_POST['sell_setting'];
    ...
    //SELL SETTING UPDATE QUERY START
    $ins_qry = "UPDATE setting_group SET sell_setting = '".$setting_group."', modified = '".date('Y-m-d H:i:s')."', modifiedby = '".$user_id."' WHERE pharmacy_id = '".$pharmacy_id."'";
    $ins_run = mysqli_query($conn, $ins_qry);

    if($ins_run){
      $_SESSION['msg']['success'] = 'Sell Setting Updated Successfully.';
      header('location:sell-setting.php');exit;
    }else{
      $_SESSION['msg']['fail'] = 'Sell Setting Updated Failed.';
      header('location:sell-setting.php');exit;
    }
    //SELL SETTING UPDATE QUERY END
  }
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <title>DigiBooks | Sell Setting</title>
  <!-- plugins:css -->
...
<body>
  ...
  <div class="card">
    <div class="card-body">
      <h4 class="card-title">Sell Setting</h4>
      <form class="forms-sample" method="post" action="" autocomplete="off">
        <div class="form-group row">     
          <div class="col-12 col-md-6">
            <label for="exampleInputName1">Prefix Setting</label>&nbsp;&nbsp;&nbsp;
              <div class="row no-gutters">
                <div class="col">
                  <div class="form-radio">
                    <label class="form-check-label">
                    <input type="radio" class="form-check-input" name="sell_setting" id="optionsRadios1" value="0" checked <?php if($data['sell_setting'] == 0 && $data != ''){ echo "checked"; } ?>>Yes</label>
                  </div>
                </div>
                ...
                <div class="col">
                  <div class="form-radio">
                    <label class="form-check-label">
                    <input type="radio" class="form-check-input" name="sell_setting" id="optionsRadios1" value="1" <?php if($data['sell_setting'] == 1 && $data != ''){ echo "checked"; } ?>>No</label>
                  </div>
                </div>
              </div>
          </div> 
