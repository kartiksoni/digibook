...
<?php 
	...
	$data = customerLedger($id, $from, $to);
?>
<!DOCTYPE html>
<html>
<head>
  			...
  			<tr>
    			<td style="border:none;padding:10px;color:#CC0000;font-size:16px;text-align:center;line-height:20px;">
    				<span style="font-size:18px">Party 	Name : <b> <?php echo (isset($customer['name'])) ? $customer['name'] : 'Unknown Name'; ?> </b></span> <br>
      				<span style="font-size:15px;line-height:24px;font-weight:bold;">City: <?php echo (isset($customer['city'])) ? $customer['city'] : 'Unknown City'; ?>
      				<?php if(isset($customer['gstno']) && $customer['gstno'] != ''){ ?>
      				    <br>GST No: <?php echo $customer['gstno']; ?> </span>
      				<?php } ?>
      			</td>
  			</tr>
		</tbody>
