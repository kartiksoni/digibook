...
<?php 
  ...
  if(isset($_POST['add'])){
    ...
    if($count > 0){
      $receiptVoucherNo = getCashTransactionVoucherNo('receipt');
      $paymentVoucherNo = getCashTransactionVoucherNo('payment');
      ...
      for($i=0; $i < $count; $i++) {
        ...
        $query = "INSERT INTO cash_transaction SET financial_id = '".$financial_id."', owner_id = '".$owner_id."', admin_id = '".$admin_id."', pharmacy_id = '".$pharmacy_id."', payment_type = '".$payment_type."', voucher_no = '".$voucher_no."', voucher_date = '".$voucher_date."', type = '".$type."', perticular = '".$perticular."', amount = '".$amount."', narration = '".$narration."', reverse_charge = '".$reverse_charge."', gst = '".$gst."', batch = '".$batch."', created = '".date('Y-m-d H:i:s')."', createdby = '".$_SESSION['auth']['id']."'";
        $res = mysqli_query($conn, $query);
      }

      $_SESSION['msg']['success'] = "Cash Transaction Added Successfully.";
      header('Location: view-cash-transaction.php');exit;
    }else{
      $_SESSION['msg']['fail'] = "At least one transaction is required!";
      header('Location: cash-transaction.php');exit;
    }
  }
  /*---------------ADD TRANSACTION END--------------------*/
  ...
  if(isset($_POST['update'])){
    ...
    $gst = (isset($_POST['gst']) && $reverse_charge == 1) ? $_POST['gst'][$i] : 0;
    $payment_type = ($debit > 0) ? 'payment' : 'receipt';
    $amount = ($debit > 0) ? $debit : $credit;

    $query = "UPDATE cash_transaction SET payment_type = '".$payment_type."', voucher_date = '".$voucher_date."', type = '".$type."', perticular = '".$perticular."', amount = '".$amount."', narration = '".$narration."', reverse_charge = '".$reverse_charge."', gst = '".$gst."', modified = '".date('Y-m-d H:i:s')."', modifiedby = '".$_SESSION['auth']['id']."' WHERE id = '".$_GET['id']."'";
    $res = mysqli_query($conn, $query);
    if($res){
      $_SESSION['msg']['success'] = "Cash Transaction Update Successfully.";
      header('Location: view-cash-transaction.php');exit;
    }else{
      $_SESSION['msg']['fail'] = "Cash Transaction Update Fail! Try Again.";
      header('Location: cash-transaction.php');exit;
    }
  }
  /*---------------UPDATE TRANSACTION END---------------*/

  /*-----------GET EDIT TIME DATA START---------------------*/
  if(isset($_GET['id']) && $_GET['id'] != ''){
    $query = "SELECT * FROM cash_transaction WHERE id = '".$_GET['id']."' AND pharmacy_id = '".$pharmacy_id."'";
    $res = mysqli_query($conn, $query);
...
<head>
  ...
  <title>DigiBooks | Cash Transaction</title>
  <!-- plugins:css -->
...
<body>
                ...
                <div class="card">
                  <div class="card-body">
                      ...
                      <div class="detail-row">
                        <?php if(isset($editData) && !empty($editData)){ ?>
                          <div class="row form-group">
                            ...
                            <div class="col-md-2">
                              ...
                              <textarea class="form-control" name="narration" placeholder="Narration" rows="1" style="resize: both;"><?php echo (isset($editData['narration'])) ? $editData['narration'] : ''; ?></textarea>
                            </div>
                          </div>
                          <div class="row form-group">
                            <div class="col-md-3">
                        ...
                        <?php }else{ ?>
                          <div class="row form-group">
                            ...
                            <div class="col-md-2">
                              <label for="narration">Narration</label>
                              <textarea class="form-control" name="narration[]" placeholder="Narration" rows="1" style="resize: both;"></textarea>
                            </div>
                          </div>
                          <div class="row form-group">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3 offset-md-3 text-right" style="padding-top: 25px;">
                    ...
                    <div id="detail-footer">
                      <a href="view-cash-transaction.php" class="btn btn-light">Back</a>
  ...
  <!-- page js -->
  <script src="js/custom/cash-transaction.js"></script>
