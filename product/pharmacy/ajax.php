...
if($_REQUEST['action'] == "customerrunning"){
    ...
    if($_REQUEST['action'] == "addvendor"){
      ...
      if(isset($_REQUEST['data']) && !empty($_REQUEST['data'])){
        ...
        $result = mysqli_query($conn, $query);
        if($result){
          $result = array('status' => true, 'message' => 'Vendor Added Success.', 'result' => mysqli_insert_id($conn));
        }else{
    ...
    if($_REQUEST['action'] == "addproduct"){
      if(isset($_REQUEST['data']) && !empty($_REQUEST['data'])){
        ...
        if(!empty($data)){
          ...
          $res['ratio'] = (isset($data['ratio']) && $data['ratio'] != '' && $data['ratio'] != 0) ? $data['ratio'] : 1;
          $res['status'] = 1;
    ...
    if($_REQUEST['action'] == "getVoucherNoByType"){
      $bill_type = (isset($_REQUEST['purchase_type'])) ? $_REQUEST['purchase_type'] : '';
      $voucher_no = getpurchaseinvoiceno($bill_type);
    ...
    // added by gautam makwana 08-01-19
    if($_REQUEST['action'] == "getLastCreditNoteNo"){
      $no = getCreditNoteNo();
      if($no){
        $result = array('status' => true, 'message' => 'found!', 'result' => $no);
      }else{
    ...
    if($_REQUEST['action'] == "addcompany"){
      ...
      if($name != ''){
        ...
        if($res){
          ...
          $result = array('status' => true, 'message' => 'Company Added successfully.', 'result' => $last_id);
        }else{
          $result = array('status' => false, 'message' => 'Company Added fail! Try again.', 'result' => '');  
        }
      }else{
    ...
    //Date:- 09/01/2019 CREATED BY VIRAG RAKHOLIYA
    if($_REQUEST['action'] == "addproducttype"){
      $financial_id = (isset($_SESSION['auth']['financial'])) ? $_SESSION['auth']['financial'] : '';
      $user_id = (isset($_SESSION['auth']['id']) && $_SESSION['auth']['id'] != '') ? $_SESSION['auth']['id'] : NULL ;
      $name = (isset($_REQUEST['data']['name'])) ? $_REQUEST['data']['name'] : '';
      if($name != ''){
        $query = "INSERT INTO `product_type`(`owner_id`, `admin_id`, `pharmacy_id`, `user_id`, `finance_year_id`, `product_type`, `status`, `created_at`, `created_by`) VALUES ('".$owner_id."', '".$admin_id."', '".$pharmacy_id."', '".$user_id."', '".$financial_id."', '".$name."', '1', '".date('Y-m-d H:i:s')."', '".$_SESSION['auth']['id']."')";
        $res = mysqli_query($conn, $query);
        if($res){
          $last_id = mysqli_insert_id($conn);
          $result = array('status' => true, 'message' => 'Product Type Added successfully.', 'result' => $last_id);
        }else{
          $result = array('status' => false, 'message' => 'Product Type Added fail! Try again.', 'result' => '');  
        }
      }else{
        $result = array('status' => false, 'message' => 'All fields is required!', 'result' => '');
      }
    ...
    //Date:- 09/01/2019 CREATED BY VIRAG RAKHOLIYA
    if($_REQUEST['action'] == "addproductcategory"){
      $financial_id = (isset($_SESSION['auth']['financial'])) ? $_SESSION['auth']['financial'] : '';
      $user_id = (isset($_SESSION['auth']['id']) && $_SESSION['auth']['id'] != '') ? $_SESSION['auth']['id'] : NULL ;
      $name = (isset($_REQUEST['data']['name'])) ? $_REQUEST['data']['name'] : '';
      if($name != ''){
        $query = "INSERT INTO `product_category`(`owner_id`, `admin_id`, `pharmacy_id`, `user_id`, `finance_year_id`, `product_cat`, `status`, `created_at`, `created_by`) VALUES ('".$owner_id."', '".$admin_id."', '".$pharmacy_id."', '".$user_id."', '".$financial_id."', '".$name."', '1', '".date('Y-m-d H:i:s')."', '".$_SESSION['auth']['id']."')";
        $res = mysqli_query($conn, $query);
        if($res){
          $last_id = mysqli_insert_id($conn);
          $result = array('status' => true, 'message' => 'Product Category Added successfully.', 'result' => $last_id);
        }else{
          $result = array('status' => false, 'message' => 'Product Category Added fail! Try again.', 'result' => '');  
        }
      }else{
        $result = array('status' => false, 'message' => 'All fields is required!', 'result' => '');
      }
    ...
    if($_REQUEST['action'] == "addcustomer"){
      if(isset($_REQUEST['data']) && !empty($_REQUEST['data'])){
        ...
        if($result){
          ...
          $lastQ = "SELECT l.id, l.name, l.mobile, l.email, l.city, st.state_code_gst as statecode FROM ledger_master l LEFT JOIN own_states st ON l.state = st.id WHERE l.id = '".$lastid."'";
    ...
    //THIS AJAX IS USED TO GET INVOICE NUMBER FOR DELIVERY CHALLAN
    if($_REQUEST['action'] == "getInvoiceNoforchallan"){
      $bill_type = (isset($_REQUEST['bill_type'])) ? $_REQUEST['bill_type'] : '';
    ...
    if($_REQUEST['action'] == "getInvoiceNoForQuatation"){
      ...
      if($bill_type == 'Cash'){
        ...
        $query = "SELECT invoice_no FROM quotation WHERE bill_type = 'Debit' AND pharmacy_id = '".$pharmacy_id."' AND financial_id = '".$financial_id."' ORDER BY id DESC LIMIT 1";
        $res = mysqli_query($conn, $query);
        if($res){
          $count = mysqli_num_rows($res);
            if($count !== '' && $count !== 0){
        ...
        if(!empty($data)){
          ...
          if($count > 0){
            for ($i=0; $i < $count; $i++) {
              ...
              $admin_id = (isset($admin_id) && $admin_id != '') ? $admin_id : NULL;
              $pharmacy_id = (isset($pharmacy_id) && $pharmacy_id != '') ? $pharmacy_id : NULL;
              $product_id = (isset($data['product'][$i])) ? $data['product'][$i] : NULL;
              //$qty = (isset($data['qty'][$i])) ? $data['qty'][$i] : 0;
              //$unit = (isset($data['unit'][$i])) ? $data['unit'][$i] : 0;
              $created = date('Y-m-d H:i:s');
              $createdby = $_SESSION['auth']['id'];

              $query = "INSERT INTO missed_order SET financial_id = '".$financial_id."', owner_id = '".$owner_id."', admin_id = '".$admin_id."', pharmacy_id = '".$pharmacy_id."', product_id = '".$product_id."', created = '".$created."', createdby = '".$createdby."'";
          ...
          if($res && mysqli_num_rows($res) > 0){
            ...
            while ($row = mysqli_fetch_array($res)) {
              ...
              if(isset($row['ids']) && $row['ids'] != ''){
                $ids = explode(',', $row['ids']);
                $productData = getAllProductWithCurrentStock('', '', 1, $ids);
                if(!empty($productData)){
                  foreach ($productData as $key => $value) {
                    $currentstock = (isset($value['currentstock']) && $value['currentstock'] != '') ? $value['currentstock'] : 0;
                    if($currentstock > 0){
                      ...
                      $detail['generic_name'] = (isset($value['generic_name'])) ? $value['generic_name'] : '';
                      $detail['batch_no'] = (isset($value['batch_no'])) ? $value['batch_no'] : '';
                      $detail['ex_date'] = (isset($value['ex_date']) && $value['ex_date'] != '' && $value['ex_date'] != '0000-00-00') ? date('d/m/Y',strtotime($value['ex_date'])) : '';
                      $detail['mrp'] = ((isset($value['mrp']) && $value['mrp'] != '' && $value['mrp'] > 0)) ? $value['mrp'] : 1;
                      $detail['total_qty'] = $currentstock;
                      $detail['ratio'] = ((isset($value['ratio']) && $value['ratio'] != '' && $value['ratio'] > 0)) ? $value['ratio'] : 1;
                      $discount_per = (isset($value['discount_per']) && $value['discount_per'] != '') ? $value['discount_per'] : 0;
                      $detail['discount'] = ((isset($value['discount']) && $value['discount'] == 1)) ? $discount_per : 0;
                      $detail['igst'] = ((isset($value['igst']) && $value['igst'] != '')) ? $value['igst'] : 0;
                      $detail['cgst'] = ((isset($value['cgst']) && $value['cgst'] != '')) ? $value['cgst'] : 0;
                      $detail['sgst'] = ((isset($value['sgst']) && $value['sgst'] != '')) ? $value['sgst'] : 0;
                      $detail['rate'] = ($detail['mrp']/$detail['ratio']);
                      $mainarr['totalproducts'][] = $detail;
                    }
                  }
                }
              }
    ...
    if($_REQUEST['action'] == 'getProductForCustomerReturn'){
      ...
      if($customer_id != '' && $search){
        $query = "SELECT pm.id, pm.product_name, pm.generic_name, pm.mfg_company, pm.batch_no, pm.ex_date, pm.mrp, tbd.qty, tbd.qty_ratio,tbd.rate, tbd.discount, tbd.gst, tbd.igst, tbd.cgst, tbd.sgst, tbd.gst_tax, tbd.totalamount, tb.id as tax_billing_id, tb.invoice_date, tb.invoice_no, tb.final_amount as invoice_amount FROM product_master pm INNER JOIN tax_billing_details tbd ON pm.id = tbd.product_id INNER JOIN tax_billing tb ON tbd.tax_bill_id = tb.id  WHERE tb.customer_id = '".$customer_id."' AND pm.product_name LIKE '%".$search."%' AND tb.pharmacy_id = '".$pharmacy_id."' AND tb.financial_id = '".$financial_id."' AND tbd.created IN (SELECT max(created) FROM tax_billing_details GROUP BY product_id) ORDER BY pm.product_name";
        $res = mysqli_query($conn,$query);
        if ($res && mysqli_num_rows($res)) {
          $data = [];
          while($row = mysqli_fetch_array($res)){
            $arr['id'] = (isset($row['id'])) ? $row['id'] : '';
            $arr['product_name'] = (isset($row['product_name'])) ? $row['product_name'] : '';
            $arr['tax_billing_id'] = (isset($row['tax_billing_id'])) ? $row['tax_billing_id'] : '';
            $arr['mrp'] = (isset($row['mrp']) && $row['mrp'] != '') ? $row['mrp'] : 0;
            $arr['mfg_company'] = (isset($row['mfg_company'])) ? $row['mfg_company'] : '';
            $arr['batch_no'] = (isset($row['batch_no'])) ? $row['batch_no'] : '';
            $arr['ex_date'] = (isset($row['ex_date']) && $row['ex_date'] != '' && $row['ex_date'] != '0000-00-00') ? date('d/m/Y',strtotime($row['ex_date'])) : '';
            ...
            $arr['gst_tax'] = (isset($row['gst_tax']) && $row['gst_tax'] != '') ? $row['gst_tax'] : 0;
            ...
            $arr['invoice_amount'] = (isset($row['invoice_amount']) && $row['invoice_amount'] != '') ? $row['invoice_amount'] : 0;
    ...
    if($_REQUEST['action'] == 'searchProductPurchaseReturn'){
      ...
      if($vendor_id != '' && $search != ''){
        $query = "SELECT pm.id, pm.product_name, pm.mfg_company, pm.batch_no, pm.ex_date, pm.mrp, pd.qty, pd.free_qty, pd.rate, pd.discount, pd.f_rate, pd.f_igst, pd.f_cgst, pd.f_sgst, p.id as purchase_id, p.invoice_date, p.invoice_no, p.total_total as invoice_amount FROM product_master pm INNER JOIN purchase_details pd ON pm.id = pd.product_id INNER JOIN purchase p ON pd.purchase_id = p.id WHERE p.vendor = '".$vendor_id."' AND pm.product_name LIKE '%".$search."%' AND p.pharmacy_id = '".$pharmacy_id."' AND p.financial_id = '".$financial_id."' AND pd.created IN (SELECT max(created) FROM purchase_details GROUP BY product_id) ORDER BY pm.product_name";
        $res = mysqli_query($conn, $query);
        if($res && mysqli_num_rows($res) > 0){
          $data = [];
          while ($row = mysqli_fetch_assoc($res)) {
            $tmp['id'] = (isset($row['id'])) ? $row['id'] : '';
            $tmp['product_name'] = (isset($row['product_name'])) ? $row['product_name'] : '';
            $tmp['mrp'] = (isset($row['mrp'])) ? $row['mrp'] : '';
            $tmp['mfg_company'] = (isset($row['mfg_company'])) ? $row['mfg_company'] : '';
            $tmp['batch_no'] = (isset($row['batch_no'])) ? $row['batch_no'] : '';
            $tmp['ex_date'] = (isset($row['ex_date']) && $row['ex_date'] != '' && $row['ex_date'] != '0000-00-00') ? date('d/m/Y',strtotime($row['ex_date'])) : '';

            $tmp['qty'] = (isset($row['qty']) && $row['qty'] != '') ? $row['qty'] : 0;
            $tmp['free_qty'] = (isset($row['free_qty']) && $row['free_qty'] != '') ? $row['free_qty'] : 0;
            $tmp['rate'] = (isset($row['rate']) && $row['rate'] != '') ? $row['rate'] : 0;
            $tmp['discount'] = (isset($row['discount']) && $row['discount'] != '') ? $row['discount'] : 0;
            $tmp['final_rate'] = (isset($row['f_rate']) && $row['f_rate'] != '') ? $row['f_rate'] : 0;
            $tmp['igst'] = (isset($row['f_igst']) && $row['f_igst'] != '') ? $row['f_igst'] : 0;
            $tmp['cgst'] = (isset($row['f_cgst']) && $row['f_cgst'] != '') ? $row['f_cgst'] : 0;
            $tmp['sgst'] = (isset($row['f_sgst']) && $row['f_sgst'] != '') ? $row['f_sgst'] : 0;

            $tmp['purchase_id'] = (isset($row['purchase_id'])) ? $row['purchase_id'] : '';
            $tmp['invoice_no'] = (isset($row['invoice_no'])) ? $row['invoice_no'] : '';
            $tmp['invoice_date'] = (isset($row['invoice_date']) && $row['invoice_date'] != '' && $row['invoice_date'] != '0000-00-00') ? date('d/m/Y',strtotime($row['invoice_date'])) : '';
            $tmp['invoice_amount'] = (isset($row['invoice_amount']) && $row['invoice_amount'] != '') ? $row['invoice_amount'] : 0;
            $data[] = $tmp;
          }
              ...
              if($productR && mysqli_num_rows($productR) > 0){
                  while($productRow = mysqli_fetch_assoc($productR)){
                      ...
                      if($currentstock > $qty){
                        ...
                        $detail['name'] = (isset($productRow['product_name'])) ? $productRow['product_name'] : '';
                        $detail['mfg_company'] = (isset($productRow['mfg_company'])) ? $productRow['mfg_company'] : '';
                        $detail['batch'] = (isset($productRow['batch_no'])) ? $productRow['batch_no'] : '';
                        $detail['expiry'] = (isset($productRow['ex_date']) && $productRow['ex_date'] != '') ? date('d/m/Y',strtotime($productRow['ex_date'])) : '';
                        $detail['mrp'] = ((isset($productRow['mrp']) && $productRow['mrp'] != '' && $productRow['mrp'] > 0)) ? $productRow['mrp'] : 1;
                        $detail['qty'] = $qty;
                        $detail['total_qty'] = $currentstock;
                        $detail['ratio'] = ((isset($productRow['ratio']) && $productRow['ratio'] != '' && $productRow['ratio'] > 0)) ? $productRow['ratio'] : 1;
                        $discount_per = (isset($productRow['discount_per']) && $productRow['discount_per'] != '') ? $productRow['discount_per'] : 0;
                        $detail['discount'] = ((isset($productRow['discount']) && $productRow['discount'] == 1)) ? $discount_per : 0;
    ...
    if($_REQUEST['action'] == "cashRunningBalance"){
      if((isset($_REQUEST['from']) && $_REQUEST['from'] != '') && (isset($_REQUEST['to']) && $_REQUEST['to'] != '')){
        ...
        $data = countCashRunningBalance($from, $to);
        $result = array('status' => true, 'message' => 'success', 'result' => $data);
      }else{
        $result = array('status' => false, 'message' => 'id not found!', 'result' => '');
      }
    ...
    if($_REQUEST['action'] == "searchProductWithExpired"){
      if(isset($_REQUEST['query']) && $_REQUEST['query'] != ''){
        ...
        $data = getAllProductWithCurrentStock('',$_REQUEST['query'], 0, [], '', $mrp);
        if(!empty($data)){
          $allproduct = [];
            foreach ($data as $key => $value) {
              ...
              $arr['batch'] = (isset($value['batch_no']) && $value['batch_no'] != '') ? $value['batch_no'] : '-';
                if(isset($value['ex_date']) && $value['ex_date'] != ''){
                    if($value['ex_date'] != '0000-00-00'){
    ...
    if($_REQUEST['action'] == "searchProductWithoutExpiredAndWithoutZeroStock-OLD"){
      if(isset($_REQUEST['query']) && $_REQUEST['query'] != ''){
        $mrp = (isset($_REQUEST['mrp'])) ? $_REQUEST['mrp'] : '';
        $data = getAllProductWithCurrentStock('',$_REQUEST['query'],1, [], '', $mrp);
        if(!empty($data)){
          ...
          $allproduct = [];
            foreach ($data as $key => $value) {
                if(isset($value['currentstock']) && $value['currentstock'] > 0){
                  ...
                  $arr['total_qty'] = ((isset($value['currentstock']) && $value['currentstock'] != '')) ? $value['currentstock'] : 0;
                  $arr['unit'] = ((isset($value['unit']) && $value['unit'] != '')) ? $value['unit'] : 0;
                  $arr['mrp'] = ((isset($value['mrp']) && $value['mrp'] != '' && $value['mrp'] > 0)) ? $value['mrp'] : 1;
                  $arr['generic_name'] = (isset($value['generic_name'])) ? $value['generic_name'] : '';
                  $arr['mfg_company'] = (isset($value['mfg_company'])) ? $value['mfg_company'] : '';
                  $arr['ratio'] = ((isset($value['ratio']) && $value['ratio'] != '' && $value['ratio'] > 0)) ? $value['ratio'] : 1;
                  $arr['igst'] = ((isset($value['igst']) && $value['igst'] != '')) ? $value['igst'] : 0;
                  $arr['cgst'] = ((isset($value['cgst']) && $value['cgst'] != '')) ? $value['cgst'] : 0;
                  $arr['sgst'] = ((isset($value['sgst']) && $value['sgst'] != '')) ? $value['sgst'] : 0;
                  $arr['rate'] = ($arr['mrp']/$arr['ratio']);
                  $allproduct[] = $arr;
                }
            }
    ...
    if($_REQUEST['action'] == "ShowCustomerBill"){
      ...
      if($customer_id != ''){
        $data = [];
        $query = "SELECT id, invoice_date, invoice_no FROM tax_billing WHERE pharmacy_id = '".$pharmacy_id."' AND financial_id = '".$financial_id."' AND customer_id = '".$customer_id."' ORDER BY id DESC LIMIT 6";
        $res = mysqli_query($conn, $query);
        if($res && mysqli_num_rows($res) > 0){
          while ($row = mysqli_fetch_assoc($res)) {
            $tmp ['id'] = (isset($row['id'])) ? $row['id'] : '';
    ...
    if($_REQUEST['action'] == "searchCustomerForSale"){
        ...
        if($query != '' && $field != ''){
            $customerQ = "SELECT l.id, l.name, l.mobile, l.email, l.customer_id, l.persional_title, ct.id as city_id, ct.name as city_name, st.state_code_gst as statecode FROM ledger_master l LEFT JOIN own_states st ON l.state = st.id LEFT JOIN own_cities ct ON l.city = ct.id WHERE l.pharmacy_id = '".$pharmacy_id."' AND l.group_id = 10 AND l.status = 1 ";
            if($city_id != ''){
    ...
    if($_REQUEST['action'] == "getcash"){
      $bill_type = (isset($_REQUEST['cash'])) ? $_REQUEST['cash'] : '';
      $voucher_no = getaccountingcashinvoiceno($bill_type);
...
if($_REQUEST['action'] == "gettax"){
  ...
  if($_REQUEST['action'] == "bankrunningbalance"){
   ...
   ";
    ...
    //date:- 26/01/2019 CREATED BY:- VIRAG RAKHOLIYA
...
exit;
    ...
    // added by gautam makwana 04/01/2018
    if($_REQUEST['action'] == "getIhisPrescription"){
        $type = (isset($_REQUEST['type']) && $_REQUEST['type'] != '') ? $_REQUEST['type'] : '0';
        $data['data'] = [];
            $prescriptionData = IpdPrescriptionData($type);
            if(!empty($prescriptionData)){
                foreach($prescriptionData as $key => $value){
                    $tmp['no'] = $key+1;
                    $tmp['patient_opd_id'] = (isset($value['patient_opd_id'])) ? $value['patient_opd_id'] : '';
                    $tmp['doctor_name'] = (isset($value['doctor_name'])) ? $value['doctor_name'] : '';
                    $tmp['p_name'] = (isset($value['p_name'])) ? $value['p_name'] : '';
                    $tmp['email'] = (isset($value['email'])) ? $value['email'] : '';
                    $tmp['mobile_no'] = (isset($value['mobile_no'])) ? $value['mobile_no'] : '';
    ...
    if($_REQUEST['action'] == "getEclinicPrescription"){
        $type = (isset($_REQUEST['type']) && $_REQUEST['type'] != '') ? $_REQUEST['type'] : '0';
        $data['data'] = [];
            $prescriptionData = eClinicPrescriptionData($type);
            if(!empty($prescriptionData)){
                foreach($prescriptionData as $key => $value){
                    $tmp['no'] = $key+1;
                    $tmp['patient_opd_id'] = (isset($value['patient_opd_id'])) ? $value['patient_opd_id'] : '';
 ...
 // Ledger master company name exits or not -------------------------shreya ----11/1/2019
 if($_REQUEST['action'] == "LedgerWiseComanyName"){
    
    $type = (isset($_REQUEST['type'])) ? $_REQUEST['type'] : '';
    $companyname = (isset($_REQUEST['companyname'])) ? $_REQUEST['companyname'] : '';
    $city =(isset($_REQUEST['city'])) ? $_REQUEST['city'] : '';
    $detailid = (isset($_REQUEST['detailid'])) ? $_REQUEST['detailid'] : '';
    $pharmacy_id = $_SESSION['auth']['pharmacy_id'];
  
     $company = "SELECT id, name, group_id FROM ledger_master ";
     $company .= "WHERE pharmacy_id = '".$pharmacy_id."' AND name = '".$companyname."' AND group_id ='".$type."'";
    if(isset($type) && $type == '14' || isset($type) && $type == '10'){
        $company .= " AND city ='".$city."'";     
    }
     if(isset($detailid) && $type != ''){
        $company .= " AND id !='".$detailid."'";     
    }
    
    $companyR = mysqli_query($conn, $company);
    if(mysqli_num_rows($companyR) > 0){
           $message = 'existing';
    } else {
           $message = 'notexisting';
    }
