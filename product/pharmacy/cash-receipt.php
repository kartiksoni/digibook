...
<?php 
  ...
  if(isset($_POST['add'])){
    ...
    if($count > 0){
      $voucher_no = getCashTransactionVoucherNo('receipt');
      ...
      for($i=0; $i < $count; $i++) {
        ...
        $query = "INSERT INTO cash_transaction SET financial_id = '".$financial_id."', owner_id = '".$owner_id."', admin_id = '".$admin_id."', pharmacy_id = '".$pharmacy_id."', payment_type = 'receipt', voucher_no = '".$voucher_no."', voucher_date = '".$voucher_date."', type = '".$type."', perticular = '".$perticular."', amount = '".$amount."', narration = '".$narration."', reverse_charge = '".$reverse_charge."', gst = '".$gst."', batch = '".$batch."', created = '".date('Y-m-d H:i:s')."', createdby = '".$_SESSION['auth']['id']."'";
  ...
  if(isset($_GET['id']) && $_GET['id'] != ''){
    $query = "SELECT * FROM cash_transaction WHERE id = '".$_GET['id']."' AND pharmacy_id = '".$pharmacy_id."' AND payment_type = 'receipt'";
    $res = mysqli_query($conn, $query);
