...
if(isset($_GET['id']) && $_GET['id'] != ''){
  $pharmacy_id = (isset($_SESSION['auth']['pharmacy_id'])) ? $_SESSION['auth']['pharmacy_id'] : '';
   ...
   $query = "SELECT tb.*, lg.name as customer_name, lg.addressline1, lg.addressline2, lg.addressline3, lg.gstno,lg.panno,lg.mobile,lg.pincode,lg.email,tm.name as transportname, st.name as customer_state, st.state_code_gst as statecode,tb.remarks, ct.name as customer_city,dp.name as doctor_name FROM tax_billing tb LEFT JOIN ledger_master lg ON tb.customer_id = lg.id LEFT JOIN own_states st ON lg.state = st.id LEFT JOIN own_cities ct ON lg.city = ct.id LEFT JOIN doctor_profile dp ON tb.doctor = dp.id LEFT JOIN transport_master tm on tb.transporter_id = tm.id WHERE tb.pharmacy_id ='".$pharmacy_id."' AND tb.id='".$_GET['id']."'";
 ...
 if(isset($data['is_general_sale']) && $data['is_general_sale'] == 1){
    ...
    if(isset($data['pharmacy_id']) && $data['pharmacy_id'] != ''){
      $pharmacyQ = "SELECT p.id, p.pharmacy_name, p.address1, p.address2, p.address3, p.gst_no,p.city_name as city,p.mobile_no,p.email,p.logo_url,p.cin_no,st.name as state_name,st.state_code_gst as pharmastatecode FROM pharmacy_profile p LEFT JOIN own_states st ON p.stateId = st.id WHERE p.id='".$data['pharmacy_id']."'";
    ...
    /*------------------------------------------------------GROUP BY HSN CODE WISE AMOUNT START-----------------------------------*/
     if(isset($data['id']) && $data['id'] != ''){
     $hsngroupQ = "SELECT sum(tbd.totalamount) as taxablevalue, tbd.gst,tbd.cgst ,tbd.sgst,pm.hsn_code as hsn  FROM tax_billing_details tbd INNER JOIN product_master pm ON tbd.product_id = pm.id INNER JOIN tax_billing tb ON tbd.tax_bill_id = tb.id WHERE tbd.tax_bill_id = '".$data['id']."' group by pm.hsn_code";
       ...
       <tr>
        <td  style="vertical-align: top; padding: 0;" colspan="2">
          <table  cellpadding="2" cellspacing="0" border="1" style="line-height:10px;font-size:9px;border-bottom: 0;border-top: 0;border-left: 0;border-right: 0;">
            <tr>     
              <td>
                <div style="width:100px;float:left;margin-right: 12px;height: 100px;    margin-bottom: 28px;">
                    <?php $url_img = "http://digibooks.cloud/product/wholesale/logo_images/".$data['pharmacy']['logo_url'];
                     ?>
                  <img src="<?php echo $url_img?>" style="height: 100px;width: 100px;">
                </div>
                <div>
                  <span style="font-size: 13px;line-height: 17px;"><b><?php echo (isset($data['pharmacy']['pharmacy_name'])) ? $data['pharmacy']['pharmacy_name'] : ''; ?> </b></span><br>
                  ...
                  <span style="font-size: 14px;line-height: 16px;"> State: <?php echo (isset($data['pharmacy']['state_name'])) ? $data['pharmacy']['state_name'] : ''; ?> , State Code : <?php echo (isset($data['pharmacy']['pharmastatecode'])) ? $data['pharmacy']['pharmastatecode'] : ''; ?></span> <br>
                  <span style="font-size: 14px;line-height: 16px;"> GST NO : <?php echo (isset($data['pharmacy']['gst_no'])) ? $data['pharmacy']['gst_no'] : ''; ?> </span> <br>
                  <span style="font-size: 14px;line-height: 16px;"> Mobile No : <?php echo (isset($data['pharmacy']['mobile_no'])) ? $data['pharmacy']['mobile_no'] : ''; ?> </span><br>
                  <span style="font-size: 14px;line-height: 16px;"> CIN : <?php echo (isset($data['pharmacy']['cin_no'])) ? $data['pharmacy']['cin_no'] : ''; ?> </span> <br>  
                  <span style="font-size: 14px;line-height: 16px;"> E-Mail : <?php echo (isset($data['pharmacy']['email'])) ? $data['pharmacy']['email'] : ''; ?> </span> <br>  
                </div>
              </td>
            ...
            <td>
             ...
             <table cellpadding="2" cellspacing="0" border="0" style="line-height:10px;font-size:9px;">
              <tr>     
                <td>
                  <div style="font-size: 14px;margin-bottom: 9px;">Buyer</div>
                  <b style="width:300px;float:left;line-height: 7px; font-size: 14px;"><?php echo (isset($data['customer_name']) && $data['customer_name'] != '') ? ucwords(strtolower($data['customer_name'])) : ''; ?> </b>
                </td>
              </tr>
              <tr>     
                <td class="sub">
                     <?php 
                                    ...
                                    ?>
                  <?php echo (isset($customerAddress)) ? $customerAddress : ''; ?> - <?php echo (isset($data['pincode'])) ? $data['pincode'] : '';?>
            ...
            <tr>     
              <td class="sub"> 
               State Name : <?php echo (isset($data['customer_state'])) ? $data['customer_state'] : ''; ?> , State Code : <?php echo (isset($data['statecode'])) ? $data['statecode'] : ''; ?>
             </td>
           </tr>
           <tr>     
            <td class="sub">
             GST NO : <?php echo (isset($data['gstno'])) ? $data['gstno'] : ''; ?>
           </td>
         </tr>
       
        <tr>     
          <td class="sub">
           Mobile No. : <?php echo (isset($data['mobile'])) ? $data['mobile'] : ''; ?> 
         </td>
       </tr>
         <tr>     
          <td class="sub">
...
<tr>
   ...
   <b><?php echo (isset($data['transportname'])) ? $data['transportname'] : ''; ?></b>
 </td>
</tr>
...
<tr class="top">
  <td colspan="3" style="padding:0px;border-bottom:none;border: none;">
   <table cellpadding="10" cellspacing="0" border="1" style="border: 0px solid;font-size:8px;line-height:16px;" frame="below" rules="cols" >					
    <tr class="heading" align="center" style="font-size: 12px;"> 
     ...
     <td rowspan="2" style="width:89px;">Prouduct Name</td>
 ...
 <tr class="heading"  style="font-size: 12px;"> 
  <td colspan="13" style="text-align:left">Amount chargeable (in words)<br>
   <b><?php echo (isset($total_amount) && $total_amount != '') ? number_to_word($total_amount) : 'Zero'; ?></b>
 </td>
 <td  colspan="3" align="center"><br>E. & O.E</td>                            
</tr>
...
<tr class="top">
  <td colspan="2" style="padding:0px; vertical-align: top;border: none; " >
   <table cellpadding="0" cellspacing="0" border="1" style="font-size:8px;line-height:18px;" frame="box" rules="cols">

    <tr class="heading" align="center" style="font-size: 12px"> 
      <td rowspan="2"><b>HSN</b></td>
      <td rowspan="2" style="width:100px;"><b>Taxable Value</b></td>
   ...
   <tr class="heading" style="background-color:#D6D6D6;border-top:1px solid #181818; line-height: 2;"> 
    ...
    <td colspan="2" style="text-align: right;"><b><?php echo (isset($total_tax_amount)) ? amount_format(number_format((round($total_tax_amount, 2)), 2, '.', '')) : 0; ?></b></td>
           ...
           <td align="right" style="text-align:right;padding:0px; vertical-align: top; border: none;"  colspan="1">
            <table cellpadding="2" cellspacing="0" border="0" style="font-size:15px;line-height:16px;" frame="box" rules="cols" >
              ...
              <tr style="background-color:#D6D6D6">
                <td><div style="width:130px;float:left;">Credit Note</div></td>
                <td align="right">
                 0
                </td>
              </tr>
...
<!--</table>-->
<div style="margin-bottom:3px;margin-top: 3px;text-transform: uppercase;text-align: center;">Subject to <?php echo (isset($data['pharmacy']['state_name'])) ? $data['pharmacy']['state_name'] : ''; ?> Jurisdiction</div>
<div style="margin-bottom:3px;margin-top: 3px;text-align: center;">This Computer Generated Bill</div>
</div>
</page>
</body>
</html>
