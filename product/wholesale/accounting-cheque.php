                        ...
                        <div class="col-12 col-md-2">
                          ...
                          <select class="js-example-basic-single" id="bank" style="width:100%" name="bank" required="" data-parsley-errors-container="#error-bank">
                            <?php
                              ...
                              $getbankQ = "SELECT id, bank_name FROM pharmacy_bank_details WHERE pharmacy_id = '".$p_id."'";
                              $getbankR = mysqli_query($conn, $getbankQ);
                              if($getbankR && mysqli_num_rows($getbankR) > 0){
                                while ($getbankRow = mysqli_fetch_assoc($getbankR)) {
                                  $allbank[] = $getbankRow;
                                }
                              }
                            ?>
                            ...
                            <?php if(isset($allbank) && !empty($allbank)){ ?>
                                <?php foreach ($allbank as $key => $value) { ?>
                                  <option value="<?php echo $value['id']; ?>" <?php echo (isset($accountdata['bank_name']) && $accountdata['bank_name'] == $value['id']) ? 'selected' : ''; ?> > <?php echo $value['bank_name']; ?> </option>
                                <?php } ?>
                            <?php } ?>
                          </select>
