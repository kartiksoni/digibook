  ...
  $insert .= "vouchar_date ='".$vouchar_date."',
    ...
    if($result){
      ...
      for($i=0;$i<$count;$i++){
        ...
        if($productid != ''){
          $expiry1 = date('Y-m-d',strtotime(str_replace('/','-',$_POST["expiry"][$i])));
          ...
          }elseif($productdata['batch_no'] != $_POST['batch'][$i]){
            $productquery = "select * from product_master where id = '".$productid."'";
            $run = mysqli_query($conn, $productquery);
            $data = mysqli_fetch_assoc($run);
            $addproduct = "INSERT INTO `product_master`(`owner_id`, `admin_id`, `pharmacy_id`, `user_id`, `finance_year_id`, `product_name`, `generic_name`, `mfg_company`, `schedule_cat`, `product_type`, `product_cat`, `sub_cat`, `hsn_code`, `batch_no`, `ex_date`, `opening_qty`, `opening_qty_godown`, `give_mrp`, `mrp`, `serial_no`, `igst`, `cgst`, `sgst`, `inward_rate`, `sale_rate_local`, `sale_rate_out`, `rack_no`, `self_no`, `box_no`, `company_code`, `opening_stock`, `unit`, `min_qty`, `max_qty`, `ratio`, `status`, `created_at`, `created_by`) VALUES ('".$data['owner_id']."', '".$data['admin_id']."', '".$data['pharmacy_id']."', '".$data['user_id']."', '".$data['finance_year_id']."', '".$data['product_name']."', '".$data['generic_name']."', '".$data['mfg_company']."', '".$data['schedule_cat']."', '".$data['product_type']."', '".$data['product_cat']."', '".$data['sub_cat']."', '".$data['hsn_code']."', '".$batch1."', '".$expiry1."', '".$data['opening_qty']."', '".$data['opening_qty_godown']."', '".$data['give_mrp']."', '".$data['mrp']."', '".$data['serial_no']."', '".$data['igst']."', '".$data['cgst']."', '".$data['sgst']."', '".$data['inward_rate']."', '".$data['sale_rate_local']."', '".$data['sale_rate_out']."', '".$data['rack_no']."', '".$data['self_no']."', '".$data['box_no']."', '".$data['company_code']."', '".$data['opening_stock']."', '".$data['unit']."', '".$data['min_qty']."', '".$data['max_qty']."', '".$data['ratio']."', '".$data['status']."', '".date('Y-m-d H:i:s')."', '".$user_id."')";
                  ...
                  <!-- Main Catagory -->
                    ...
                    <div class="col-12">
                        <div class="purchase-top-btns">
                          <?php 
                          if(isset($user_sub_module) && in_array("Purchase Bill", $user_sub_module)){ 
                          ?>
                            <a href="purchase.php" class="btn btn-dark active">Purchase Bill</a>
                          <?php } 
                          if(isset($user_sub_module) && in_array("Purchase Return", $user_sub_module)){ 
                          ?>
                            <a href="purchase-return.php" class="btn btn-dark">Purchase Return</a>
                          <?php }
                          if(isset($user_sub_module) && in_array("Purchase Return List", $user_sub_module)){
                          ?>
                            <a href="purchase-return-list.php" class="btn btn-dark">Purchase Return List</a>
                          <?php } 
                          if(isset($user_sub_module) && in_array("Cancel List", $user_sub_module)){
                          ?>
                            <a href="purchase-cancel-list.php" class="btn btn-dark btn-fw">Cancel List</a>
                          <?php }
                          if(isset($user_sub_module) && in_array("History", $user_sub_module)){
                           ?>
                 ...
                 <br>
                  ...
                  <div class="form-group row">
                      ...
                      <div class="col-12 col-md-2">
                        ...
                        ?>
                        <input type="text" required="" class="form-control" name="voucher_no" id="voucher_no" value="<?php if(isset($_GET['id'])){ echo $purchase_data['voucher_no']; }else{echo $voucherVal;} ?>" placeholder="Voucher No">
                      </div>
                    ...
                    <div class="form-group row">
                        <div class="col-12 col-md-2">
                          ...
                          <div class="input-group date datepicker">
                            <input type="text" name="invoice_date" class="form-control border" value="<?php if(isset($_GET['id'])){ echo date("d/m/Y",strtotime(str_replace("-","/",$purchase_data['invoice_date']))); }else{echo date("d/m/Y");} ?>">
                            <span class="input-group-addon input-group-append border-left">
                              <span class="mdi mdi-calendar input-group-text"></span>
                            </span>
                          </div>
              ...
              <div class="card">
                <div class="card-body">
                            ...
                            <table id="order-listing1" class="table">
                              ...
                              <tbody id="product-tbody">
                                ...
                                <tr class="product-tr">
                                    ...
                                    <td>
                                      <input type="text" name="qty[]" class="form-control qty onlynumber" id="qty" placeholder="Qty.">
                                    ...
                                    <td>
                                      <input type="text" name=f_rate[] class="form-control f_rate onlynumber" id="f_rate" placeholder="Rate" autocomplete="off">
                                    </td>
                                    <td>
                              ...
                              <?php }else{ ?>
                                ...
                                <tr class="product-tr">
                                    ...
                                    <td>
                                      <input type="text" name="expiry[]" value="<?php echo date("d/m/Y",strtotime(str_replace("-","/",$value['expiry']))); ?>" class="form-control datepicker-ex expiry" style="width: 80px;" id="expiry" placeholder="Expiry" autocomplete="off">
                                    </td>
                                    <td>
                                      <input type="text" value="<?php echo $value['qty']; ?>" name="qty[]" class="form-control qty onlynumber" id="qty" placeholder="Qty.">
                                    ...
                                    <td>
                                      <input type="text" name=f_rate[] value="<?php echo $value['f_rate']; ?>" class="form-control f_rate onlynumber" id="f_rate" placeholder="Rate" autocomplete="off">
                                    </td>
                                    <td>
                                    ...
                                    <tr >
                                      ...
                                      <td align="right">
                                        ...
                                        <div class="radio-inline ml-2">            
                                              ...
                                              <input type="text" value="<?php echo (isset($purchase_data['rs_discount'])) ? $purchase_data['rs_discount'] : ''; ?>" name="rs_discount" class="form-control f_discount priceOnly onlynumber" id="rs_dis" placeholder="Rs." style="display:inline-block;width:80px;">
                                          </div>
                    ...
                    <div class="col-md-12">
                      ...
                      if(isset($_GET['id'])){
                        ?>
                      <button type="submit" name="submit" class="btn btn-success pull-right">Edit</button>
                        <?php 
                      }else{
                      ?>
        ...
        <div id="html-copy" style="display: none;">
          <table>
            <tr class="product-tr">
              ...
              <td>
                <input type="text" name="qty[]" class="form-control qty onlynumber" id="qty" placeholder="Qty." autocomplete="off">
