...
//define('BASE_URL', 'http://digibooks.cloud/product/api/api.php' );
    if(isset($_GET['id'])){
      ...
      $edit_pharamcy = explode(",",$edit['multiple_pharmacy_id']);
    
    }

    if(isset($_POST['submit'])){
      $owner_id = $_SESSION['auth']['id'];
      $pharmacy_id = $_POST['pharmacy_id'][0];
      $mutiple_pharmacy_id = implode(",",$_POST['pharmacy_id']);
      $user_name = $_POST['user_name'];
      $email = $_POST['email'];
      $mobile = $_POST['mobile'];
      $password = md5($_POST['password']);
      $status = $_POST['status'];
      $url = BASE_URL."?action=add_user&name=$user_name&owner_id=$owner_id&pharmacy_id=$pharmacy_id&multiple_pharmacy_id=$mutiple_pharmacy_id&user_type=admin&user_name=$user_name&email=$email&mobile=$mobile&password=$password&type=PHARMACY_WHOLESALE&status=$status";
    ...
    if(isset($_POST['edit'])){
      ...
      $url = BASE_URL."?action=edit_user_data&name=$user_name&user_name=$user_name&email=$email&mobile=$mobile&password=$password&status=$status&pharmacy_id=$pharmacy_id&mutiple_pharmacy_id=$mutiple_pharmacy_id&id=$id";
      $data = file_get_contents($url);
...
<head>
  ...
  <style type="text/css">
    .user-profile .bs-placeholder{border: 1px solid !important;}
    .user-profile .dropdown-toggle{border: 1px solid !important;}
    .user-profile .bootstrap-select{width: 100% !important;}
    .user-profile div.dropdown-menu{border: 1px solid grey !important;}
  </style>
</head>
<body>
                ...
                <div class="card-body user-profile">
                  ...
                  <div class="form-group row">
                    <div class="col-12 col-md-4">
                        <label for="pharmacy_id">Select Pharmacy<span class="text-danger">*</span></label><br/>
                        <select class="selectpicker" data-live-search="true" multiple data-selected-text-format="count" data-style="btn-default" name="pharmacy_id[]" data-parsley-errors-container="#error-vendor" required>
                        ...
                        <?php 
                            ...
                            ?>
                            <option <?php if(isset($edit_pharamcy) && in_array($row['id'],$edit_pharamcy)){echo "selected";} ?>  value="<?php echo $row['id']; ?>"><?php echo $row['pharmacy_name']; ?></option>
                            <?php } ?>
                    </select>
                              ...
                              <table id="order-listing1" class="table">
                                ...
                                <tbody>
                                  ...
                                  <tr>
                                      ...
                                      <?php 
                                      $phamacy_name = array();
                                      $multipal_id = $value['multiple_pharmacy_id'];
                                      $selectQry = "SELECT * FROM `pharmacy_profile` WHERE id IN ($multipal_id)";
    		                          $res = mysqli_query($conn, $selectQry);
    		                          while($row = mysqli_fetch_assoc($res)){
    		                              $phamacy_name[] = $row['pharmacy_name'];
    		                          }
                                      ?>
                                      <td><?php echo implode(",",$phamacy_name); ?></td>
