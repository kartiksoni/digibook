...
<?php 
  ...
  if(isset($_POST['submit'])){
    ...
    if(isset($_GET['id']) && $_GET['id'] != ''){
        $query = "UPDATE unit_master SET ";
    }else{
        $query = "INSERT INTO unit_master SET ";
    }
    $query .= "pharmacy_id = '".$pharmacy_id."', unit_name = '".$unit_name."' , status = '".$status."', ";
    ...
    if($res){
        if(isset($_GET['id']) && $_GET['id'] != ''){
            $_SESSION['msg']['success'] = 'Unit Update successfully.';
        }else{
            $_SESSION['msg']['success'] = 'Unit Add successfully.';
        }
    }else{
        if(isset($_GET['id']) && $_GET['id'] != ''){
            $_SESSION['msg']['fail'] = 'Unit Update Fail!';
        }else{
            $_SESSION['msg']['fail'] = 'Unit Add Fail!';
        }
    }
    header('location:unit-master.php');exit;
  }
  if(isset($_GET['id']) && $_GET['id'] != ''){
    $editQuery = "SELECT * FROM unit_master WHERE id = '".$_GET['id']."' AND pharmacy_id = '".$pharmacy_id."'";
                  ...
                  <form class="forms-sample" method="POST" autocomplete="off">
                    <div class="form-group row">
                        <div class="col-12 col-md-3">
                          ...
                          <input type="text" required name="unit_name" value="<?php echo (isset($data['unit_name'])) ? $data['unit_name'] : ''; ?>" class="form-control" placeholder="Unit Name">
                        </div>
                    ...
                    <h4 class="card-title mt-30">View Unit Master</h4>
                    <hr class="alert-dark">
                    <div class="row">
                        <div class="col-12">
                          <table class="table datatable">
                            ...
                            <tbody>
                              <!-- Row Starts -->   
                              <?php
                                $getAllTransportQ = "SELECT * FROM `unit_master` WHERE pharmacy_id = '".$pharmacy_id."' ORDER BY id DESC";
                              ...
                              <?php if($getAllTransportR && mysqli_num_rows($getAllTransportR) > 0){ $i = 1; ?>
                                <?php while ($row = mysqli_fetch_array($getAllTransportR)) { ?>
                                  <tr>
                                      ...
                                      ?>
                                      <td>
                                        <button type="button" class="btn btn-sm btn-toggle changestatus <?php echo (isset($row['status']) && $row['status'] == 1) ? 'active' : ''; ?>" data-table="unit_master" data-id="<?php echo $row['id']; ?>" data-toggle="button" aria-pressed="<?php echo (isset($row['status']) && $row['status'] == 1) ? true : false; ?>" autocomplete="off">
                                          <div class="handle"></div>
                                        </button>
                                      </td>
                                      <td>
                                        <a class="btn  btn-behance p-2" href="unit-master.php?id=<?php echo $row['id']; ?>" title="edit"><i class="fa fa-pencil mr-0"></i></a>
                                      </td>
                                  </tr>
                                  <?php $i++; } ?>
                              <?php } ?>
