...
<?php 
    if(isset($_GET['id']) && $_GET['id'] != ''){
        $pharmacy_id = (isset($_SESSION['auth']['pharmacy_id'])) ? $_SESSION['auth']['pharmacy_id'] : '';
         ...
         $query = "SELECT tb.id, tb.customer_name, tb.city_id, tb.is_general_sale, tb.pharmacy_id, tb.invoice_no, tb.invoice_date, tb.bill_type, tb.total_return_amount, tb.alltotalamount, tb.couriercharge, tb.couriercharge_val, tb.totaligst, tb.totalcgst, tb.totalsgst, tb.totaltaxgst, tb.discount_type, tb.discount_per, tb.discount_rs, tb.cr_db_type, tb.cr_db_val, tb.final_amount, lg.name as l_customer_name, lg.addressline1, lg.addressline2, lg.addressline3, lg.gstno,st.name as customer_state, ct.name as customer_city,dp.name as doctor_name FROM tax_billing tb LEFT JOIN ledger_master lg ON tb.customer_id = lg.id LEFT JOIN own_states st ON lg.state = st.id LEFT JOIN own_cities ct ON lg.city = ct.id LEFT JOIN doctor_profile dp ON tb.doctor = dp.id  WHERE tb.pharmacy_id ='".$pharmacy_id."' AND tb.id='".$_GET['id']."'";
