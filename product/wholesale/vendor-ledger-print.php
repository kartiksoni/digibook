...
<?php 
	...
	$vendor = getPerticularDetail($id);
	$pharmacy = getPharmacyDetail();
	$data = vendorLedger($id, $from, $to, 1);
?>
<!DOCTYPE html>
<html>
<head>
  			...
  			<tr>
    			<td style="border:none;padding:10px;color:#CC0000;font-size:16px;text-align:center;line-height:20px;">
    				<span style="font-size:18px">Party 	Name : <b> <?php echo (isset($vendor['name'])) ? $vendor['name'] : 'Unknown Name'; ?> </b></span> <br>
      				<span style="font-size:15px;line-height:24px;font-weight:bold;">City: <?php echo (isset($vendor['city'])) ? $vendor['city'] : 'Unknown City'; ?>  <br>GST No: <?php echo (isset($vendor['gstno'])) ? $vendor['gstno'] : 'Unknown GSTNO'; ?> </span>
      			</td>
  			</tr>
		</tbody>
		</table>
