...
<?php 
  ...
  if(isset($_POST['add'])){
    ...
    if($count > 0){
      $voucher_no = getCashTransactionVoucherNo('payment');
      ...
      for($i=0; $i < $count; $i++) {
        ...
        $query = "INSERT INTO cash_transaction SET financial_id = '".$financial_id."', owner_id = '".$owner_id."', admin_id = '".$admin_id."', pharmacy_id = '".$pharmacy_id."', payment_type = 'payment', voucher_no = '".$voucher_no."', voucher_date = '".$voucher_date."', type = '".$type."', perticular = '".$perticular."', amount = '".$amount."', narration = '".$narration."', reverse_charge = '".$reverse_charge."', gst = '".$gst."', batch = '".$batch."', created = '".date('Y-m-d H:i:s')."', createdby = '".$_SESSION['auth']['id']."'";
        $res = mysqli_query($conn, $query);
      }
      $_SESSION['msg']['success'] = "Cash Payment Added Successfully.";
      header('Location: view-cash-payment.php');exit;
    }else{
      $_SESSION['msg']['fail'] = "At least one transaction is required!";
      header('Location: cash-payment.php');exit;
    }
  }
  ...
  if(isset($_POST['update'])){
    ...
    $res = mysqli_query($conn, $query);
    if($res){
      $_SESSION['msg']['success'] = "Cash Payment Update Successfully.";
      header('Location: view-cash-payment.php');exit;
    }else{
      $_SESSION['msg']['fail'] = "Cash Payment Update Fail! Try Again.";
      header('Location: cash-payment.php');exit;
    }
  }

  if(isset($_GET['id']) && $_GET['id'] != ''){
    $query = "SELECT * FROM cash_transaction WHERE id = '".$_GET['id']."' AND pharmacy_id = '".$pharmacy_id."' AND payment_type = 'payment'";
    $res = mysqli_query($conn, $query);
                ...
                <div class="card">
                  <div class="card-body">
                      ...
                      <div class="detail-row">
                        <?php if(isset($editData) && !empty($editData)){ ?>
                          <div class="row form-group">
                            ...
                            <div class="col-md-3">
                              ...
                              <input type="text" class="form-control amount onlynumber" name="amount" placeholder="Amount" value="<?php echo (isset($editData['amount'])) ? $editData['amount'] : ''; ?>" required>
                            </div>
                            <div class="col-md-3">
                        ...
                        <?php }else{ ?>
                          <div class="row form-group">
                            ...
                            <div class="col-md-3">
                              <label for="amount">Amount <span class="text-danger">*</span></label>
                              <input type="text" class="form-control amount onlynumber" name="amount[]" placeholder="Amount" required>
                            </div>
                            <div class="col-md-3">
                              <label for="narration">Narration</label>
                    ...
                    <div id="detail-footer">
                      <a href="view-cash-payment.php" class="btn btn-light">Back</a>
