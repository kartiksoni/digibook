<?php 
    ...
    function getLastSaleRateByProduct($product_id = null, $customer_id = null){
        ...
        $query = "SELECT tbd.id, tbd.rate, tb.created FROM tax_billing_details tbd INNER JOIN tax_billing tb ON tbd.tax_bill_id = tb.id WHERE tb.pharmacy_id = '".$pharmacy_id."' AND tb.financial_id = '".$financial_id."' AND product_id = '".$product_id."' AND tb.customer_id = '".$customer_id."' ORDER BY tb.id DESC LIMIT 3";
    ...
    function getCurrentAssetsSecond($fromdate, $todate, $is_financial = 0){
        ...
        $data = [];
            ...
            $totalClosingStocke = getTotalClosingStock($from, $to, $is_financial);
            ...
            $sundaryDebotors = getGroupWiseClosingBalance(14, $from, $to, $is_financial);
            ...
            $cashinHand = countCashRunningBalance($from, $to, $is_financial);
    ...
    /*----THIS FUNCTION IS USED TO GET INVOICE NO FOR BILL OF SUPPLY  - GAUTAM MAKWANA - 28-11-2018 - START------*/
    function getInvoiceNoForBillOfSupply($bill_type = null){
        global $conn;
        global $pharmacy_id;
        global $financial_id;
        $invoice_no = '';
        
        $pefixQ = "SELECT id, bos_cash, bos_debit FROM `series_prefix` WHERE pharmacy_id='".$pharmacy_id."'";
        $pefixR = mysqli_query($conn, $pefixQ);
        if($pefixR && mysqli_num_rows($pefixR) > 0){
            $pefixRow = mysqli_fetch_array($pefixR);
            $pefix_row['pefix_name_cash'] = (isset($pefixRow['bos_cash'])) ? $pefixRow['bos_cash'] : '';
            $pefix_row['pefix_name_debit'] = (isset($pefixRow['bos_debit'])) ? $pefixRow['bos_debit'] : '';
        }else{
        ...
        if($bill_type == 'Cash'){
            $query = "SELECT invoice_no FROM bill_of_supply WHERE bill_type = 'Cash' AND pharmacy_id = '".$pharmacy_id."' AND financial_id = '".$financial_id."' ORDER BY id DESC LIMIT 1";
            $res = mysqli_query($conn, $query);
            if($res){
            ...
            $query = "SELECT invoice_no FROM bill_of_supply WHERE bill_type = 'Debit' AND pharmacy_id = '".$pharmacy_id."' ORDER BY id DESC LIMIT 1";
            $res = mysqli_query($conn, $query);
            if($res){
              $count = mysqli_num_rows($res);
                if($count !== '' && $count !== 0){
    ...
    /*----THIS FUNCTION IS USED TO GET INVOICE NO FOR BILL OF SUPPLY  - GAUTAM MAKWANA - 28-11-2018 - END------*/
    ...
    /*---THIS FUNCTION IS USED TO DELETE ADMIN - Kartik Champaneriya - 05-12-2018 - START----*/ 
    ...
    /*---THIS FUNCTION IS USED TO DELETE ADMIN - Kartik Champaneriya - 05-12-2018 - END----*/
    
    /*------------------------------------- TAX BILLING MAIL-------SHREYA----------------5-12-2018-----------------------   */
  ...
  .company-gst
  {
    ...
    /*------------------------------------- TAX BILLING MAIL-----------------------5-12-2018--------END---------------   */
    
    function defult_leger(){
        global $conn;
        global $account_flag;
        
        $pharmacyQ = "SELECT * FROM `ledger_master` WHERE pharmacy_id = '".$_SESSION['auth']['pharmacy_id']."'"; 
    	$pharmacyR = mysqli_query($conn, $pharmacyQ);
    	if($pharmacyR){
    ...
    // THIS FUNCTION IS USED TO DELETE LEDGER - 08-12-2018 By KARTIK
    ...
    function getTotalClosingStock($from = null, $to = null, $is_financial = 0){
        ...
        $getMethosQ = "SELECT id,close_type FROM pharmacy_profile WHERE id = '".$pharmacy_id."'";
        ...
        if($getMethosR && mysqli_num_rows($getMethosR) > 0){
            ...
            $method = (isset($getMethosRow['close_type'])) ? $getMethosRow['close_type'] : '';
        }
        
        if(isset($method) && $method == 'LIFO'){
            $getAllProductQ = "SELECT id, opening_stock FROM product_master WHERE pharmacy_id = '".$pharmacy_id."' ORDER BY id";
            ...
            if($getAllProductR && mysqli_num_rows($getAllProductR) > 0){
                while($getAllProductRow = mysqli_fetch_assoc($getAllProductR)){
                    ...
                    if($purchaseR && mysqli_num_rows($purchaseR) > 0){
                        $purchaseRow = mysqli_fetch_assoc($purchaseR);
                        $total_purchase_qty = (isset($purchaseRow['totalqty']) && $purchaseRow['totalqty'] != '') ? $purchaseRow['totalqty'] : 0;
    ...
    /*---------------THIS FUNCTION IS USED TO GET TAX SUMMARY REPORT-GAUTAM MAKWANA-17-12-2018-START---------------*/
    function getTaxSummaryReport($from = null, $to = null, $is_financial = 0){
        global $conn;
        global $pharmacy_id;
        global $financial_id;
        $data = [];
        ...
        // Sale
        $saleQ = "SELECT tb.id, tb.customer_name, tb.invoice_date, tb.invoice_no, tb.bill_type, tb.totaligst, tb.totalcgst, tb.totalsgst, tb.final_amount, tb.is_general_sale, lg.name as customer FROM tax_billing tb LEFT JOIN ledger_master lg ON tb.customer_id = lg.id WHERE tb.pharmacy_id = '".$pharmacy_id."'";
        if(isset($is_financial) && $is_financial == 1){
            $saleQ .= " AND tb.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
        ...
        if($saleR && mysqli_num_rows($saleR) > 0){
            while ($saleRow = mysqli_fetch_assoc($saleR)) {
                $tmp['id'] = (isset($saleRow['id'])) ? $saleRow['id'] : '';
                ...
                $tmp['bill_type'] = (isset($saleRow['bill_type'])) ? $saleRow['bill_type'] : '';
                if(isset($saleRow['is_general_sale']) && $saleRow['is_general_sale'] == 1){
                    $tmp['perticular'] = (isset($saleRow['customer_name'])) ? $saleRow['customer_name'].' (General)' : '';
                }else{
                    $tmp['perticular'] = (isset($saleRow['customer'])) ? $saleRow['customer'] : '';
                }
                $tmp['igst'] = (isset($saleRow['totaligst']) && $saleRow['totaligst'] != '') ? $saleRow['totaligst'] : 0;
                $tmp['cgst'] = (isset($saleRow['totalcgst']) && $saleRow['totalcgst'] != '') ? $saleRow['totalcgst'] : 0;
                $tmp['sgst'] = (isset($saleRow['totalsgst']) && $saleRow['totalsgst'] != '') ? $saleRow['totalsgst'] : 0;
                $tmp['amount'] = (isset($saleRow['final_amount']) && $saleRow['final_amount'] != '') ? $saleRow['final_amount'] : 0;
                $tmp['url'] = 'sales-tax-billing.php?id='.$tmp['id'];
                $tmp['type'] = 'Sale';
                $data[] = $tmp;
            }
        }
        ...
        // Purchase
        $purchaseQ = "SELECT p.id, p.invoice_date, p.invoice_no, p.purchase_type, p.total_igst, p.total_cgst, p.total_sgst, p.total_total as final_amount, lg.name as vondor_name FROM purchase p LEFT JOIN ledger_master lg ON p.vendor = lg.id WHERE p.pharmacy_id = '".$pharmacy_id."'";
        if(isset($is_financial) && $is_financial == 1){
            $purchaseQ .= " AND p.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
        ...
        if($purchaseR && mysqli_num_rows($purchaseR) > 0){
            while ($purchaseRow = mysqli_fetch_assoc($purchaseR)) {
                $tmp1['id'] = (isset($purchaseRow['id'])) ? $purchaseRow['id'] : '';
                $tmp1['invoice_date'] = (isset($purchaseRow['invoice_date'])) ? $purchaseRow['invoice_date'] : '';
                $tmp1['invoice_no'] = (isset($purchaseRow['invoice_no'])) ? $purchaseRow['invoice_no'] : '';
                $tmp1['bill_type'] = (isset($purchaseRow['purchase_type'])) ? $purchaseRow['purchase_type'] : '';
                $tmp1['perticular'] = (isset($purchaseRow['vondor_name'])) ? $purchaseRow['vondor_name'] : '';
                $tmp1['igst'] = (isset($purchaseRow['total_igst']) && $purchaseRow['total_igst'] != '') ? $purchaseRow['total_igst'] : 0;
                $tmp1['cgst'] = (isset($purchaseRow['total_cgst']) && $purchaseRow['total_cgst'] != '') ? $purchaseRow['total_cgst'] : 0;
                $tmp1['sgst'] = (isset($purchaseRow['total_sgst']) && $purchaseRow['total_sgst'] != '') ? $purchaseRow['total_sgst'] : 0;
                $tmp1['amount'] = (isset($purchaseRow['final_amount']) && $purchaseRow['final_amount'] != '') ? $purchaseRow['final_amount'] : 0;
                $tmp1['url'] = 'purchase.php?id='.$tmp1['id'];
                $tmp1['type'] = 'Purchase';
                $data[] = $tmp1;
            }
        }
    ...
    /*---------------THIS FUNCTION IS USED TO GET TAX SUMMARY REPORT-GAUTAM MAKWANA-17-12-2018-END-----------------*/
    
    /*-----------THIS FUNCTION IS USED TO GET FREIGHT/COURIER TAX LEDGER - GAUTAM MAKWANA - 17-12-2018-START-----------*/
    function getFreightCourierLedger($from = null, $to = null, $is_financial = 0){
        global $conn;
        global $pharmacy_id;
        global $financial_id;
        $data = [];
        ...
        // Sale
        $saleQ = "SELECT tb.id, tb.customer_name, tb.invoice_date, tb.invoice_no, tb.bill_type, tb.couriercharge_val, tb.is_general_sale, lg.name as customer FROM tax_billing tb LEFT JOIN ledger_master lg ON tb.customer_id = lg.id WHERE tb.pharmacy_id = '".$pharmacy_id."' AND tb.couriercharge_val IS NOT NULL AND tb.couriercharge_val != 0";
        if(isset($is_financial) && $is_financial == 1){
            $saleQ .= " AND tb.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
        ...
        if($saleR && mysqli_num_rows($saleR) > 0){
            while ($saleRow = mysqli_fetch_assoc($saleR)) {
                $tmp['id'] = (isset($saleRow['id'])) ? $saleRow['id'] : '';
                $tmp['date'] = (isset($saleRow['invoice_date'])) ? $saleRow['invoice_date'] : '';
                $tmp['no'] = (isset($saleRow['invoice_no'])) ? $saleRow['invoice_no'] : '';
                $tmp['bill_type'] = (isset($saleRow['bill_type'])) ? $saleRow['bill_type'] : '';
                if(isset($saleRow['is_general_sale']) && $saleRow['is_general_sale'] == 1){
                    $tmp['perticular'] = (isset($saleRow['customer_name'])) ? $saleRow['customer_name'].' (General)' : '';
                }else{
                ...
                $amount = (isset($saleRow['couriercharge_val']) && $saleRow['couriercharge_val'] != '') ? $saleRow['couriercharge_val'] : 0;
                if($amount < 0){
                    $tmp['debit'] = $amount;
                    $tmp['credit'] = '';
                }else{
        ...
        // Purchase
        $purchaseQ = "SELECT p.id, p.invoice_date, p.invoice_no, p.vouchar_date, p.voucher_no, p.purchase_type, p.total_courier, lg.name as vondor_name FROM purchase p LEFT JOIN ledger_master lg ON p.vendor = lg.id WHERE p.pharmacy_id = '".$pharmacy_id."' AND p.total_courier IS NOT NULL AND p.total_courier != 0";
        if(isset($is_financial) && $is_financial == 1){
            $purchaseQ .= " AND p.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
            $purchaseQ .= " AND p.vouchar_date >= '".$from."' AND p.vouchar_date <= '".$to."'";
        }
        $purchaseR = mysqli_query($conn, $purchaseQ);
        if($purchaseR && mysqli_num_rows($purchaseR) > 0){
            while ($purchaseRow = mysqli_fetch_assoc($purchaseR)) {
                ...
                $tmp1['no'] = (isset($purchaseRow['voucher_no'])) ? $purchaseRow['voucher_no'] : '';
                $tmp1['bill_type'] = (isset($purchaseRow['purchase_type'])) ? $purchaseRow['purchase_type'] : '';
                $tmp1['perticular'] = (isset($purchaseRow['vondor_name'])) ? $purchaseRow['vondor_name'] : '';

                $amount1 = (isset($purchaseRow['total_courier']) && $purchaseRow['total_courier'] != '') ? $purchaseRow['total_courier'] : 0;
                if($amount1 < 0){
                    $tmp1['credit'] = $amount1;
                    $tmp1['debit'] = '';
                }else{
    ...
    /*-----------THIS FUNCTION IS USED TO GET FREIGHT/COURIER TAX LEDGER - GAUTAM MAKWANA - 17-12-2018-END-----------*/
    
    /*-----------THIS FUNCTION IS USED TO GET FREIGHT/COURIER TAX LEDGER - GAUTAM MAKWANA - 17-12-2018-START-----------*/
    function getDiscountLedger($from = null, $to = null, $is_financial = 0){
        global $conn;
        global $pharmacy_id;
        global $financial_id;
        $data = [];
        ...
        // Sale
        $saleQ = "SELECT tb.id, tb.customer_name, tb.invoice_date, tb.invoice_no, tb.bill_type, tb.discount_rs, tb.is_general_sale, lg.name as customer FROM tax_billing tb LEFT JOIN ledger_master lg ON tb.customer_id = lg.id WHERE tb.pharmacy_id = '".$pharmacy_id."' AND tb.discount_rs IS NOT NULL AND tb.discount_rs != 0";
        if(isset($is_financial) && $is_financial == 1){
            $saleQ .= " AND tb.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
        ...
        if($saleR && mysqli_num_rows($saleR) > 0){
            while ($saleRow = mysqli_fetch_assoc($saleR)) {
                ...
                $amount = (isset($saleRow['discount_rs']) && $saleRow['discount_rs'] != '') ? $saleRow['discount_rs'] : 0;
                if($amount < 0){
                    $tmp['debit'] = $amount;
                    $tmp['credit'] = '';
                }else{
        ...
        // Purchase
        $purchaseQ = "SELECT p.id, p.invoice_date, p.invoice_no, p.vouchar_date, p.voucher_no, p.purchase_type, p.rs_discount, lg.name as vondor_name FROM purchase p LEFT JOIN ledger_master lg ON p.vendor = lg.id WHERE p.pharmacy_id = '".$pharmacy_id."' AND p.rs_discount IS NOT NULL AND p.rs_discount != 0";
        if(isset($is_financial) && $is_financial == 1){
            $purchaseQ .= " AND p.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
        ...
        if($purchaseR && mysqli_num_rows($purchaseR) > 0){
            while ($purchaseRow = mysqli_fetch_assoc($purchaseR)) {
                ...
                $amount1 = (isset($purchaseRow['rs_discount']) && $purchaseRow['rs_discount'] != '') ? $purchaseRow['rs_discount'] : 0;
                if($amount1 < 0){
                    $tmp1['credit'] = $amount1;
                    $tmp1['debit'] = '';
                }else{
    ...
    /*-----------THIS FUNCTION IS USED TO GET GSTR B2B REPORT - GAUTAM MAKWANA - 17-12-2018-END-----------*/
    ...
    function getGSTR1B2CLReport($from = null, $to = null, $is_financial = 0){
        ...
        $saleQ = "SELECT tbd.id as tbdid, tbd.gst as rate, SUM(tbd.totalamount) as taxable_amount,tb.id as tbid, tb.invoice_date, tb.invoice_no, tb.final_amount, tb.totaligst, tb.totalcgst, tb.totalsgst, lg.id as customer_id, lg.name as customer_name, lg.gstno as customer_gstno, CONCAT(st.state_code_gst, '-',st.name) AS customer_state FROM tax_billing_details tbd INNER JOIN tax_billing tb ON tbd.tax_bill_id = tb.id LEFT JOIN ledger_master lg ON tb.customer_id = lg.id LEFT JOIN own_states st ON lg.state = st.id WHERE tb.pharmacy_id = '".$pharmacy_id."' AND tb.is_general_sale != 1 AND lg.customer_type = 'GST_Regular' AND st.state_code_gst != '".$state_code."' AND tbd.gst IS NOT NULL AND tbd.gst != 0 AND tb.final_amount >= 250000";
        if(isset($is_financial) && $is_financial == 1){
            $saleQ .= " AND tb.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
    ...
    function getGSTR1B2CSReport($from = null, $to = null, $is_financial = 0){
        ...
        $saleQ = "SELECT tbd.id as tbdid, tbd.gst as rate, SUM(tbd.totalamount) as taxable_amount,tb.id as tbid, tb.invoice_date, tb.invoice_no, tb.final_amount, tb.totaligst, tb.totalcgst, tb.totalsgst, lg.id as customer_id, lg.name as customer_name, lg.gstno as customer_gstno, CONCAT(st.state_code_gst, '-',st.name) AS customer_state FROM tax_billing_details tbd INNER JOIN tax_billing tb ON tbd.tax_bill_id = tb.id LEFT JOIN ledger_master lg ON tb.customer_id = lg.id LEFT JOIN own_states st ON lg.state = st.id WHERE (lg.customer_type = 'GST_unregistered' OR lg.customer_type = 'Consumer') AND tb.pharmacy_id = '".$pharmacy_id."' AND tb.is_general_sale != 1";
        if(isset($is_financial) && $is_financial == 1){
            $saleQ .= " AND tb.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
    ...
    function getGSTR1HSNReport($from = null, $to = null, $is_financial = 0){
        ...
        $total_sgst = 0;
            ...
            $saleReturnQ = "SELECT SUM(srd.qty) as totalqty, SUM(srd.amount) as taxable_amount, SUM(srd.amount*srd.igst/100) as igst_amount, SUM(srd.amount*srd.cgst/100) as cgst_amount, SUM(srd.amount*srd.sgst/100) as sgst_amount, pm.id as product_id, pm.product_name, pm.hsn_code FROM sale_return_details srd LEFT JOIN sale_return sr ON srd.sale_return_id = sr.id LEFT JOIN product_master pm ON srd.product_id = pm.id WHERE sr.pharmacy_id = '".$pharmacy_id."'";
            if(isset($is_financial) && $is_financial == 1){
    ...
    /*function getGSTR1HSNReport($from = null, $to = null, $is_financial = 0){
        ...
        $saleQ = "SELECT SUM(tbd.qty) as totalqty, SUM(tbd.totalamount) as taxable_amount, SUM(tbd.totalamount*tbd.igst/100) as igst_amount, SUM(tbd.totalamount*tbd.cgst/100) as cgst_amount, SUM(tbd.totalamount*tbd.sgst/100) as sgst_amount, pm.hsn_code FROM tax_billing_details tbd LEFT JOIN tax_billing tb ON tbd.tax_bill_id = tb.id LEFT JOIN product_master pm ON tbd.product_id = pm.id WHERE tb.pharmacy_id = '".$pharmacy_id."' AND tb.is_general_sale != 1";
        if(isset($is_financial) && $is_financial == 1){
            $saleQ .= " AND tb.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
        ...
        // $saleQ .= " GROUP BY pm.hsn_code ORDER BY tb.invoice_date";
        $saleQ .= " GROUP BY tbd.product_id, pm.hsn_code ORDER BY pm.hsn_code";
        $saleR = mysqli_query($conn, $saleQ);
        if($saleR && mysqli_num_rows($saleR) > 0){
            $detail = [];
            $no_of_hsn = 0;
            ...
            $total_sgst = 0;
            while($saleRow = mysqli_fetch_assoc($saleR)){
                $tmp['hsn'] = (isset($saleRow['hsn_code']) && $saleRow['hsn_code'] != '') ? $saleRow['hsn_code'] : 'Unknown HSN';
                $tmp['desc'] = '';
                $tmp['uqc'] = '';
                $tmp['total_qty'] = (isset($saleRow['totalqty']) && $saleRow['totalqty'] != '') ? $saleRow['totalqty'] : 0;
                $tmp['total_taxable'] = (isset($saleRow['taxable_amount']) && $saleRow['taxable_amount'] != '') ? $saleRow['taxable_amount'] : 0;
                $tmp['total_igst'] = (isset($saleRow['igst_amount']) && $saleRow['igst_amount'] != '') ? $saleRow['igst_amount'] : 0;
                $tmp['total_cgst'] = (isset($saleRow['cgst_amount']) && $saleRow['cgst_amount'] != '') ? $saleRow['cgst_amount'] : 0;
                $tmp['total_sgst'] = (isset($saleRow['sgst_amount']) && $saleRow['sgst_amount'] != '') ? $saleRow['sgst_amount'] : 0;
                $tmp['total_cess'] = 0;
                $tmp['total_value'] = ($tmp['total_taxable']+$tmp['total_igst']+$tmp['total_cgst']+$tmp['total_sgst']);
    ...
    function tax_summary($from = null, $to = null){
        global $conn;
        global $pharmacy_id;
        $gst['gst'] = array();
        $SaleQ = "SELECT * FROM tax_billing_details tbd INNER JOIN tax_billing tb ON tb.id = tbd.tax_bill_id WHERE tb.pharmacy_id = '".$_SESSION['auth']['pharmacy_id']."'AND tb.invoice_date >='".$from."' and tb.invoice_date <='".$to."'";
        $saleR = mysqli_query($conn, $SaleQ);
        if($saleR && mysqli_num_rows($saleR) > 0){
            while($saleReturnRow = mysqli_fetch_assoc($saleR)){
                $gst['gst']['cgst'][$saleReturnRow['cgst']][] = $saleReturnRow['cgst'];
                $gst['gst']['sgst'][$saleReturnRow['sgst']][] = $saleReturnRow['sgst'];
            }
        }
        echo"<pre>";
        print_r($gst);exit;
    ...
    /*-----------------------GET TAX LIABILITY REPORT - 24-12-2018 - GAUTAM MAKWANA - START------------------------*/
    function getTaxLiabilityReport($from = null, $to = null, $is_financial = 0){
        ...
        if($saleCgstR && mysqli_num_rows($saleCgstR) > 0){
            while ($saleCgstRow = mysqli_fetch_assoc($saleCgstR)) {
                ...
                $freight_tax = 0;
                if($saleCgstRow['freight_amount'] != 0 && $saleCgstRow['freight_amount'] != '' && $saleCgstRow['freight_per'] == $gst){
        ...
        $saleReturnCgstQ = "SELECT sr.id as sr_id, srd.id as srd_id, srd.cgst, SUM(srd.qty*srd.rate) as taxable_value, SUM(((srd.qty+srd.free_qty)*(srd.rate))*srd.cgst/100) as tax FROM `sale_return` sr INNER JOIN sale_return_details srd ON sr.id = srd.sale_return_id WHERE srd.cgst IS NOT NULL AND srd.cgst != 0 AND sr.pharmacy_id = '".$pharmacy_id."'";
        if(isset($is_financial) && $is_financial == 1){
        ...
        if($saleSgstR && mysqli_num_rows($saleSgstR) > 0){
            while ($saleSgstRow = mysqli_fetch_assoc($saleSgstR)) {
                ...
                $freight_tax = 0;
                if($saleSgstRow['freight_amount'] != 0 && $saleSgstRow['freight_amount'] != '' && $saleSgstRow['freight_per'] == $gst){
        ...
        $saleReturnSgstQ = "SELECT sr.id as sr_id, srd.id as srd_id, srd.sgst, SUM(srd.qty*srd.rate) as taxable_value, SUM(((srd.qty+srd.free_qty)*(srd.rate))*srd.sgst/100) as tax FROM `sale_return` sr INNER JOIN sale_return_details srd ON sr.id = srd.sale_return_id WHERE srd.sgst IS NOT NULL AND srd.sgst != 0 AND sr.pharmacy_id = '".$pharmacy_id."'";
        if(isset($is_financial) && $is_financial == 1){
        ...
        $saleReturnIgstQ = "SELECT sr.id as sr_id, srd.id as srd_id, srd.igst, SUM(srd.qty*srd.rate) as taxable_value, SUM(((srd.qty+srd.free_qty)*(srd.rate))*srd.igst/100) as tax FROM `sale_return` sr INNER JOIN sale_return_details srd ON sr.id = srd.sale_return_id WHERE srd.igst IS NOT NULL AND srd.igst != 0 AND sr.pharmacy_id = '".$pharmacy_id."'";
        if(isset($is_financial) && $is_financial == 1){
        ...
        if($purchaseCgstR && mysqli_num_rows($purchaseCgstR) > 0){
            while ($purchaseCgstRow = mysqli_fetch_assoc($purchaseCgstR)) {
                ...
                $freight_tax = 0;
                if($purchaseCgstRow['freight_amount'] != 0 && $purchaseCgstRow['freight_amount'] != '' && $gst == $purchaseCgstRow['freight_per']){
        ...
        if($purchaseSgstR && mysqli_num_rows($purchaseSgstR) > 0){
            while ($purchaseSgstRow = mysqli_fetch_assoc($purchaseSgstR)) {
                ...
                $freight_tax = 0;
                if($purchaseSgstRow['freight_amount'] != 0 && $purchaseSgstRow['freight_amount'] != '' && $gst == $purchaseSgstRow['freight_per']){
    ...
    /*-----------------------GET TAX LIABILITY REPORT - 24-12-2018 - GAUTAM MAKWANA - END------------------------*/
    
    /*-----------------------GET MIN QTY OF PRODUCT - 26-12-2018 - GAUTAM MAKWANA - START----------------------*/
    ...
    /*-----------------------GET MIN QTY OF PRODUCT - 26-12-2018 - GAUTAM MAKWANA - END------------------------*/
    
    /*-----------------------GET DEFULT % - 25-12-2018 - Kartik Champaneriya - START------------------------*/
    function defult_gst_per(){
        global $conn;
        global $pharmacy_id;
        global $financial_id;
        $gstMaster1Q = "SELECT * FROM `gst_master` WHERE pharmacy_id='".$pharmacy_id."'";
    ...
    /*-----------------------GET DEFULT % - 25-12-2018 - Kartik Champaneriya - END------------------------*/
    
    /*----------------CREDIT NOTE REPORT - GAUTAM MAKWANA- 29-01-2019 - START------------------------------*/
        ...
        if($view == 'detail'){
            ...
            if($res && mysqli_num_rows($res) > 0){
                while ($row = mysqli_fetch_assoc($res)) {
                    ...
                    $subQuery = "SELECT srd.id, srd.qty, srd.rate, srd.igst, srd.cgst, srd.sgst, ROUND((srd.qty*srd.rate), 2) as taxable_amount, pm.product_name FROM sale_return_details srd LEFT JOIN product_master pm ON srd.product_id = pm.id WHERE srd.sale_return_id = '".$row['id']."'";
                    ...
                    if($subRes && mysqli_num_rows($subRes) > 0){
                        while ($subRow = mysqli_fetch_assoc($subRes)) {
                            ...
                            if($state_code == $cur_statecode){
                                ...
                                $detail['gst_tax'] = round(($sgst_amount+$cgst_amount), 2);
                                $detail['amount'] = ($detail['taxable_amount']+$detail['gst_tax']);
            ...
            $query = "SELECT sr.id, sr.credit_note_no, sr.credit_note_date, lg.name as party_name, lg.gstno, ct.name as city_name, ROUND(SUM(srd.qty*srd.rate), 2) as taxable_amount, ROUND(SUM((srd.qty*srd.rate)*srd.gst/100), 2) as tax_amount, ROUND(SUM((srd.qty*srd.rate)*srd.gst/100)+SUM(srd.qty*srd.rate), 2) as total_amount FROM sale_return sr INNER JOIN sale_return_details srd ON sr.id = srd.sale_return_id LEFT JOIN ledger_master lg ON sr.customer_id = lg.id LEFT JOIN own_cities ct ON lg.city = ct.id WHERE sr.pharmacy_id = '".$pharmacy_id."'";
            if(isset($is_financial) && $is_financial == 1){
                $query .= " AND sr.financial_id = '".$financial_id."'";
            }
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
    ...
    /*----------------CREDIT NOTE REPORT - GAUTAM MAKWANA- 29-01-2019 - END------------------------------*/
            ...
            if($res && mysqli_num_rows($res) > 0){
                while ($row = mysqli_fetch_assoc($res)) {
                    ...
                    if($subRes && mysqli_num_rows($subRes)){
                        ...
                        $tmp['id'] = '';
                        $tmp['item_name'] = ($state_code == $cur_statecode) ? 'GST ON Purchase' : 'IGST On Sale';
                        $tmp['qty'] = '';
    ...
    /*----------------DEBIT NOTE REPORT - GAUTAM MAKWANA- 29-01-2019 - END------------------------------*/
     ...
     /*-----------------------------------Expiry Date Report Start-Rajesh -----------------------------------------------*/
    ...
    /*------------------------------------Expiry Date Report End-----------------------------------------------*/
    
     /*-----------------------------------GET BANK ONE OR ALL - 30-01-19 - GAUTAM MAKWANA - START---------------------------*/
    ...
    /*-----------------------------------GET BANK ONE OR ALL - 30-01-19 - GAUTAM MAKWANA - END---------------------------*/
    ...
    /*------------------------------------Purchase Report 30-01-19-End-----------------------------------------------*/
     ...
     /*--------------------------JV REGISTER REPORT- JIGAR BHALIYA  - 30-01-2018 - START------------------------------*/
    ...
    /*-----------------------------JV REGISTER REPORT- JIGAR BHALIYA -30-01-2018 - END------------------------------*/
    
    /*----------------------LEDGER SUMMARY REPORT - JIGAR BHALIYA -- 31-01-2019----------------------------------*/
    ...
    /*----------------------LEDGER SUMMARY REPORT END - JIGAR BHALIYA -- 31-01-2019----------------------------------*/
    
    /*-----------------------------DAY BOOK REPORT GET CASH ON HAND - GAUTAM MAKWANA - 31-01-2018 - START---------------------------*/
    function daybookCashOnHand($from = null, $is_financial = 0){
        ...
        /*---------------------------SALE BILL START---------------------------------*/
        $saleQ = "SELECT tb.id, ROUND(tb.overalldiscount, 2) as taxable_amount, ROUND((tb.totaligst+tb.totalcgst+tb.totalsgst), 2) as total_gst, tb.roundoff_amount FROM tax_billing tb WHERE tb.pharmacy_id = '".$pharmacy_id."'";
        if(isset($is_financial) && $is_financial == 1){
            $saleQ .= " AND tb.financial_id = '".$financial_id."'";
        }
        if(isset($from) && $from != ''){
        ...
        /*---------------------------SALE RETURN START-----------------------------*/
        $saleReturnQ = "SELECT id, ROUND(taxable_amount, 2) as taxable_amount, ROUND((totaligst+totalcgst+totalsgst), 2) as total_gst, ROUND(finalamount) as total_amount, ROUND((ROUND(finalamount)-(finalamount)), 2) as round_off FROM `sale_return` WHERE pharmacy_id = '".$pharmacy_id."'";
        if(isset($is_financial) && $is_financial == 1){
            $saleReturnQ .= " AND financial_id = '".$financial_id."'";
        }
        if(isset($from) && $from != ''){
          $saleReturnQ .= " AND credit_note_date < '".$from."'";
        }
        $saleReturnR = mysqli_query($conn, $saleReturnQ);
        if($saleReturnR && mysqli_num_rows($saleReturnR) > 0){
            while($saleReturnRow = mysqli_fetch_assoc($saleReturnR)){
        ...
        /*---------------------------PURCHASE BILL START---------------------------*/
        $purchaseQ = "SELECT id, overall_value as taxable_amount, (total_igst+total_cgst+total_sgst) as gst_tax, round_off FROM `purchase` WHERE pharmacy_id = '".$pharmacy_id."'";
        if(isset($is_financial) && $is_financial == 1){
            $purchaseQ .= " AND financial_id = '".$financial_id."'";
        }
        if(isset($from) && $from != ''){
          $purchaseQ .= " AND invoice_date < '".$from."'";
        }
        $purchaseR = mysqli_query($conn, $purchaseQ);
        if($purchaseR && mysqli_num_rows($purchaseR) > 0){
            while($purchaseRow = mysqli_fetch_assoc($purchaseR)){
        ...
        /*--------------------------PURCHASE RETURN START--------------------------*/
        $purchaseReturnQ = "SELECT id, totalamount as taxable_amount, (igst+cgst+sgst) as gst_tax, ROUND((ROUND(finalamount)-(finalamount)), 2) as round_off FROM `purchase_return`  WHERE pharmacy_id = '".$pharmacy_id."'";
        if(isset($is_financial) && $is_financial == 1){
            $purchaseReturnQ .= " AND financial_id = '".$financial_id."'";
        }
        if(isset($from) && $from != ''){
          $purchaseReturnQ .= " AND debit_note_date < '".$from."'";
        }
        $purchaseReturnR = mysqli_query($conn, $purchaseReturnQ);
        if($purchaseReturnR && mysqli_num_rows($purchaseReturnR) > 0){
            while($purchaseReturnRow = mysqli_fetch_assoc($purchaseReturnR)){
    ...
    /*-----------------------------DAY BOOK REPORT GET CASH ON HAND - GAUTAM MAKWANA - 31-01-2018 - END---------------------------*/

    /*-----------------------------DAY BOOK REPORT - GAUTAM MAKWANA - 31-01-2018 - START---------------------------*/
    function daybook($from = null, $to = null, $is_financial = 0){
        ...
        /*---------------------------SALE BILL START---------------------------------*/
        $saleQ = "SELECT tb.id, ROUND(tb.overalldiscount, 2) as taxable_amount, ROUND((tb.totaligst+tb.totalcgst+tb.totalsgst), 2) as total_gst, tb.roundoff_amount, tb.created, tb.invoice_no, tb.invoice_date, lg.name as customer_name, st.state_code_gst as state_code FROM tax_billing tb LEFT JOIN ledger_master lg ON tb.customer_id = lg.id LEFT JOIN own_states st ON lg.state = st.id WHERE tb.pharmacy_id = '".$pharmacy_id."'";
        if(isset($is_financial) && $is_financial == 1){
            $saleQ .= " AND tb.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
        ...
        /*---------------------------SALE RETURN START-----------------------------*/
        $saleReturnQ = "SELECT sr.id, sr.credit_note_no, sr.credit_note_date, ROUND(sr.taxable_amount, 2) as taxable_amount, ROUND((sr.totaligst+sr.totalcgst+sr.totalsgst), 2) as total_gst, ROUND(sr.finalamount) as total_amount, ROUND((ROUND(sr.finalamount)-(sr.finalamount)), 2) as round_off, lg.name as party_name, st.state_code_gst as state_code FROM `sale_return` sr LEFT JOIN ledger_master lg On sr.customer_id = lg.id LEFT JOIN own_states st ON lg.state = st.id WHERE sr.pharmacy_id = '".$pharmacy_id."'";
        if(isset($is_financial) && $is_financial == 1){
            $saleReturnQ .= " AND sr.financial_id = '".$financial_id."'";
        }
        if((isset($from) && $from != '') && (isset($to) && $to != '')){
    ...
    /*-----------------------------DAY BOOK REPORT- GAUTAM MAKWANA  - 31-01-2018 - END------------------------------*/
    ...
    /*------------------------------------------Free goods Report -Rajesh -06-02-2019-Start---------------------------*/
    ...
    function freegoods($from = null , $to = null, $bill_type = null, $ledger = null, $company = null, $type = null, $sub_type = null){
        ...
        if($bill_type == 'detail'){
            $dfreePurchaseQ = "SELECT p.vouchar_date, p.voucher_no, pm.product_name, c.name, (pd.free_qty * pd.f_rate) as amount, pd.free_qty FROM purchase p INNER JOIN purchase_details pd ON p.id = pd.purchase_id LEFT JOIN product_master pm ON pm.id = pd.product_id INNER JOIN company_master c ON c.id = pm.company_code WHERE p.pharmacy_id = '".$pharmacy_id."' AND p.financial_id = '".$financial_id."' AND pd.free_qty > 0 ";
            ...
            $dfreeSalesQ = "SELECT  pm.product_name, c.name, tbd.freeqty as freeqty1, tb.totaltaxgst as amount1, tb.invoice_no, tb.invoice_date FROM tax_billing AS tb INNER JOIN tax_billing_details AS tbd ON tbd.tax_bill_id = tb.id LEFT JOIN product_master AS pm ON tbd.product_id = pm.id INNER JOIN company_master AS c ON c.id = pm.company_code WHERE  tb.pharmacy_id = '".$pharmacy_id."' AND tb.financial_id = '".$financial_id."' AND tbd.freeqty > 0 ";

            if(isset($company) && !empty($company) ){
                $dfreeSalesQ .= " AND pm.company_code = '".$company."'";
            }
            if(isset($ledger) && !empty($ledger) ){
                $dfreeSalesQ .= " AND tb.customer_id = '".$ledger."'";
            }
            if((isset($from) && $from != '') && (isset($to) && $to != '')){
                $dfreeSalesQ .= " AND tb.invoice_date >= '".$from."' AND tb.invoice_date <= '".$to."' ";
            }
            $dfreeSalesR = mysqli_query($conn, $dfreeSalesQ);
            
            if($dfreeSalesR && mysqli_num_rows($dfreeSalesR) > 0){

                while($dsalesRow = mysqli_fetch_assoc($dfreeSalesR)){
                    $dsalesRow['type'] = 'Sales';
                    $dsalesRow['date'] = (isset($dsalesRow['invoice_date']) && $dsalesRow['invoice_date'] != '') ? $dsalesRow['invoice_date'] : '';
                    $dsalesRow['no'] = (isset($dsalesRow['invoice_no']) && $dsalesRow['invoice_no'] != '') ? $dsalesRow['invoice_no'] : '';
                    $dsalesRow['product_name'] = (isset($dsalesRow['product_name']) && $dsalesRow['product_name'] != '') ? $dsalesRow['product_name'] : '';
                    $dsalesRow['name'] = (isset($dsalesRow['name']) && $dsalesRow['name'] != '') ? $dsalesRow['name'] : '';
                    $dsalesRow['amount1'] = (isset($dsalesRow['amount1']) && $dsalesRow['amount1'] != '') ? $dsalesRow['amount1'] : '';
                    $dsalesRow['free_qty1'] = (isset($dsalesRow['freeqty1']) && $dsalesRow['freeqty1'] != '') ? $dsalesRow['freeqty1'] : '';
                    $data[] = $dsalesRow;
                }
                
            }
        }elseif($bill_type == 'summary'){
           $freePurchaseQ = "SELECT pm.product_name, c.name, SUM(pd.free_qty * pd.f_rate) as amount, SUM(pd.free_qty) as free_qty FROM purchase p INNER JOIN purchase_details pd ON p.id = pd.purchase_id LEFT JOIN product_master pm ON pm.id = pd.product_id INNER JOIN company_master c ON c.id = pm.company_code WHERE p.pharmacy_id = '".$pharmacy_id."' AND p.financial_id = '".$financial_id."' AND pd.free_qty > 0 ";
            if(isset($company) && !empty($company) ){
            ...
            $dfreeSalesQ = "SELECT  pm.product_name, c.name, SUM(tbd.freeqty) as freeqty1, SUM(tb.totaltaxgst) as amount1 FROM tax_billing AS tb INNER JOIN tax_billing_details AS tbd ON tbd.tax_bill_id = tb.id LEFT JOIN product_master AS pm ON tbd.product_id = pm.id INNER JOIN company_master AS c ON c.id = pm.company_code WHERE  tb.pharmacy_id = '".$pharmacy_id."' AND tb.financial_id = '".$financial_id."' AND tbd.freeqty > 0 ";
            ...
            if((isset($bill_type) && $bill_type == 'summary') && ((isset($type)) && ($type == 'all')) && ((isset($sub_type)) && ($sub_type == 'all1'))){
                $dfreeSalesQ .= " GROUP BY pm.product_name";
            }elseif((isset($bill_type) && $bill_type == 'summary') && (isset($type) && $type == 'company_wise')){
                $dfreeSalesQ .= " GROUP BY pm.company_code";
            }
            $dfreeSalesR = mysqli_query($conn, $dfreeSalesQ);
            ...
            if($dfreeSalesR && mysqli_num_rows($dfreeSalesR) > 0){
                ...
                while($dsalesRow = mysqli_fetch_assoc($dfreeSalesR)){
                    $dsalesRow['type'] = 'Sales';
                    if((isset($bill_type) && $bill_type == 'summary') && (isset($type) && $type == 'company_wise')){
                    $dsalesRow['product_name'] = (isset($dsalesRow['name']) && $dsalesRow['name'] != '') ? $dsalesRow['name'] : '';
                    }else if((isset($bill_type) && $bill_type == 'summary') && ((isset($type)) && ($type == 'all')) && ((isset($sub_type)) && ($sub_type == 'all1'))){
                     $dsalesRow['product_name'] = (isset($dsalesRow['product_name']) && $dsalesRow['product_name'] != '') ? $dsalesRow['product_name'] : '';
                    }
                    $dsalesRow['amount1'] = (isset($dsalesRow['amount1']) && $dsalesRow['amount1'] != '') ? $dsalesRow['amount1'] : '';
    ...
    /*------------------------------------------Free goods Report -Rajesh -06-02-2019-End---------------------------*/
