$( document ).ready(function() {
  $('body').on('change', '#bank_id', function () {
    if($(this).val() == 'cash'){
      selectedCash();
      assignVoucherNo('cash');
    }else{
      selectedBank();
      assignVoucherNo('bank');
    }
  });

  function selectedCash(){
    $('.narration_div').show();

    $('.payment_mode_div').hide();
    $('.payment_mode_date_div').hide();
    $('.payment_mode_no_div').hide();
    $('.card_name_div').hide();
    $('.other_reference_div').hide();
    $('.other_reference_div').hide();

    $('.payment_mode').val('');
    // $('.payment_mode_date').val('');
    $('.payment_mode_no').val('');
    $('.card_name').val('');
    $('.other_reference').val('');
  }
  ...
  function assignVoucherNo(type = null){
    $.ajax({
      type: "POST",
      url: 'ajax_second.php',
      data: {'type':type, 'action':'getPaymentVoucherNo'},
      dataType: "json",
