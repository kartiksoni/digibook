...
<?php 
    ...
    $reportqry = "SELECT tb.invoice_date, tb.invoice_no, tb.totaltaxgst as taxable_amount, lm.name, lm.gstno, SUM(tbd.totalamount) as total_amount FROM tax_billing AS tb INNER JOIN ledger_master AS lm ON lm.id = tb.customer_id INNER JOIN tax_billing_details tbd ON tb.id = tbd.tax_bill_id WHERE tb.pharmacy_id = '".$pharmacy_id."' AND tb.invoice_date >= '".$from."' AND tb.invoice_date <= '".$to."' AND lm.group_id = 10";
