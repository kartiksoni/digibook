...
<?php 
  ...
  if(isset($_POST['add'])){
    ...
    if($count > 0){
      $cashVoucherNo = getCashTransactionVoucherNo('receipt');
      $bankVoucherNo = getBankTransactionVoucherNo('receipt');
      ...
      for($i=0; $i < $count; $i++) {
        ...
        if($bank_id == 'cash'){//cash receipt
          $query = "INSERT INTO cash_transaction SET financial_id = '".$financial_id."', owner_id = '".$owner_id."', admin_id = '".$admin_id."', pharmacy_id = '".$pharmacy_id."', payment_type = 'receipt', voucher_no = '".$cashVoucherNo."', voucher_date = '".$voucher_date."', type = '".$type."', perticular = '".$perticular."', amount = '".$amount."', narration = '".$narration."', reverse_charge = '".$reverse_charge."', gst = '".$gst."', batch = '".$batch."', created = '".date('Y-m-d H:i:s')."', createdby = '".$_SESSION['auth']['id']."'";
        }else{//bank receipt
           $query = "INSERT INTO bank_transaction SET financial_id = '".$financial_id."', owner_id = '".$owner_id."', admin_id = '".$admin_id."', pharmacy_id = '".$pharmacy_id."', payment_type = 'receipt', voucher_no = '".$bankVoucherNo."', voucher_date = '".$voucher_date."', bank_id = '".$bank_id."', type = '".$type."', perticular = '".$perticular."', amount = '".$amount."', payment_mode = '".$payment_mode."', payment_mode_date = '".$payment_mode_date."', payment_mode_no = '".$payment_mode_no."', card_name = '".$card_name."', other_reference = '".$other_reference."', reverse_charge = '".$reverse_charge."', gst = '".$gst."', batch = '".$batch."', created = '".date('Y-m-d H:i:s')."', createdby = '".$_SESSION['auth']['id']."'";
        }
        $res = mysqli_query($conn, $query);
      }
      $_SESSION['msg']['success'] = "Receipt Added Successfully.";
      header('Location: view-receipt.php');exit;
    }else{
      $_SESSION['msg']['fail'] = "At least one receipt is required!";
  ...
  if(isset($_POST['update'])){
    ...
    if($bank_id == 'cash'){
      $query = "UPDATE cash_transaction SET voucher_date = '".$voucher_date."', type = '".$type."', perticular = '".$perticular."', amount = '".$amount."', narration = '".$narration."', reverse_charge = '".$reverse_charge."', gst = '".$gst."', modified = '".date('Y-m-d H:i:s')."', modifiedby = '".$_SESSION['auth']['id']."' WHERE id = '".$_GET['id']."'";
    }else{
      $query = "UPDATE bank_transaction SET voucher_date = '".$voucher_date."', bank_id = '".$bank_id."', type = '".$type."', perticular = '".$perticular."', amount = '".$amount."', payment_mode = '".$payment_mode."', payment_mode_date = '".$payment_mode_date."', payment_mode_no = '".$payment_mode_no."', card_name = '".$card_name."', other_reference = '".$other_reference."', reverse_charge = '".$reverse_charge."', gst = '".$gst."', modified = '".date('Y-m-d H:i:s')."', modifiedby = '".$_SESSION['auth']['id']."' WHERE id = '".$_GET['id']."'";
    }
    $res = mysqli_query($conn, $query);
    if($res){
      $_SESSION['msg']['success'] = "Receipt Update Successfully.";
      header('Location: view-receipt.php');exit;
    }else{
      $_SESSION['msg']['fail'] = "Receipt Update Fail!";
      header('Location: receipt.php');exit;
    }
  }
  /*------------------UPDATE RECEIPT START-----------------------*/
  ...
  if(isset($_GET['id']) && $_GET['id'] != ''){
    if(isset($_GET['type']) && $_GET['type'] == 'cash'){
      $query = "SELECT *, 'cash' as bank_id, 'cash_receipt' as record_type  FROM cash_transaction WHERE pharmacy_id = '".$pharmacy_id."' AND financial_id = '".$financial_id."' AND payment_type = 'receipt' AND id = '".$_GET['id']."'";
      $res = mysqli_query($conn, $query);
    ...
    }elseif(isset($_GET['type']) && $_GET['type'] == 'bank'){
      $query = "SELECT *, 'bank_receipt' as record_type FROM bank_transaction WHERE pharmacy_id = '".$pharmacy_id."' AND financial_id = '".$financial_id."' AND payment_type = 'receipt' AND id = '".$_GET['id']."'";
      $res = mysqli_query($conn, $query);
      if($res && mysqli_num_rows($res) > 0){
        $editData = mysqli_fetch_assoc($res);
      }else{
...
<head>
  ...
  <title>DigiBooks | Bank Receipt</title>
  <!-- plugins:css -->
                  ...
                  <div class="card-body">
                    <div class="row form-group">
                      <div class="col-md-2">
                        ...
                        <input type="text" class="form-control" id="voucher_no" name="voucher_no" placeholder="Voucher No." value="<?php echo (isset($editData['voucher_no'])) ? $editData['voucher_no'] : getCashTransactionVoucherNo('receipt'); ?>" required readonly>
                      </div>
                      <div class="col-md-2">
                      ...
                      <div class="col-md-2">
                        <label for="bank_id">Cash / Bank <span class="text-danger">*</span></label>
                        <select class="js-example-basic-single" name="bank_id" id="bank_id" style="width:100%" data-parsley-errors-container="#error-bank" required>
                          <!-- <option value="">Select Cash/Bank</option> -->
                          <option value="cash" <?php echo (isset($_GET['type']) && $_GET['type'] != 'cash') ? 'disabled' : ''; ?> >Cash</option>
                          <?php if(isset($allBank) && !empty($allBank)){ ?>
                            <?php foreach ($allBank as $key => $value) { ?>
                              <option value="<?php echo (isset($value['id'])) ? $value['id'] : ''; ?>" <?php echo (isset($editData['bank_id']) && $editData['bank_id'] == $value['id']) ? 'selected' : ''; ?>  <?php echo (isset($_GET['type']) && $_GET['type'] != 'bank') ? 'disabled' : ''; ?> ><?php echo (isset($value['name'])) ? $value['name'] : ''; ?></option>
                            <?php } ?>
                          <?php } ?>
                        </select>
                ...
                <div class="card">
                  <div class="card-body">
                      ...
                      <div class="detail-row">
                          <?php if(isset($editData) && !empty($editData)){ ?>
                            <div class="row form-group">
                              ...
                              <div class="col-md-2">
                                <label for="perticular">Particular <span class="text-danger">*</span></label>
                              ...
                              <div class="col-md-2">
                                ...
                                <input type="text" class="form-control amount onlynumber" name="amount" value="<?php echo (isset($editData['amount'])) ? $editData['amount'] : ''; ?>" placeholder="Amount">
                              </div>
                              <div class="col-md-3 narration_div" <?php echo (isset($_GET['type']) && $_GET['type'] != 'cash') ? 'style="display:none;"' : ''; ?>>
                                <label for="narration">Narration</label>
                                <textarea class="form-control" name="narration" rows="1" style="resize: both;"><?php echo (isset($editData['narration'])) ? $editData['narration'] : ''; ?></textarea>
                              </div>
                              <div class="col-md-2 payment_mode_div" <?php echo (isset($_GET['type']) && $_GET['type'] != 'bank') ? 'style="display:none;"' : ''; ?> >
                                <label for="payment_mode">Payment Mode <span class="text-danger">*</span></label>
                                <select class="form-control payment_mode" name="payment_mode" required>
                              ...
                              <div class="col-md-2 payment_mode_date_div" <?php echo ((isset($_GET['type']) && $_GET['type'] != 'bank') || (isset($editData['payment_mode'])) && ($editData['payment_mode'] == 'net_banking' || $editData['payment_mode'] == 'credit_debit_card' || $editData['payment_mode'] == 'other')) ? 'style="display:none;"' : ''; ?> >
                                <?php 
                              ...
                              <div class="col-md-2 payment_mode_no_div" <?php echo ((isset($_GET['type']) && $_GET['type'] != 'bank') || (isset($editData['payment_mode']) && $editData['payment_mode'] == 'other')) ? 'style="display: none;"' : ''; ?> >
                              ...
                              <div class="col-md-2 card_name_div" <?php echo ((isset($_GET['type']) && $_GET['type'] != 'bank') || (isset($editData['payment_mode']) && $editData['payment_mode'] != 'credit_debit_card')) ? 'style="display: none;"' : ''; ?> >
                              ...
                              <div class="col-md-2 other_reference_div" <?php echo ((isset($_GET['type']) && $_GET['type'] != 'bank') || (isset($editData['payment_mode']) && $editData['payment_mode'] != 'other')) ? 'style="display: none;"' : ''; ?> >
                            ...
                            <div class="row form-group">
                              ...
                              <div class="col-md-2 offset-md-6 text-right" style="padding-top: 25px;">
                          ...
                          <?php }else{ ?>
                            <div class="row form-group">
                              ...
                              <div class="col-md-2">
                                <label for="amount">Amount <span class="text-danger">*</span></label>
                                <input type="text" class="form-control amount onlynumber" name="amount[]" placeholder="Amount">
                              </div>
                              <div class="col-md-3 narration_div">
                                <label for="narration">Narration</label>
                                <textarea class="form-control" name="narration[]" rows="1" style="resize: both;"></textarea>
                              </div>
                              <div class="col-md-2 payment_mode_div" style="display: none;">
                                <label for="payment_mode">Payment Mode <span class="text-danger">*</span></label>
                                <select class="form-control payment_mode" name="payment_mode[]" required>
                                  <option value="">Select Mode</option>
                    ...
                    <div id="detail-footer">
                      <a href="view-receipt.php" class="btn btn-light">Back</a>
