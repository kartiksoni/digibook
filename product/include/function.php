<?php    
    ...
    $update_token = "UPDATE users SET password_reset = '1' reset_token = '".$reset_token."' WHERE id = '".$user_id."'";
    $update_run = mysqli_query($conn, $update_token);

    if($update_run){
      $user_details = "SELECT id, name FROM users WHERE id = '".$user_id."'";
      $user_run = mysqli_query($conn, $user_details);
      $user_data = mysqli_fetch_assoc($user_run);
      $user_name = $user_data['name'];

      $reset_button_link = BASE_URL . "changepassword.php?reset_token=" . $reset_token;
      ...
      $html = '<!doctype html>
        ...
        $html .= '<p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; Margin-bottom: 15px;">
                        Thanks,<br>The Digiwallet Team</p>
                                        </td>
                                    </tr>
                                </table>
                            </td>
...
function smtpmail($to, $from='', $replyto='', $subject, $data = '', $username='', $password='', $attachfilepath=''){
        ...
        if(!$mail->send()) {
            ...
            $_SESSION['msg']['success'] = 'Your Mail has been sent Successfully.';
            return true;
        }
